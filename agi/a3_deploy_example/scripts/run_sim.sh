...
Environment:
  A3_SIM_FLAVOR=auto|source|standalone
      ...
      source     use AGI's A3_MuJoCo_Sim install package
...
find_source_sim_install() {
  ...
  candidates+=(
    "${AGI_ROOT}/A3_MuJoCo_Sim/aimrt_mujoco_sim/build/install"
    "${AGI_ROOT}/aimrt_mujoco_sim_source/aimrt_mujoco_sim/build/install"
  )
