...
def test_nominal_inputs_reproduce_the_blueprint_table():
    ...
    fr = out["frozen"]
    # v2.2:landing 独扛。w_land = 1.2×8.1×100/(0.7×0.8) ≈ 1735.7;pass_net 冻结 0
    assert fr["virtual_landing_weight"] == pytest.approx(1735.7, abs=0.3)
...
def test_one_shot_prize_lives_only_in_the_table_group():
    ...
    assert out["accounting"]["per_swing_table_prize"] == pytest.approx(1388.6, abs=1.0)
