...
def _parse_action_ball_actor_obs_contract(
    ...
    (``193 + N``); the preferred teacher-start successor is ``194 + N``.
    The legacy layout remains ``181 + N``.  Every spelling
...
def runtime_execution_facts(
    ...
    critic_facts = {}
    actor_contract_name = getattr(actor_contract, "name", None)
    if actor_contract_name == (
        _STAGE1_NATURAL_CLIP_PADDLE_WORLD_V2_ACTOR_OBS_CONTRACT
    ):
