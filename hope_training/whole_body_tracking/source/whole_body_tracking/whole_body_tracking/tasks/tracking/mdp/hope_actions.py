...
followed by a deterministic clamp of the position targets to the joint limits (a numeric transform,
not a rejection gate — it emits no failure status). The policy still emits 31 raw values, but the two
passive head joints (idx 3, 4) are held at their articulation default and their applied-action
feedback is zeroed, matching a deploy runner that holds the neck passive. ``applied_raw_actions`` (the
exact 31 values that were applied, with the passive columns zeroed) is fed back as the next tick's
``last_action`` observation.

The residual form and ``action_scale`` are the shared public action adapter: the reference
runner applies the identical transform so training and deployment agree.
"""
