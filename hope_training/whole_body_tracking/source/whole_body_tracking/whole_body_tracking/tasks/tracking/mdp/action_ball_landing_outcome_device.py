...
class ActionBallLandingOutcomeDeviceCoordinator:
    ...
    def action_epoch_reward_facts_v1(
        ...
        """Decode the immutable epoch R06 slice without touching live state."""

        owner = self._action_ball_full_mdp_epoch_owner
        if owner is None:
            raise LandingOutcomeDeviceError(
                "R06 direct Reward facts require a bound ActionEpoch owner"
            )
        if __package__:
            from . import action_ball_full_mdp_epoch as epoch_v1
        else:
            import action_ball_full_mdp_epoch as epoch_v1
        if type(epoch) is not epoch_v1.ActionEpochRecord:
            raise LandingOutcomeDeviceError(
                "epoch must be the exact current ActionEpochRecord"
            )
        # Snapshot freshness for the legacy 14-term Reward reader only; these
        # scalar journal fields never identify or authorize a shot close.
        current = owner.current()
        if (
            epoch.epoch != current.epoch
            or epoch.version != current.version
        ...
        owner_slot = epoch_v1.OWNER_ORDER.index("r06_landing_outcome")
        valid = epoch.fact_valid_bits[:, :, owner_slot].detach().clone()
        values = epoch.fact_f32[:, :, owner_slot].detach().clone()
        faults = epoch.owner_fault_bits[:, :, owner_slot].detach().clone()
        present = torch.bitwise_and(
            valid, R06_ACTION_EPOCH_PRESENT
        ).ne(0)
        source_valid = torch.bitwise_and(
            valid, R06_ACTION_EPOCH_SOURCE_VALID
        ).ne(0)
        policy_eligible = torch.bitwise_and(
            valid, R06_ACTION_EPOCH_POLICY_ELIGIBLE
        ).ne(0)
        eligible = present & source_valid & faults.eq(0)
        policy_eligible &= eligible

        def finite_zero(offset: int) -> torch.Tensor:
            value = values[:, :, offset]
            finite = torch.isfinite(value)
            return torch.where(
                eligible & finite, value, torch.zeros_like(value)
            )

        if __package__:
            from .action_ball_full_mdp_lean_rewards import DirectR06RewardFacts
        else:
            from action_ball_full_mdp_lean_rewards import DirectR06RewardFacts
        return DirectR06RewardFacts(
            eligible=eligible,
            policy_eligible=policy_eligible,
            producer_fault_bits=faults,
            common_on_table_outcome=finite_zero(
                R06_ACTION_EPOCH_COMMON_ON_TABLE_F32
            ),
            canonical_total=finite_zero(
                R06_ACTION_EPOCH_CANONICAL_TOTAL_F32
            ),
            placement_treatment_gain=finite_zero(
    ...
    def _poison_full_mdp_reward_protocol(self, reason: str) -> None:
        """Record one device-resident Reward protocol fault and retain debt."""

        self._full_mdp_reward_binding_window_open = False
        fault_bits = torch.full(
            (self.num_envs,),
            FAULT_PAYMENT_EPOCH,
            dtype=torch.int64,
            device=self.device,
        )
        self._lifecycle_fault_bits.bitwise_or_(fault_bits)
        self._device_sticky_poison.fill_(True)
        self._record_fault_events(fault_bits)
        self._increment_mutation()
        ...
        self._full_mdp_reward_close_debt = True
    ...
    def _bind_full_mdp_reward_graph_from_top(
        ...
        *,
        ...
        """Construction-bind R06 to the exact top Reward owner exactly once."""

        if not self._full_mdp_reward_binding_window_open:
            raise LandingOutcomeDeviceError(
                "R06 full-MDP Reward graph binding is closed"
            )
        self._full_mdp_reward_binding_window_open = False
        owner_type = type(runtime_owner)
        source = inspect.getsourcefile(owner_type)
        expected_source = Path(__file__).with_name(
            "action_ball_full_mdp_runtime_owner.py"
        ).resolve()
        validator = getattr(
            runtime_owner, "require_owned_full_mdp_pre_reward", None
        )
        if (
            ordered_consumers != CONSUMERS
            or owner_type.__name__ != "ActionBallFullMdpRuntimeOwner"
            or source is None
            ...
            or getattr(validator, "__func__", None)
            is not getattr(
                owner_type, "require_owned_full_mdp_pre_reward", None
            )
            or getattr(runtime_owner, "_reward_owner_binding_open", None)
            is not True
            or getattr(runtime_owner, "_reward_owner_binding", None)
            is not None
            or self._active_full_mdp_reward_cycle_identity is not None
            or self._full_mdp_reward_graph_bound
        ):
            self._full_mdp_reward_poisoned = True
            self._full_mdp_reward_poison_reason = (
                "R06 full-MDP Reward construction bind differed"
            )
            raise LandingOutcomeDeviceError(
                "R06 full-MDP Reward requires one exact top construction bind"
            )
    ...
    def open_full_mdp_reward_cycle(
        ...
        *,
        ...
        """Open only from the bound top's exact active pre-Reward publication."""

        if type(control_step) is not int or control_step < 0:
            raise LandingOutcomeDeviceError(
                "R06 Reward control_step must be a non-negative exact int"
            )
        validator = self._full_mdp_reward_prepublication_validator
        if (
            not self._full_mdp_reward_graph_bound
            or runtime_owner is not self._full_mdp_reward_runtime_owner
            or not callable(validator)
            or self._active_full_mdp_reward_cycle_identity is not None
            or self._active_full_mdp_reward_cycle_token is not None
            or self._full_mdp_reward_close_debt
            or self._full_mdp_reward_poisoned
        ):
            self._poison_full_mdp_reward_protocol(
                "R06 Reward open was foreign, repeated, or debt-bearing"
            )
            raise LandingOutcomeDeviceError(
                "R06 full-MDP Reward open requires the bound top and no prior debt"
            )
        try:
            owned = validator(
                pre_reward_publication,
                runtime_lease=runtime_owner.full_mdp_runtime_lease,
                control_step=control_step,
            )
        except BaseException:
            self._poison_full_mdp_reward_protocol(
                "R06 Reward prepublication authentication failed"
            )
            raise
        if owned is None:
            self._poison_full_mdp_reward_protocol(
                "R06 Reward prepublication authentication returned no view"
            )
            raise LandingOutcomeDeviceError(
                "R06 Reward prepublication validation returned no owned view"
            )
        cycle_identity = object()
        epoch = torch.full(
            (self.num_envs,),
        ...
        token = object.__new__(LandingOutcomeFullMdpRewardCycleToken)
        _FULL_MDP_REWARD_CYCLE_REGISTRY[token] = _FullMdpRewardCyclePayload(
            owner_identity=self._owner_identity,
            cycle_identity=cycle_identity,
            runtime_owner=runtime_owner,
            pre_reward_publication=pre_reward_publication,
            control_step=control_step,
            reward_epoch=epoch.detach().clone(),
        )
        self._active_full_mdp_reward_cycle_identity = cycle_identity
        self._active_full_mdp_reward_cycle_token = token
        self._active_full_mdp_reward_prepublication = pre_reward_publication
        self._active_full_mdp_reward_epoch = epoch
        self._active_full_mdp_reward_verdicts = {}
        self._active_full_mdp_reward_views = set()
        self._active_full_mdp_reward_runtime_owner = runtime_owner
        self._latest_full_mdp_reward_close = None
        self._full_mdp_reward_close_debt = True
        return token

    def _require_owned_full_mdp_reward_cycle(
        self,
        token: object,
    ) -> _FullMdpRewardCyclePayload:
        try:
            payload = _FULL_MDP_REWARD_CYCLE_REGISTRY.get(token)
        except TypeError:
            payload = None
        if (
            type(token) is not LandingOutcomeFullMdpRewardCycleToken
            or token is not self._active_full_mdp_reward_cycle_token
            or payload is None
            or payload.owner_identity is not self._owner_identity
            or payload.cycle_identity
            is not self._active_full_mdp_reward_cycle_identity
            or payload.runtime_owner is not self._full_mdp_reward_runtime_owner
            or payload.pre_reward_publication
            is not self._active_full_mdp_reward_prepublication
            or self._full_mdp_reward_poisoned
        ):
            self._poison_full_mdp_reward_protocol(
                "R06 Reward cycle token was foreign, stale, or replayed"
            )
            raise LandingOutcomeDeviceError(
                "R06 full-MDP Reward cycle token is foreign, stale, or replayed"
            )
        return payload
    ...
    def view(
        self,
        consumer: str,
        reward_cycle_token: LandingOutcomeFullMdpRewardCycleToken | None = None,
        *,
        reward_epoch: torch.Tensor | None = None,
    ) -> SharedLandingOutcomeDeviceView:
        """Return one top-cycle-bound consumer view; invalid rows remain zero."""

        self._require_operable(allow_full_mdp_reward_cycle=True)
        if reward_epoch is not None:
            self._poison_full_mdp_reward_protocol(
                "caller-authored R06 Reward epochs are tombstoned"
            )
            raise LandingOutcomeDeviceError(
                "R06 Reward view requires the exact top-issued cycle token; caller epochs are tombstoned"
            )
        index = self._consumer(consumer)
        payload = self._require_owned_full_mdp_reward_cycle(
            reward_cycle_token
        )
        expected_consumer = CONSUMERS[len(self._active_full_mdp_reward_views)]
        if (
            consumer != expected_consumer
            or len(self._active_full_mdp_reward_verdicts)
            != len(self._active_full_mdp_reward_views)
        ):
            self._poison_full_mdp_reward_protocol(
                "R06 Reward consumer view order differed"
            )
            raise LandingOutcomeDeviceError(
                "R06 full-MDP Reward consumer view order differs"
            )
        epoch = payload.reward_epoch
        self._active_full_mdp_reward_views.add(consumer)
        epoch_grid = epoch.unsqueeze(1).expand(self._mailbox_shape)
        bit = 1 << index
        state_visible = (self._mailbox_state == MAILBOX_SETTLED_UNPAID) | (
            self._mailbox_state == MAILBOX_PARTIALLY_PAID
        )
        unpaid = torch.bitwise_and(self._mailbox_paid_mask, bit) == 0
        poisoned = self._device_sticky_poison.unsqueeze(1).expand(
            self._mailbox_shape
        )
        poisoned_view = state_visible & unpaid & poisoned
        candidate = (
            state_visible
            & unpaid
            & ~self._mailbox_consumer_blocked[..., index]
            & ~poisoned
        )
        previous_epoch = self._mailbox_view_epoch[..., index]
        invalid_epoch = candidate & (epoch_grid < 0)
        first_view = previous_epoch < 0
        first_late = (
            candidate
            & first_view
            & (epoch_grid > self._mailbox_settlement_control_step)
        )
        first_early = (
            candidate
            & first_view
        ...
        duplicate = candidate & (previous_epoch == epoch_grid) & (previous_epoch >= 0)
        missed = first_late | (
            candidate & (previous_epoch >= 0) & (epoch_grid > previous_epoch)
        )
        regression = candidate & (previous_epoch >= 0) & (epoch_grid < previous_epoch)
        good = candidate & ~(
            invalid_epoch | first_early | duplicate | missed | regression
        )
        faults = torch.zeros(self._mailbox_shape, dtype=torch.int64, device=self.device)
        faults = self._with_fault(faults, duplicate, FAULT_DUPLICATE_VIEW)
        faults = self._with_fault(faults, missed, FAULT_MISSED_PAYMENT)
        faults = self._with_fault(
            faults, invalid_epoch | first_early | regression, FAULT_PAYMENT_EPOCH
        )
        faults = self._with_fault(
            faults, poisoned_view, FAULT_SAFETY_CLEANUP
        )
        self._mailbox_fault_bits.bitwise_or_(faults)
        self._mailbox_consumer_blocked[..., index].copy_(
            self._mailbox_consumer_blocked[..., index]
            | duplicate
            | missed
            | invalid_epoch
        ...
        _masked_copy_(
            self._mailbox_view_epoch[..., index], epoch_grid, good
        )
        self._record_fault_events(faults)
        changed = (good | (faults != 0)).any().to(torch.int64)
        self._mutation_version.add_(changed)
        return self._build_view(good, index)

    def _consumer(self, consumer: str) -> int:
        if consumer not in self._consumer_index:
            raise LandingOutcomeDeviceError(
                f"unknown consumer {consumer!r}; expected one of {CONSUMERS!r}"
            )
        return self._consumer_index[consumer]

    def _mask_view(self, value: torch.Tensor, eligible: torch.Tensor) -> torch.Tensor:
        return torch.where(
            _expand(eligible, value.ndim), value, torch.zeros_like(value)
        )

    def _build_view(
        self, eligible: torch.Tensor, consumer_index: int
    ) -> SharedLandingOutcomeDeviceView:
        treatment_eligible = (
            eligible
            if consumer_index == 1
        ...
        treatment_family_code = torch.where(
            treatment_eligible,
            self._mailbox_c10_family_code,
            torch.full_like(self._mailbox_c10_family_code, -1),
        )
        key = self._key_storage("mailbox")
        masked_key = DeviceLandingOutcomeKey(
            **{
                name: self._mask_view(getattr(key, name), eligible)
                for name in _KEY_FIELDS
            }
        )
        return SharedLandingOutcomeDeviceView(
            eligible=eligible.detach().clone(),
            policy_eligible=eligible & self._mailbox_policy_eligible,
            mailbox_state=self._mask_view(self._mailbox_state, eligible),
            physical_retired=self._mask_view(
                self._mailbox_physical_retired, eligible
            ),
            task_key=masked_key,
            full_key_sha256=self._mask_view(self._mailbox_full_key_sha256, eligible),
            full_key_receipt_sha256=self._mask_view(
                self._mailbox_full_key_receipt_sha256, eligible
            ),
            ball_generation=self._mask_view(self._mailbox_ball_generation, eligible),
            reserved_flight_slot=self._mask_view(
                self._mailbox_reserved_flight_slot, eligible
            ),
            settlement_cause=self._mask_view(
                self._mailbox_settlement_cause, eligible
            ),
            canonical_reason_code=self._mask_view(
                self._mailbox_canonical_reason_code, eligible
            ),
            source_control_step=self._mask_view(
                self._mailbox_source_control_step, eligible
            ),
            source_physics_substep=self._mask_view(
                self._mailbox_source_physics_substep, eligible
            ),
            settlement_control_step=self._mask_view(
                self._mailbox_settlement_control_step, eligible
            ),
            settlement_physics_substep=self._mask_view(
                self._mailbox_settlement_physics_substep, eligible
            ),
            contact_valid=self._mask_view(self._mailbox_contact_valid, eligible),
            selected_contact_ball_center_m=self._mask_view(
                self._mailbox_contact_ball_center_m, eligible
            ),
            first_plane_crossing_present=self._mask_view(
                self._mailbox_crossing_present, eligible
            ),
            first_plane_crossing_valid=self._mask_view(
                self._mailbox_crossing_valid, eligible
            ),
            first_plane_crossing_nonfinite=self._mask_view(
                self._mailbox_crossing_nonfinite, eligible
            ),
            first_plane_crossing_xy_m=self._mask_view(
                self._mailbox_crossing_xy_m, eligible
            ),
            first_plane_crossing_control_step=self._mask_view(
                self._mailbox_crossing_stamp_control, eligible
            ),
            first_plane_crossing_physics_substep=self._mask_view(
                self._mailbox_crossing_stamp_substep, eligible
            ),
            ball_center_net_crossed=self._mask_view(
                self._mailbox_net_crossed, eligible
            ),
            ball_center_net_clear=self._mask_view(
                self._mailbox_net_clear, eligible
            ),
            ball_center_net_crossing_control_step=self._mask_view(
                self._mailbox_net_stamp_control, eligible
            ),
            ball_center_net_crossing_physics_substep=self._mask_view(
                self._mailbox_net_stamp_substep, eligible
            ),
            common_on_table_outcome=self._mask_view(
                self._mailbox_common_on_table, eligible
            ),
            on_opponent_table=self._mask_view(
                self._mailbox_on_opponent_table, eligible
            ),
            placement_error_m=self._mask_view(
                self._mailbox_placement_error_m, eligible
            ),
            broad_kernel=self._mask_view(self._mailbox_broad_kernel, eligible),
            narrow_kernel=self._mask_view(self._mailbox_narrow_kernel, eligible),
            blended_kernel=self._mask_view(self._mailbox_blended_kernel, eligible),
            table_gate=self._mask_view(self._mailbox_table_gate, eligible),
            canonical_total=self._mask_view(
                self._mailbox_canonical_total, eligible
            ),
            treatment_family_code=treatment_family_code,
            c10_projection_sha256=self._mask_view(
                self._mailbox_c10_projection_sha256, treatment_eligible
            ),
            placement_treatment_gain=self._mask_view(
                self._mailbox_placement_treatment_gain, treatment_eligible
            ),
            consumer_view_epoch=self._mask_view(
                self._mailbox_view_epoch[..., consumer_index], eligible
            ),
            consumer_paid_mask=self._mask_view(self._mailbox_paid_mask, eligible),
            payment_values=self._mask_view(self._mailbox_payment_values, eligible),
            fault_bits=self._mask_view(self._mailbox_fault_bits, eligible),
        )
    ...
    def record_payment(
        ...
        *,
        ...
        """Acknowledge and pay exactly one consumer; zero reward still pays."""

        self._require_operable(allow_full_mdp_reward_cycle=True)
        if reward_epoch is not None:
            self._poison_full_mdp_reward_protocol(
                "caller-authored R06 payment epochs are tombstoned"
            )
            raise LandingOutcomeDeviceError(
                "R06 Reward payment requires the exact top-issued cycle token; caller epochs are tombstoned"
            )
        index = self._consumer(consumer)
        cycle_payload = self._require_owned_full_mdp_reward_cycle(
            reward_cycle_token
        )
        cycle_identity = self._active_full_mdp_reward_cycle_identity
        if (
            cycle_identity is None
            or cycle_payload.cycle_identity is not cycle_identity
            or consumer not in self._active_full_mdp_reward_views
            or consumer
            != CONSUMERS[len(self._active_full_mdp_reward_verdicts)]
        ):
            self._poison_full_mdp_reward_protocol(
                "R06 payment lacked its exact current-cycle view"
            )
            raise LandingOutcomeDeviceError(
                "R06 payment requires one fresh owner-issued view in the active cycle"
            )
        epoch = cycle_payload.reward_epoch
        mask = self._tensor(
            mask, name="mask", shape=self._mailbox_shape, dtype=torch.bool
        )
        token = self._tensor(
            full_key_sha256,
            name="full_key_sha256",
            shape=self._mailbox_shape + (TOKEN_BYTES,),
            dtype=torch.uint8,
        )
        generation = self._tensor(
            ball_generation,
            name="ball_generation",
            shape=self._mailbox_shape,
            dtype=torch.int64,
        )
        raw = self._float_tensor(
            raw_reward, name="raw_reward", shape=self._mailbox_shape
        )
        epoch_grid = epoch.unsqueeze(1).expand(self._mailbox_shape)
        bit = 1 << index
        state_visible = (self._mailbox_state == MAILBOX_SETTLED_UNPAID) | (
        ...
        duplicate = mask & (torch.bitwise_and(self._mailbox_paid_mask, bit) != 0)
        identity = torch.eq(token, self._mailbox_full_key_sha256).all(dim=-1) & (
            generation == self._mailbox_ball_generation
        )
        viewed = self._mailbox_view_epoch[..., index] == epoch_grid
        epoch_valid = epoch_grid >= 0
        finite = torch.isfinite(raw)
        if index == 0:
            expected = self._mailbox_common_on_table.to(self.dtype)
            treatment_binding_valid = torch.ones_like(state_visible)
        else:
            treatment_binding_valid = (
            ...
            expected = (
                self._mailbox_canonical_total
                * self._mailbox_placement_treatment_gain
            )
        expected = torch.where(
            self._mailbox_policy_eligible, expected, torch.zeros_like(expected)
        )
        value_valid = finite & treatment_binding_valid & torch.eq(raw, expected)
        poisoned = self._device_sticky_poison.unsqueeze(1).expand(
            self._mailbox_shape
        )
        accepted = (
        ...
        rejected = mask & ~accepted
        faults = torch.zeros(self._mailbox_shape, dtype=torch.int64, device=self.device)
        faults = self._with_fault(faults, duplicate, FAULT_DUPLICATE_PAYMENT)
        faults = self._with_fault(
            faults, mask & state_visible & ~viewed, FAULT_PAYMENT_BEFORE_VIEW
        )
        faults = self._with_fault(
            faults, mask & state_visible & viewed & ~epoch_valid, FAULT_PAYMENT_EPOCH
        )
        faults = self._with_fault(
            faults, mask & state_visible & viewed & epoch_valid & ~identity, FAULT_KEY_BINDING
        )
        faults = self._with_fault(
            faults,
            mask & state_visible & viewed & epoch_valid & identity & ~value_valid,
            FAULT_INVALID_PAYMENT,
        )
        faults = self._with_fault(
            faults, mask & ~state_visible & ~duplicate, FAULT_INVALID_PAYMENT
        )
        faults = self._with_fault(
            faults, mask & poisoned, FAULT_SAFETY_CLEANUP
        )
        self._mailbox_fault_bits.bitwise_or_(faults)
        self._mailbox_consumer_blocked[..., index].copy_(
            self._mailbox_consumer_blocked[..., index] | rejected
        )
        paid_value = torch.where(accepted, raw, self._mailbox_payment_values[..., index])
        self._mailbox_payment_values[..., index].copy_(paid_value)
        self._mailbox_payment_epoch[..., index].copy_(
            torch.where(accepted, epoch_grid, self._mailbox_payment_epoch[..., index])
        )
        new_paid = torch.where(
            accepted,
            torch.bitwise_or(self._mailbox_paid_mask, bit),
            self._mailbox_paid_mask,
        )
        self._mailbox_paid_mask.copy_(new_paid)
        now_paid = accepted & (new_paid == FULL_CONSUMER_MASK)
        now_partial = accepted & ~now_paid
        _masked_fill_(self._mailbox_state, now_partial, MAILBOX_PARTIALLY_PAID)
        _masked_fill_(self._mailbox_state, now_paid, MAILBOX_PAID)
        self._payment_totals[index].add_(accepted.to(torch.int64).sum())
        self._record_fault_events(faults)
        self._increment_mutation()
        verdict = object.__new__(LandingOutcomeFullMdpRewardPaymentVerdict)
        payload = _FullMdpRewardPaymentPayload(
            owner_identity=self._owner_identity,
            cycle_identity=cycle_identity,
            consumer=consumer,
            reward_epoch=epoch.detach().clone(),
        ...
        _FULL_MDP_REWARD_PAYMENT_REGISTRY[verdict] = payload
        if consumer not in self._active_full_mdp_reward_verdicts:
            self._active_full_mdp_reward_verdicts[consumer] = verdict
        return verdict
    ...
    def require_owned_full_mdp_reward_payment(
        ...
        *,
        ...
        """Authenticate one exact R06 verdict for the top Reward owner."""

        self._require_operable(allow_full_mdp_reward_cycle=True)
        payload = _owned_full_mdp_reward_payment(verdict)
        cycle_token = self._active_full_mdp_reward_cycle_token
        cycle_payload = self._require_owned_full_mdp_reward_cycle(cycle_token)
        if (
            payload.owner_identity is not self._owner_identity
            or payload.cycle_identity
            is not self._active_full_mdp_reward_cycle_identity
            or payload.consumer != consumer
            or self._active_full_mdp_reward_verdicts.get(consumer) is not verdict
            or type(control_step) is not int
            or control_step < 0
            or runtime_owner is not cycle_payload.runtime_owner
            or control_step != cycle_payload.control_step
        ):
            self._poison_full_mdp_reward_protocol(
                "R06 Reward verdict identity/current-cycle token differed"
            )
            raise LandingOutcomeDeviceError(
                "R06 full-MDP Reward verdict identity/epoch differs"
            )
        return verdict

    def _close_paid_many_for_full_mdp_reward(self) -> torch.Tensor:
        """Close every paid+retired mailbox row, including multi-slot envs."""

        closed = torch.zeros(
            self._mailbox_shape, dtype=torch.bool, device=self.device
        )
        epoch = self._active_full_mdp_reward_epoch
        if epoch is None:
            raise LandingOutcomeDeviceError(
                "R06 Reward close has no active owner epoch"
            )
        epoch_grid = epoch.unsqueeze(1).expand(self._mailbox_shape)
        for slot_index in range(self.mailbox_capacity):
            mask = (
                (self._mailbox_state[:, slot_index] == MAILBOX_PAID)
                & self._mailbox_physical_retired[:, slot_index]
                & ~self._device_sticky_poison
                & (
                    self._mailbox_payment_epoch[:, slot_index, 0]
                    == epoch_grid[:, slot_index]
                )
                & (
                    self._mailbox_payment_epoch[:, slot_index, 1]
                    == epoch_grid[:, slot_index]
                )
            )
            result = self.close_paid(
                mask=mask,
                mailbox_slot=torch.full(
                    (self.num_envs,),
                    slot_index,
                    dtype=torch.int64,
                    device=self.device,
                ...
                full_key_sha256=(
                    self._mailbox_full_key_sha256[:, slot_index].detach().clone()
                ),
                ball_generation=(
                    self._mailbox_ball_generation[:, slot_index].detach().clone()
                ),
            )
            closed[:, slot_index].copy_(result.accepted)
        return closed
    ...
    def close_paid_many(
        ...
        *,
        control_step: int,
        pre_reward_publication: object,
        ordered_consumers: tuple[str, ...],
        ordered_payment_verdicts: tuple[
            LandingOutcomeFullMdpRewardPaymentVerdict, ...
        ],
        runtime_owner: object,
    ) -> LandingOutcomeFullMdpRewardCloseReceipt:
        """Close the exact two-payment epoch and its complete mailbox grid."""
        ...
        if (
            ...
            or type(control_step) is not int
            or control_step != cycle_payload.control_step
            or type(ordered_consumers) is not tuple
            or ordered_consumers != CONSUMERS
            or type(ordered_payment_verdicts) is not tuple
            or len(ordered_payment_verdicts) != CONSUMER_COUNT
            or len({id(value) for value in ordered_payment_verdicts})
            != CONSUMER_COUNT
            or self._active_full_mdp_reward_runtime_owner is not runtime_owner
            or pre_reward_publication
            is not cycle_payload.pre_reward_publication
        ):
            self._poison_full_mdp_reward_protocol(
                "R06 Reward close cycle/order/token differed"
            )
            raise LandingOutcomeDeviceError(
                "R06 full-MDP Reward close cycle/order/epoch differs"
            )
        for consumer, verdict in zip(CONSUMERS, ordered_payment_verdicts):
            if (
                self._active_full_mdp_reward_verdicts.get(consumer) is not verdict
                or self.require_owned_full_mdp_reward_payment(
                    verdict,
                    consumer=consumer,
                    control_step=control_step,
                    runtime_owner=runtime_owner,
                ...
                self._poison_full_mdp_reward_protocol(
                    "R06 Reward close exact verdict set differed"
                )
                raise LandingOutcomeDeviceError(
                    "R06 full-MDP Reward close verdict set differs"
                )

        # ``close_paid`` is the sole durable row-release implementation.  Its
        # all-grid caller is already authenticated above; temporarily lift the
        # cycle debt only around those exact owner-derived rows.  Any exception
        # restores the debt and poisons the owner because earlier row closes
        # are irreversible.
        self._active_full_mdp_reward_cycle_identity = None
        self._full_mdp_reward_close_debt = False
        try:
            closed_rows = self._close_paid_many_for_full_mdp_reward()
        except BaseException:
            self._active_full_mdp_reward_cycle_identity = cycle_identity
            self._full_mdp_reward_close_debt = True
            self._poison_full_mdp_reward_protocol(
                "R06 Reward irreversible row close failed"
            )
            raise
        receipt = object.__new__(LandingOutcomeFullMdpRewardCloseReceipt)
        payload = _FullMdpRewardClosePayload(
            owner_identity=self._owner_identity,
            cycle_identity=cycle_identity,
            control_step=control_step,
            pre_reward_publication=pre_reward_publication,
            runtime_owner=runtime_owner,
            ordered_payment_verdicts=ordered_payment_verdicts,
            closed_rows=closed_rows.detach().clone(),
        )
        _FULL_MDP_REWARD_CLOSE_REGISTRY[receipt] = payload
        if cycle_token is not None:
            _FULL_MDP_REWARD_CYCLE_REGISTRY.pop(cycle_token, None)
        self._active_full_mdp_reward_cycle_token = None
        self._active_full_mdp_reward_prepublication = None
        self._active_full_mdp_reward_epoch = None
        self._active_full_mdp_reward_verdicts = {}
        self._active_full_mdp_reward_views = set()
        self._active_full_mdp_reward_runtime_owner = None
        self._full_mdp_reward_close_debt = False
        self._latest_full_mdp_reward_close = receipt
        return receipt
    ...
    def require_owned_full_mdp_reward_close(
        ...
        *,
        ...
        payload = _owned_full_mdp_reward_close(receipt)
        if (
            payload.owner_identity is not self._owner_identity
            or payload.control_step != control_step
            or payload.runtime_owner is not runtime_owner
            or receipt is not self._latest_full_mdp_reward_close
        ):
            self._poisoned = True
            raise LandingOutcomeDeviceError(
                "R06 full-MDP Reward close receipt is stale or foreign"
            )
        return receipt
    ...
    def close_paid(
        ...
        *,
        ...
        """Release only a PAID mailbox whose physical owner was retired."""

        self._require_operable()
        n = (self.num_envs,)
        mask = self._tensor(mask, name="mask", shape=n, dtype=torch.bool)
        slot = self._tensor(
            mailbox_slot, name="mailbox_slot", shape=n, dtype=torch.int64
        )
        token = self._tensor(
            full_key_sha256,
            name="full_key_sha256",
        ...
        generation = self._tensor(
            ball_generation, name="ball_generation", shape=n, dtype=torch.int64
        )
        in_range = (slot >= 0) & (slot < self.mailbox_capacity)
        safe = slot.clamp(0, self.mailbox_capacity - 1)
        state = torch.gather(
            self._mailbox_state.to(torch.int64), 1, safe.unsqueeze(1)
        ).squeeze(1)
        retired = torch.gather(
            self._mailbox_physical_retired, 1, safe.unsqueeze(1)
        ).squeeze(1)
        owner_generation = torch.gather(
            self._mailbox_ball_generation, 1, safe.unsqueeze(1)
        ).squeeze(1)
        owner_token = torch.gather(
            self._mailbox_full_key_sha256,
            1,
            safe.reshape(self.num_envs, 1, 1).expand(-1, 1, TOKEN_BYTES),
        ).squeeze(1)
        valid = (
            in_range
            & (state == MAILBOX_PAID)
            ...
            & torch.eq(owner_token, token).all(dim=-1)
            & ~self._device_sticky_poison
        )
        accepted = mask & valid
        rejected = mask & ~valid
        faults = torch.where(
            rejected,
            torch.full(n, FAULT_INVALID_CLOSE, dtype=torch.int64, device=self.device),
            torch.zeros(n, dtype=torch.int64, device=self.device),
        )
        faults = torch.where(
            mask & self._device_sticky_poison,
            torch.bitwise_or(
                faults,
                torch.full_like(faults, FAULT_SAFETY_CLEANUP),
            ),
            faults,
        )
        self._lifecycle_fault_bits.bitwise_or_(faults)
        target = accepted.unsqueeze(1) & (
            self._mailbox_slot_ids == safe.unsqueeze(1)
        )

        # Snapshot accepted rows before releasing their reusable mailbox slot.
        # There is exactly one selected mailbox per environment, so the gather
        # below cannot mix shots.  Rejected rows leave previous-paid bitwise
        # unchanged.
        gather_token = safe.reshape(self.num_envs, 1, 1).expand(
            -1, 1, TOKEN_BYTES
        )
        candidate_reset = torch.gather(
            self._mailbox_key_ints["reset_generation"],
            1,
            safe.unsqueeze(1),
        ).squeeze(1)
        candidate_swing = torch.gather(
            self._mailbox_key_ints["swing_generation"],
            1,
            safe.unsqueeze(1),
        ).squeeze(1)
        candidate_shot = torch.gather(
            self._mailbox_key_ints["shot_index"],
            1,
            safe.unsqueeze(1),
        ).squeeze(1)
        previous_reset = self._previous_paid_key_ints["reset_generation"]
        previous_swing = self._previous_paid_key_ints["swing_generation"]
        previous_shot = self._previous_paid_key_ints["shot_index"]
        newer_chronology = (
            (candidate_reset > previous_reset)
            | (
                (candidate_reset == previous_reset)
                & (candidate_swing > previous_swing)
            )
            | (
                (candidate_reset == previous_reset)
                & (candidate_swing == previous_swing)
                & (candidate_shot > previous_shot)
            )
        )
        save_previous = accepted & (
            ~self._previous_paid_valid | newer_chronology
        )
        for name in _INT_KEY_FIELDS:
            source = torch.gather(
                self._mailbox_key_ints[name], 1, safe.unsqueeze(1)
            ).squeeze(1)
            _masked_copy_(
                self._previous_paid_key_ints[name], source, save_previous
            )
        for name in _DIGEST_KEY_FIELDS:
            source = torch.gather(
                self._mailbox_key_digests[name], 1, gather_token
            ).squeeze(1)
            _masked_copy_(
                self._previous_paid_key_digests[name], source, save_previous
            )
        previous_sources = (
            (
                self._previous_paid_full_key_sha256,
                torch.gather(
                    self._mailbox_full_key_sha256, 1, gather_token
                ).squeeze(1),
            ),
            (
                self._previous_paid_ball_generation,
            ...
            (
                self._previous_paid_observation_ordinal,
                torch.gather(
                    self._mailbox_observation_ordinal, 1, safe.unsqueeze(1)
                ).squeeze(1),
            ),
            (
                self._previous_paid_settlement_control_step,
            ...
            (
                self._previous_paid_selected_contact,
                torch.gather(
                    self._mailbox_contact_valid, 1, safe.unsqueeze(1)
                ).squeeze(1),
            ),
            (
                self._previous_paid_first_crossing_valid,
                torch.gather(
                    self._mailbox_crossing_valid, 1, safe.unsqueeze(1)
                ).squeeze(1),
            ),
            (
                self._previous_paid_on_opponent_table,
                torch.gather(
                    self._mailbox_on_opponent_table, 1, safe.unsqueeze(1)
                ).squeeze(1),
            ),
            (
                self._previous_paid_target_error_m,
                torch.gather(
                    self._mailbox_placement_error_m, 1, safe.unsqueeze(1)
                ).squeeze(1),
            ),
            (
                self._previous_paid_target_xy_m,
                torch.gather(
                    self._mailbox_target_xy_m,
                    1,
                    safe.reshape(self.num_envs, 1, 1).expand(-1, 1, 2),
                ).squeeze(1),
            ),
        )
        for destination, source in previous_sources:
            _masked_copy_(destination, source, save_previous)
        _masked_copy_(
            self._previous_paid_action_epoch,
            torch.gather(
                self._mailbox_action_epoch, 1, safe.unsqueeze(1)
            ).squeeze(1),
            save_previous,
        )
        _masked_fill_(self._previous_paid_valid, save_previous, 1)

        _masked_fill_(self._mailbox_state, target, MAILBOX_EMPTY)
        _masked_fill_(self._mailbox_reserved, target, 0)
        _masked_fill_(self._mailbox_physical_retired, target, 0)
        # The durable previous-paid record and replay high-water now own the
        # closed shot.  Release history ownership so the finite mailbox slot is
        # actually reusable; stale installs remain rejected by replay order.
        _masked_fill_(self._mailbox_history_valid, target, 0)
        _masked_fill_(self._mailbox_action_epoch, target, 0)
        self._closed_total.add_(accepted.to(torch.int64).sum())
        self._record_fault_events(faults)
        self._increment_mutation()
        return self._mutation_result(accepted, rejected, faults)
