...
def action_rate_l2_clamped(env: ManagerBasedRLEnv, value_clamp: float = 9.0) -> torch.Tensor:
    """一阶动作平滑罚的值封顶版。**2026-08-08 起没有任何现役配方启用它。**

    人话:与 isaaclab 内置 ``action_rate_l2`` 同式 ‖a_t−a_{t−1}‖²,但每 env 封顶
    ``value_clamp``。clamp 内梯度原样、clamp 外为零。

    **退役理由(Franco 2026-08-08 裁定,详见 exp §5.6.25)。**

    先排除一个好听但错的说法:**这不是"封顶给 σ=0.02 标定、我们把 σ 提到 1.0 才焊死它"。**
    本函数的原 docstring 自己写着"fresh 随机策略一步能把 31 维动作甩出 ‖Δa‖²≈60+"
    (与 ``2×31×1.0² = 62`` 一致),也就是说 9.0 这个档位**就是知道 σ≈1.0 才加的**,
    是有意的设计选择,它想解决的问题(早期净流为负 → 摔死最优)也确实存在。

    它退役是因为**代价全付了、东西没买到**:

    1. **代价**:31 维、相邻两步独立采样 ⇒ ``E‖Δa‖² = 2×31×σ² = 62`` ≫ 9,于是
       s15r1 的 C0/C1 两格 × 5 个 update 上 raw **逐位**等于 9.0
       (``raw_sum = 98304×9 = 884736``),导数处处为零,只剩每步 −0.036 的死税。
       build_1 训到 21896 iter 收敛时 ‖Δa‖² 仍是 10.8~12.05,**从未低于 9.0**,
       所以这是整条谱系上的永久焊死,"暂时饱和、以后会解冻"在这里不发生。
    2. **没买到**:封顶后仍然摔死最优。C0 u4(post-24254020)正 +0.0187/步、
       负 −0.0776/步 ⇒ 净 −0.0589/步;55 步 episode、γ=0.99 折现 V_继续 = −2.50,
       V_摔死 = ``death_penalty −10 × dt`` = −0.20。仍差 12.5 倍,方向没变。
    3. **判据本身站不住**:唯一已知能打到球的 build_1 在同等策略水平(iter 4)是
       净 −0.191~−0.334/步、V_继续 = −19~−33,而它**根本没有 death penalty**
       (V_摔死 = 0)—— 它比我们深得多地待在同一个盆地里,照样学会了。
    4. **它爬出去用的引擎正是这一项**:build_1 的 action_rate 占早期罚金 32~60%,
       每步从 −0.126 衰减到 −0.022(5.2~5.8×),这个衰减就是它 |负|/正 穿过 1.0 的原因。
       封顶把该梯度置零 ⇒ 买到的是"更浅的盆地",卖掉的是"爬出去的唯一梯度"。

    现役一阶平滑改用上游那条无封顶的 ``action_rate_l2``(= isaaclab ``mdp.action_rate_l2``,
    四家模仿系逐字同形、同权重 −0.1)。本函数保留为可复现历史臂的死开关:RewTerm
    ``weight=0`` = IsaacLab 直接跳过 = 字节等价。要复活它必须**同时**给出一个高于当时
    工作区的 ``value_clamp``,否则复活的是同一个焊死的常数项。
    """
