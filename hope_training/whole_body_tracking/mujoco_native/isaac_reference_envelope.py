...
def _class_body_names(node: ast.ClassDef) -> tuple:
    ...
    for statement in node.body:
        if isinstance(statement, ast.Assign):
            ...
            continue
        if not isinstance(getattr(statement, "value", None), ast.Call):
            continue
...
def live_declared_terms(
    class_name: str = ACTION_BALL_TERMINATIONS_CLASS, config_path: Any = None
) -> tuple:
...
def live_class_chain(
    ...
    """The in-file inheritance chain, most-derived first.

    A base declared in another module ends the chain: this reader never guesses
    at a file it was not pointed at, so a term that moves into such a base reads
    as absent and fails closed.
    """
    ...
    while True:
        ...
        if len(parents) != 1:
            raise IsaacReferenceEnvelopeError(
                f"live Isaac class {current} has {len(parents)} in-file bases; "
                "this value reader only follows single inheritance"
            )
        current = parents[0]
...
def live_term_owner_class(
    ...
    source = _config(config_path)
    classes = _class_defs(source)
    for name in live_class_chain(class_name, config_path):
        if term_name in _class_body_names(classes[name]):
            return name
    raise IsaacReferenceEnvelopeError(
...
def live_declared_term_blockers(config_path: Any = None) -> tuple:
    ...
    for class_name, expected in DECLARED_TERMS.items():
        try:
            live = live_declared_terms(class_name, config_path)
