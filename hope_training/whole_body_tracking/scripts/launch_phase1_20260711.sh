...
case "$role" in
  smoke|pod1|pod2) ;;
  *)
    echo "usage: $0 {smoke|pod1|pod2}" >&2
    exit 2
    ;;
esac
...
git rev-parse HEAD >"$artifact_root/runs/code_commit.txt"
...
launch_fresh() {
  local arm=$1 gpu=$2 seed=$3 iterations=$4 marker=$5
  verify_fresh_inputs
...
launch_causal() {
  local arm=$1 gpu=$2 checkpoint=$3 fh=$4 bh=$5 bank=$6 phases=$7 pairing=$8 guidance=$9
  shift 9
  ...
  launch_arm "$arm" "$gpu" "Learning iteration" \
    seed=1 num_envs=4096 max_iterations=4000 run_name="$arm" \
