...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    mode_211 = str(getattr(env_cfg, "obs_mode", "") or "")
    if mode_211 == C211_ACTOR_CONTRACT:
        validate_action_ball_c211_cfg_trainability(
    ...
    if mode_211 in (A211_ACTOR_CONTRACT, C211_ACTOR_CONTRACT):
        if bool(agent_cfg.resume):
            raise RuntimeError(
                "RSL A211/C211 entry is fresh-only; checkpoint/normalizer reuse is forbidden"
            )
    ...
    if mode_211 == C211_ACTOR_CONTRACT:
        validate_action_ball_c211_runtime(env.unwrapped)
    else:
        validate_action_ball_211_runtime(env.unwrapped)
    ...
    if mode_211 == C211_ACTOR_CONTRACT:
        validate_action_ball_c211_runner(runner)
    else:
        validate_action_ball_211_runner(runner)
