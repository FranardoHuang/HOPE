...
- **Teacher-centered target sampling** — `racket.target_mode: reference_perturbed`. The target center is
  each imitated clip's own strike-frame racket FK state (position, velocity, face normal), avoiding the
  old failure mode where the policy imitated one teacher strike while chasing a different sampled target.
  The long-run distribution widens with the success-gated `ref_perturb_*` curriculum.
- **Per-clip strike** — `racket.strike_phase_per_clip: [0.47, 0.33]` (the racket-tip contact phase
...
2. `racket.strike_phase_per_clip: [0.47, 0.33]` — the racket-tip contact phase is per clip (v2 forehand
   frame 65/139, backhand frame 43/132); the strike window is computed per clip.
3. `racket.target_mode: reference_perturbed` — target position/velocity/normal centers are derived from
   the selected teacher clip's strike-frame racket FK state, then widened by the success-gated
   `ref_perturb_*` curriculum for long-run distribution coverage.
