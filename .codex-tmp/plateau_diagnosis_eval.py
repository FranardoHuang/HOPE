...
OUT_PATH = Path('/home/dongc1/workspace/HOPE/.codex-tmp/plateau_diagnosis_eval.json')
ERR_PATH = Path('/home/dongc1/workspace/HOPE/.codex-tmp/plateau_diagnosis_eval.err')
TASK_ID = 'HOPE-PingPong-AgibotA3-v0'
CHECKPOINT = str(RUN_DIR / 'model_4400.pt')
NUM_ENVS = 256
MIN_EXACT_SAMPLES = 1024
MAX_STEPS = 2600
    ...
    for getter_name in ('get_dof_max_velocities', 'get_dof_velocity_limits'):
        ...
        if getter is not None:
            ...
            if not isinstance(joint_ids, slice):
                vals = vals[joint_ids]
            return vals
    vals = getattr(robot.data, 'joint_vel_limits', None)
    if vals is not None:
        if vals.dim() > 1:
            vals = vals[0]
        if not isinstance(joint_ids, slice):
    ...
    for getter_name in ('get_dof_max_forces', 'get_dof_effort_limits'):
        ...
        if getter is not None:
            ...
            if not isinstance(joint_ids, slice):
                vals = vals[joint_ids]
            return vals
    vals = getattr(robot.data, 'joint_effort_limits', None)
    if vals is not None:
        if vals.dim() > 1:
            vals = vals[0]
        if not isinstance(joint_ids, slice):
...
def evaluate_variant(name: str, zero_perturb: bool):
    ...
    exact_samples = {
        ...
        'perturb_pos_z': [],
        'perturb_vel_norm': [],
        'target_normal_perturb_deg': [],
        'moving_reward_value': [],
        'static_reward_value': [],
        'right_arm_raw_abs': [],
        'right_arm_raw_gt1': [],
        'right_arm_target_actual_abs': [],
        'right_arm_target_outside_limits': [],
        'right_arm_actual_near_limits': [],
        'right_arm_vel_sat': [],
        'right_arm_tau_sat': [],
    }
    ...
    try:
        for step in range(MAX_STEPS):
            with torch.inference_mode():
                actions = policy(obs)
                obs, _, dones, extras = env.step(actions.to(uenv.device))
            ...
            if dones.any():
                ep_lens = uenv.episode_length_buf[dones].detach().cpu().numpy().tolist()
                completed_ep_lengths.extend(float(x) for x in ep_lens)
            ...
            if exact.any():
                ...
                exact_samples['time_to_strike'].extend(cmd.time_to_strike[idx].detach().cpu().numpy().tolist())
                exact_samples['static_pos_err'].extend(static_err[idx].detach().cpu().numpy().tolist())
                exact_samples['moving_pos_err'].extend(moving_err[idx].detach().cpu().numpy().tolist())
                exact_samples['vel_err'].extend(vel_err[idx].detach().cpu().numpy().tolist())
                exact_samples['normal_err_deg'].extend(normal_err_deg[idx].detach().cpu().numpy().tolist())
                exact_samples['base_xy_err'].extend(base_xy_err[idx].detach().cpu().numpy().tolist())
                exact_samples['abs_x'].extend(static_err_vec[idx, 0].abs().detach().cpu().numpy().tolist())
                exact_samples['abs_y'].extend(static_err_vec[idx, 1].abs().detach().cpu().numpy().tolist())
                exact_samples['abs_z'].extend(static_err_vec[idx, 2].abs().detach().cpu().numpy().tolist())
                exact_samples['signed_x'].extend(static_err_vec[idx, 0].detach().cpu().numpy().tolist())
                exact_samples['signed_y'].extend(static_err_vec[idx, 1].detach().cpu().numpy().tolist())
                exact_samples['signed_z'].extend(static_err_vec[idx, 2].detach().cpu().numpy().tolist())
                exact_samples['perturb_pos_norm'].extend(torch.norm(perturb_pos[idx], dim=-1).detach().cpu().numpy().tolist())
                exact_samples['perturb_pos_x'].extend(perturb_pos[idx, 0].detach().cpu().numpy().tolist())
                exact_samples['perturb_pos_y'].extend(perturb_pos[idx, 1].detach().cpu().numpy().tolist())
                exact_samples['perturb_pos_z'].extend(perturb_pos[idx, 2].detach().cpu().numpy().tolist())
                exact_samples['perturb_vel_norm'].extend(torch.norm(perturb_vel[idx], dim=-1).detach().cpu().numpy().tolist())
                exact_samples['target_normal_perturb_deg'].extend(target_n_deg[idx].detach().cpu().numpy().tolist())
                exact_samples['moving_reward_value'].extend(moving_rew[idx].detach().cpu().numpy().tolist())
                exact_samples['static_reward_value'].extend(static_rew[idx].detach().cpu().numpy().tolist())
                ...
                if right_arm_idx:
                    ...
                    exact_samples['right_arm_raw_abs'].extend(raw[:, ridx].abs().detach().cpu().numpy().tolist())
                    exact_samples['right_arm_raw_gt1'].extend((raw[:, ridx].abs() > 1.0).float().detach().cpu().numpy().tolist())
                    exact_samples['right_arm_target_actual_abs'].extend(jtgt_gap[:, ridx].detach().cpu().numpy().tolist())
                    if joint_lower is not None and joint_upper is not None:
                        ...
                        exact_samples['right_arm_target_outside_limits'].extend(outside.detach().cpu().numpy().tolist())
                        exact_samples['right_arm_actual_near_limits'].extend(near.detach().cpu().numpy().tolist())
                    if vel_limits is not None:
                        vlim = vel_limits[ridx].clamp(min=1e-6)
                        vsat = ((jvel[:, ridx].abs() / vlim) > 0.9).float()
                        exact_samples['right_arm_vel_sat'].extend(vsat.detach().cpu().numpy().tolist())
                    ...
                    if tau is not None:
                        ...
                        if effort_limits is not None:
                            ...
                            exact_samples['right_arm_tau_sat'].extend(tsat.detach().cpu().numpy().tolist())

            if len(exact_samples['static_pos_err']) >= MIN_EXACT_SAMPLES:
                break
    finally:
        env.close()

    static_arr = np.asarray(exact_samples['static_pos_err'], dtype=np.float64)
    vel_arr = np.asarray(exact_samples['vel_err'], dtype=np.float64)
    normal_arr = np.asarray(exact_samples['normal_err_deg'], dtype=np.float64)
    base_arr = np.asarray(exact_samples['base_xy_err'], dtype=np.float64)

    right_arm_summary = None
    if right_arm_idx:
        def per_joint_mean(key):
            arr = np.asarray(exact_samples[key], dtype=np.float64)
            if arr.size == 0:
                return None
    ...
    moving_racket_step_mean = moving_reward_global_sum / total_env_steps
    static_racket_step_mean = static_reward_global_sum / total_env_steps

    return {
        'variant': name,
        'num_envs': NUM_ENVS,
        'steps_run': step + 1,
        'exact_samples': int(len(exact_samples['static_pos_err'])),
        'mean_completed_episode_length_steps': mean_ep_len,
        'step_dt': step_dt,
        'success_thresholds': {
            'position_m': success_pos_thresh,
            'velocity_mps': 0.5,
            'normal_deg': 15.0,
            'base': None,
        },
        ...
        'errors': {
            'time_to_strike_exact_s': qstats(exact_samples['time_to_strike']),
            'racket_pos_error_static_exact': qstats(exact_samples['static_pos_err']),
            'racket_pos_error_moving_exact': qstats(exact_samples['moving_pos_err']),
            'racket_pos_error_abs_x_exact': qstats(exact_samples['abs_x']),
            'racket_pos_error_abs_y_exact': qstats(exact_samples['abs_y']),
            'racket_pos_error_abs_z_exact': qstats(exact_samples['abs_z']),
            'racket_pos_error_signed_x_exact': qstats(exact_samples['signed_x']),
            'racket_pos_error_signed_y_exact': qstats(exact_samples['signed_y']),
            'racket_pos_error_signed_z_exact': qstats(exact_samples['signed_z']),
            'racket_vel_error_exact': qstats(exact_samples['vel_err']),
            'racket_normal_error_deg_exact': qstats(exact_samples['normal_err_deg']),
            'base_xy_error_exact': qstats(exact_samples['base_xy_err']),
        },
        'reward_alignment': {
            'racket_position_weight': racket_pos_weight,
            'racket_position_std': racket_pos_std,
            'moving_reward_step_mean': moving_racket_step_mean,
            'static_reward_step_mean': static_racket_step_mean,
            'moving_reward_episode_contrib_est': None if mean_ep_len is None else moving_racket_step_mean * step_dt * mean_ep_len * racket_pos_weight,
            'static_reward_episode_contrib_est': None if mean_ep_len is None else static_racket_step_mean * step_dt * mean_ep_len * racket_pos_weight,
            ...
            'exact_static_error_stats': qstats(exact_samples['static_pos_err']),
            'exact_moving_error_stats': qstats(exact_samples['moving_pos_err']),
            'exact_moving_reward_mean': float(np.mean(exact_samples['moving_reward_value'])) if exact_samples['moving_reward_value'] else None,
            'exact_static_reward_mean': float(np.mean(exact_samples['static_reward_value'])) if exact_samples['static_reward_value'] else None,
        },
        'perturbation': {
            'pos_norm_stats': qstats(exact_samples['perturb_pos_norm']),
            'vel_norm_stats': qstats(exact_samples['perturb_vel_norm']),
            'normal_deg_stats': qstats(exact_samples['target_normal_perturb_deg']),
            'corr_pos_perturb_norm_vs_static_error': corr(exact_samples['perturb_pos_norm'], exact_samples['static_pos_err']),
            'corr_abs_y_perturb_vs_abs_y_error': corr(np.abs(exact_samples['perturb_pos_y']), exact_samples['abs_y']),
            'corr_pos_perturb_norm_vs_vel_error': corr(exact_samples['perturb_pos_norm'], exact_samples['vel_err']),
        },
        'control': {
            'right_arm': right_arm_summary,
        },
    }
...
try:
    results = {
        'current': evaluate_variant('current', zero_perturb=False),
        'zero_perturb': evaluate_variant('zero_perturb', zero_perturb=True),
    }
