"""BallKalmanEstimator gates: venue-noise tracking, bounce continuity, gaps.
...
def test_kf_bounce_continuity_no_nan_psd_available_next_frame():
    ...
    # Seed chosen so the three-sample z-pattern actually fires: at ~2.6 m/s
    # descent the ball drops 8.7 mm/frame, so with 5.5 mm marginal noise the
    # legacy detection (shared by BOTH estimator paths) misses most bounces.
    # This test probes KF continuity GIVEN a detection, not detection recall.
...
def test_kf_chi2_gate_rejects_and_counts_outlier():
    cfg = PlannerConfig()
