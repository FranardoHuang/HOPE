...
There are **three machines** (see `agi/a3_deploy_example/HARDWARE_BRINGUP_CHECKLIST.md`, the
...
This is the **only** SSH step. Follow `agi/a3_deploy_example/HARDWARE_BRINGUP_CHECKLIST.md` (steps
...
- **No joint-state/IMU topic in `ros2 topic list`** → robot state is on iceoryx (default Rockchip/MDU),
  ...
  Normal per `agi/a3_deploy_example/HARDWARE_BRINGUP_CHECKLIST.md`, not a `wbc_runner` bug.
