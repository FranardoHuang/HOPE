...
- **Success:** prompt shows you're inside (e.g. `dongc1@hope:~$`).
...
find ~/workspace/HOPE/hope_training/whole_body_tracking/logs/rsl_rl/agibot_a3_hope/ \
...
- **The one you want** is the **unified `model_15200`** export, ~**1,243,884 bytes (≈1.24 MB)**:
  ```
  ~/workspace/HOPE/hope_training/whole_body_tracking/logs/rsl_rl/agibot_a3_hope/2026-06-28_23-01-24_phase050_perclippos_scratch/exported/policy.onnx
  ```
...
- **Verify the size is right** (this is the #1 gotcha):
  ...
  - **≈ 1141378 (1.14 MB) → BAD** = the old forehand-only export; the backhand will be
    dead. Re-export with both clips (see `reimplement.md` Step 15) before continuing.
...
- **`Could not import 'rosidl_typesupport_c' for package 'hope_msgs'`** or
  **`libpython3.X.so: cannot open shared object file`** → `hope_msgs` was built with a
  different python. Fix: `cd ~/workspace/HOPE/hope_ws && rm -rf build install && colcon build`,
...
- **Want it to run longer / let backhands finish without the training cutoff?** add
  `--ee-term-z 100` (relaxes a training-only tracking guard; see `reimplement.md` 16.1b).
