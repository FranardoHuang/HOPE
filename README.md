...
3. **A WBC training scaffold** under [hope_training/whole_body_tracking/](hope_training/whole_body_tracking/): a new A3 ping-pong task (`HOPE-PingPong-AgibotA3-v0`), a Hydra config tree, the `setup_train_env.sh` launcher, richer Weights & Biases telemetry, and a reward-shaping fix (the `strike_success=0` fix).
