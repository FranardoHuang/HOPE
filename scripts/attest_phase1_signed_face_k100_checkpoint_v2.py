...
This v2 attestation-only consumer closes the v1 ignored-asset packaging gap. It
never launches a trainer, exporter, judge, simulator, deployment process,
signal, SSH command, or robot command. C3 verifies the exact training-time A3
asset inventory and hydrates it once into a fresh eval checkout; D3 verifies
that same destination. Both require loadable libGLU before any claim is written.
...
MANIFEST_ID = "phase1-signed-face-k100-checkpoint-attestor-20260714-v2"
MANIFEST_RELATIVE_PATH = Path(
    "configs/phase1_signed_face_k100_checkpoint_attestor_v2_20260714.json"
)
OUTPUT_ROOT = Path(
    "/workspace/codexschema/phase1_signed_face_rescue_20260713/"
    "executions/signed_face_k100_v2"
)
CORRECTION_ID = (
    "phase1-signed-face-exam-k100-runtime-receipt-correction-20260714-v1"
)
REQUEST_STATUS = "exact_checkpoint_attestation_v2_requested_judge_not_started"
ACTIVATION_KIND = "phase1_signed_face_exam_k100_paper_activation"
EVIDENCE_KIND = "phase1_signed_face_k100_checkpoint_execution_evidence_v2"
CLAIM_KIND = "phase1_signed_face_k100_checkpoint_execution_claim_v2"
ASSET_INVENTORY_ALGORITHM = "canonical-relative-path-bytes-sha256-v1"
ASSET_RELATIVE_PATH = Path(
    "hope_training/whole_body_tracking/source/whole_body_tracking/"
    "whole_body_tracking/assets/agibot_a3"
)
TRAINING_ASSET_ROOT = Path(
    "/workspace/codexschema/nohope_signed_face_c3d3_l1_4467d79"
) / ASSET_RELATIVE_PATH
REQUIRED_URDF_RELATIVE = Path("urdf/model.urdf")
REQUIRED_URDF_BYTES = 43240
REQUIRED_URDF_SHA256 = (
    "79655f05d204c24f028778425aa971410773d1f8bbbd214de6fdb8f8ae75d1cc"
)
LIBGLU_SONAME = "libGLU.so.1"
HYDRATION_MODES = ("hydrate_absent", "verify_existing")
...
def load_manifest(path: Path, *, repo_root: Path) -> dict[str, Any]:
    ...
    require_exact(
        semantics["isaac_asset_hydration"],
        {
            "source_root": str(TRAINING_ASSET_ROOT),
            "destination_repo_relative": str(ASSET_RELATIVE_PATH),
            "inventory_algorithm": ASSET_INVENTORY_ALGORITHM,
            "required_urdf": {
                "relative_path": str(REQUIRED_URDF_RELATIVE),
                "bytes": REQUIRED_URDF_BYTES,
                "sha256": REQUIRED_URDF_SHA256,
            },
            "source_is_training_checkout_ignored_bundle": True,
            "destination_created_exclusive_before_publish": True,
            "libglu_soname_preflight": LIBGLU_SONAME,
        },
        "Isaac ignored asset hydration",
    )
...
def _required_urdf_spec() -> dict[str, Any]:
    return {
        "relative_path": str(REQUIRED_URDF_RELATIVE),
        "bytes": REQUIRED_URDF_BYTES,
        "sha256": REQUIRED_URDF_SHA256,
...
def validate_asset_request_spec(
    ...
    spec = exact_keys(
        ...
        "Isaac ignored asset bundle",
    )
    source_root = require_absolute(spec["source_root"], "asset source_root")
    destination_root = require_absolute(
        spec["destination_root"], "asset destination_root"
    )
    expected_semantics = manifest["execution_semantics"]["isaac_asset_hydration"]
    if source_root != Path(expected_semantics["source_root"]):
        raise ContractError("asset source is not the training-time ignored bundle")
    expected_destination = (
        Path(request["source_checkout"]["path"])
        / expected_semantics["destination_repo_relative"]
    )
    if destination_root != expected_destination:
        raise ContractError("asset destination is not inside the exact eval checkout")
    if source_root == destination_root:
        raise ContractError("asset source and destination must be different roots")
    inventory = exact_keys(
        spec["inventory"],
        {"schema_version", "algorithm", "file_count", "total_bytes", "canonical_sha256"},
        "asset inventory binding",
    )
    require_exact(inventory["schema_version"], 1, "asset inventory schema")
    require_exact(
        inventory["algorithm"], ASSET_INVENTORY_ALGORITHM, "asset inventory algorithm"
    )
    require_plain_int(inventory["file_count"], "asset inventory file_count", minimum=1)
    require_plain_int(inventory["total_bytes"], "asset inventory total_bytes", minimum=1)
    require_sha(inventory["canonical_sha256"], "asset inventory canonical SHA")
    require_exact(spec["required_urdf"], _required_urdf_spec(), "required Isaac URDF")
    if spec["hydration_mode"] not in HYDRATION_MODES:
        raise ContractError("asset hydration_mode must be hydrate_absent or verify_existing")
    return spec
...
def load_request(path: Path, manifest: Mapping[str, Any], *, runtime: bool) -> dict[str, Any]:
    ...
    validate_asset_request_spec(request, manifest)
...
def _asset_inventory(root: Path) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    if root.is_symlink() or root.resolve() != root or not root.is_dir():
        raise ContractError(f"asset root must be one canonical non-symlink directory: {root}")
    ...
    for current, directories, names in os.walk(root, followlinks=False):
        ...
        for directory in directories:
            ...
            if candidate.is_symlink() or candidate.resolve() != candidate:
                raise ContractError(f"asset inventory forbids symlink directories: {candidate}")
        for name in names:
            candidate = current_path / name
            if candidate.is_symlink() or candidate.resolve() != candidate:
                raise ContractError(f"asset inventory forbids symlink files: {candidate}")
            before = require_regular(candidate, "asset inventory file")
            relative = candidate.relative_to(root).as_posix()
            require_repo_relative(relative, "asset inventory relative path")
            digest = sha256_file(candidate)
            after = require_regular(candidate, "asset inventory file")
            if _stat_identity(before) != _stat_identity(after):
                raise ContractError(f"asset file changed while hashing: {candidate}")
            files.append({"path": relative, "bytes": after.st_size, "sha256": digest})
    files.sort(key=lambda item: item["path"])
    if not files:
        raise ContractError("asset inventory must contain at least one regular file")
    document = {
        "schema_version": 1,
        "algorithm": ASSET_INVENTORY_ALGORITHM,
        "files": files,
    }
    summary = {
        "schema_version": 1,
...
def asset_inventory_summary(root: Path) -> dict[str, Any]:
    summary, _ = _asset_inventory(root)
    return summary


def validate_libglu_presence() -> dict[str, Any]:
    try:
        ctypes.CDLL(LIBGLU_SONAME)
    except OSError as exc:
        raise ContractError(f"required {LIBGLU_SONAME} is not loadable: {exc}") from exc
    return {"soname": LIBGLU_SONAME, "loadable": True}
...
def validate_asset_bundle(
    ...
    *,
    ...
    spec = validate_asset_request_spec(request, manifest)
    source_root = Path(spec["source_root"])
    source_inventory = asset_inventory_summary(source_root)
    require_exact(source_inventory, spec["inventory"], "training-time asset inventory")
    urdf = source_root / REQUIRED_URDF_RELATIVE
    validate_file_binding(
        urdf,
        size=REQUIRED_URDF_BYTES,
        digest=REQUIRED_URDF_SHA256,
        label="training-time required URDF",
    )
    destination_root = Path(spec["destination_root"])
    destination_verified = False
    if require_destination:
        destination_inventory = asset_inventory_summary(destination_root)
        require_exact(destination_inventory, source_inventory, "hydrated asset inventory")
        validate_file_binding(
            destination_root / REQUIRED_URDF_RELATIVE,
            size=REQUIRED_URDF_BYTES,
            digest=REQUIRED_URDF_SHA256,
            label="hydrated required URDF",
        )
        destination_verified = True
    elif _lexists(destination_root):
        raise ContractError("hydrate_absent destination already exists before attestation")
    return {
        "source_root": str(source_root),
        "destination_root": str(destination_root),
        "inventory": source_inventory,
        "required_urdf": _required_urdf_spec(),
        "hydration_mode": spec["hydration_mode"],
        "destination_verified_exact": destination_verified,
    }


def hydrate_asset_bundle(request: Mapping[str, Any], manifest: Mapping[str, Any]) -> None:
    spec = validate_asset_request_spec(request, manifest)
    if spec["hydration_mode"] != "hydrate_absent":
        raise ContractError("only hydrate_absent may publish the ignored asset bundle")
    source_root = Path(spec["source_root"])
    destination_root = Path(spec["destination_root"])
    source_inventory, files = _asset_inventory(source_root)
    require_exact(source_inventory, spec["inventory"], "pre-hydration asset inventory")
    if _lexists(destination_root):
        raise ContractError("asset destination must be absent before exclusive hydration")
    parent = destination_root.parent
    if not parent.is_dir() or parent.is_symlink() or parent.resolve() != parent:
        raise ContractError("asset destination parent must be an existing canonical directory")
    stage = parent / f".{destination_root.name}.hydrate-{request['request_id']}"
    if _lexists(stage):
        raise ContractError(f"no-clobber: asset hydration staging root exists: {stage}")
    stage.mkdir(mode=0o755)
    for entry in files:
        source = source_root / entry["path"]
        target = stage / entry["path"]
        target.parent.mkdir(parents=True, exist_ok=True)
        before = require_regular(source, "asset hydration source")
        ...
        try:
            ...
            try:
                ...
                pass
            raise
        after = require_regular(source, "asset hydration source")
        if _stat_identity(before) != _stat_identity(after):
            raise ContractError(f"asset source changed while copying: {source}")
        validate_file_binding(
            target,
            size=entry["bytes"],
            digest=entry["sha256"],
            label="hydrated staging asset",
        )
    staged_inventory = asset_inventory_summary(stage)
    require_exact(staged_inventory, source_inventory, "staged asset inventory")
    require_exact(
...
def validate_runtime_request(
    ...
    manifest: Mapping[str, Any],
    *,
    pre_hydration: bool = False,
) -> dict[str, Any]:
    source = validate_source_checkout(request, manifest)
    asset_spec = request["isaac_asset_bundle"]
    if pre_hydration and asset_spec["hydration_mode"] != "hydrate_absent":
        raise ContractError("pre_hydration is only valid for hydrate_absent requests")
    libglu = validate_libglu_presence()
    asset_bundle = validate_asset_bundle(
        request, manifest, require_destination=not pre_hydration
    )
    checkpoint = request["checkpoint"]
    ...
    return {
        ...
        "mjcf": dict(mjcf),
        "paper": paper,
        "isaac_asset_bundle": asset_bundle,
        "libglu": libglu,
    }
...
def attest(
    ...
    if request["isaac_asset_bundle"]["hydration_mode"] == "hydrate_absent":
        validate_runtime_request(request, manifest, pre_hydration=True)
        hydrate_asset_bundle(request, manifest)
    ...
    content = {
        ...
        "isaac_asset_bundle": runtime["isaac_asset_bundle"],
        "libglu": runtime["libglu"],
        "paper": runtime["paper"],
    ...
    claim_content = {
        ...
        "isaac_asset_inventory_sha256": runtime["isaac_asset_bundle"]["inventory"]["canonical_sha256"],
        "isaac_asset_destination_root": runtime["isaac_asset_bundle"]["destination_root"],
        "libglu_soname": runtime["libglu"]["soname"],
...
def parser() -> argparse.ArgumentParser:
    ...
    value.add_argument(
        "--manifest",
        type=Path,
        default=root / "configs/phase1_signed_face_k100_checkpoint_attestor_v2_20260714.json",
    )
    value.add_argument("--repo-root", type=Path, default=root)
    value.add_argument("--request", type=Path)
    value.add_argument(
        "command", choices=("static-validate", "asset-plan", "plan", "attest")
    )
    return value
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "asset-plan":
        inventory = asset_inventory_summary(TRAINING_ASSET_ROOT)
        validate_file_binding(
            TRAINING_ASSET_ROOT / REQUIRED_URDF_RELATIVE,
            size=REQUIRED_URDF_BYTES,
            digest=REQUIRED_URDF_SHA256,
            label="training-time required URDF",
        )
        print(
            json.dumps(
                {
                    "status": "training_asset_inventory_valid_no_writes",
                    "source_root": str(TRAINING_ASSET_ROOT),
                    "inventory": inventory,
                    "required_urdf": _required_urdf_spec(),
                    "libglu": validate_libglu_presence(),
