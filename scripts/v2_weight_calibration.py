...
人话:v2 的名义数值(capture 850、质量核 60/45/35、action clamp@10)是拍脑袋假设推的,
...
输出:B(capture one-shot)、质量组 Σw 与 60:45:35 比例的分账、strike_success、
action clamp 值、罚项预算核对(早期分母=模仿收入)。
...
STRIKE_SUCCESS_RATIO = 30.0 / 140.0  # strike_success 相对质量组 Σw 的名义占比
...
def calibrate(measured: dict) -> dict:
    ...
    pse_mimic = I * rho_i
    # L2 击中 one-shot:B·p*/T_c = m1·PSE_mimic
    pse_hit = M1 * pse_mimic
    b_capture = pse_hit * t_c / p_star
    # L3 质量(窗口 dense):Σw·duty·rho_Q = m2·PSE_hit
    pse_quality = M2 * pse_hit
    duty = w_steps / t_c
    ...
    w_success = quality_sum * STRIKE_SUCCESS_RATIO
...
def main(argv: list) -> int:
    ...
    if not acc["ordering_ok"]:
        print("!! 定序 模仿<击中<质量 不成立 — 检查输入或设计常量")
        return 1
    return 0
