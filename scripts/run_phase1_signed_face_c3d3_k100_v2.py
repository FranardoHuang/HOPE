#!/usr/bin/env python3
"""Run one v2-attested C3/D3 pair on the immutable signed-face K100 paper.
...
requires the same hydrated ignored A3 asset inventory and loadable libGLU, then
runs C3 and D3 on the same 100-question paper. It never signals an
...
MANIFEST_RELATIVE = Path("configs/phase1_signed_face_c3d3_k100_execution_v2_20260714.json")
MANIFEST_ID = "phase1-signed-face-c3-d3-k100-paired-execution-20260714-v2"
REQUEST_STATUS = "exact_c3_d3_signed_k100_pair_execution_v2_requested_not_started"
CELL_ORDER = ("C3", "D3")
...
ROOT_CONFIRMATION = "ROOT_APPROVES_SIM_ONLY_C3_D3_SIGNED_K100_PAIRED_EXECUTION_V2"
...
def load_request(
    ...
    if read_attestor_requests:
        ...
        require_exact(
            loaded_attestor_requests["C3"]["isaac_asset_bundle"]["hydration_mode"],
            "hydrate_absent",
            "C3 asset hydration role",
        )
        require_exact(
            loaded_attestor_requests["D3"]["isaac_asset_bundle"]["hydration_mode"],
            "verify_existing",
            "D3 asset hydration role",
        )
        for key in ("source_root", "destination_root", "inventory", "required_urdf"):
            require_exact(
                loaded_attestor_requests["C3"]["isaac_asset_bundle"][key],
                loaded_attestor_requests["D3"]["isaac_asset_bundle"][key],
                f"paired ignored asset {key}",
            )
...
def _validate_attestation(
    ...
    claim_expected = {
        ...
        "isaac_asset_inventory_sha256": attestor_request["isaac_asset_bundle"]["inventory"]["canonical_sha256"],
        "isaac_asset_destination_root": attestor_request["isaac_asset_bundle"]["destination_root"],
        "libglu_soname": A.LIBGLU_SONAME,
...
def execute_pair(
    *, request_path: Path, request: dict[str, Any], manifest_path: Path,
    ...
    asset_shas = {
        attested[cell_id]["runtime"]["isaac_asset_bundle"]["inventory"]["canonical_sha256"]
        for cell_id in CELL_ORDER
    }
    asset_destinations = {
        attested[cell_id]["runtime"]["isaac_asset_bundle"]["destination_root"]
        for cell_id in CELL_ORDER
    }
    if (
        len(mjcf_shas) != 1
        or len(plant_shas) != 1
        or len(asset_shas) != 1
        or len(asset_destinations) != 1
        or any(attested[cell_id]["runtime"]["libglu"]["loadable"] is not True for cell_id in CELL_ORDER)
    ):
        raise ContractError(
            "C3/D3 attestations do not share one MJCF/plant/ignored-asset runtime contract"
        )
    ...
    claim_content = {
        ...
        "activation_file_sha256": manifest["paper"]["activation"]["file_sha256"],
        "activation_content_sha256": manifest["paper"]["activation"]["content_sha256"],
        "isaac_asset_inventory_sha256": next(iter(asset_shas)),
        "isaac_asset_destination_root": next(iter(asset_destinations)),
        "libglu_soname": A.LIBGLU_SONAME,
