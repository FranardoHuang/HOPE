#!/usr/bin/env python3
"""Validate and render the W/V lower-body stability ablation queue.
...
The training stage additionally consumes six digest-bound probe
...
DEFAULT_QUEUE = Path("configs/phase1_lower_body_stability_20260720.yaml")
QUEUE_CONFIG_RELATIVE = "configs/phase1_lower_body_stability_20260720.yaml"
QUEUE_RUNNER_RELATIVE = "scripts/run_phase1_lower_body_stability_queue.py"
LAUNCH_MANIFEST_RELATIVE = "configs/phase1_lower_body_stability_launch_manifest_20260720.json"
AUTHORITY_NOW_TITLE = "- **[11｜P1] 稳定机制 Wave A/B。**"
AUTHORITY_OWNER_EXECUTOR = "责任人 franco；执行者 Codex；执行分支"
AUTHORITY_WAVE_A_BRANCH = "Franco_codex/balance-ablation-round-20260720"
AUTHORITY_WAVE_B_BRANCH = "Franco_codex/lowerbody-wave-queue-20260720"
SAFE_ID = re.compile(r"^[A-Za-z0-9][A-Za-z0-9_.-]*$")
HYDRA_KEY = re.compile(r"^[A-Za-z_][A-Za-z0-9_.]*$")
SHA256 = re.compile(r"^[0-9a-f]{64}$")
COMMIT = re.compile(r"^[0-9a-f]{40}$")
EXPECTED_QUEUE_ID = "phase1_lower_body_stability_20260720"
EXPECTED_NAMESPACE = "/workspace/codexschema/phase1_lower_body_stability_20260720"
EXPECTED_SOURCE = "/workspace/codexschema/nohope_lowerbody_wave_20260720"
EXPECTED_REMOTE_SOURCE_COMMIT = "5db7366aaa1562d592093dc0d512ec212f14e39e"
...
FACTOR_KEYS = {
    ...
    "task.rewards.lower_body_pose_imitation_std",
    "task.rewards.lower_body_pose_imitation_support_pre_s",
    "task.rewards.lower_body_pose_imitation_support_post_s",
    "task.rewards.lower_body_stability_bundle_weight",
    "task.rewards.lower_body_stability_min_stance_width_m",
    "task.rewards.lower_body_stability_stance_scale_m",
    "task.rewards.lower_body_stability_leg_velocity_margin_radps",
    "task.rewards.lower_body_stability_leg_velocity_scale_radps",
    "task.rewards.lower_body_stability_support_pre_s",
    "task.rewards.lower_body_stability_support_post_s",
}
FORBIDDEN_PROCESSED_SLEW_KEYS = {
...
POSE_PROBE_COUNTERS = (
    ...
    "support_eligible_sample_count",
    "reward_enabled_eligible_sample_count",
    "gated_kernel_sum",
    "gated_joint_abs_error_mean_sum",
    "gated_reference_motion_l1_mean_sum",
)
BUNDLE_PROBE_COUNTERS = (
    "observed_sample_count",
    "support_eligible_sample_count",
    "reward_enabled_eligible_sample_count",
    "narrow_or_crossed_sample_count",
    "gated_bundle_sum",
    "gated_stance_tail_sum",
    "gated_leg_velocity_tail_sum",
    "gated_signed_stance_width_m_sum",
)
EXPECTED_LEG_JOINTS = [
...
EXPECTED_FOOT_BODIES = ["left_ankle_roll_Link", "right_ankle_roll_Link"]
...
def _authority_now_entry(text: str) -> str:
    ...
    if AUTHORITY_OWNER_EXECUTOR not in entry or any(
        branch not in entry
        for branch in (AUTHORITY_WAVE_A_BRANCH, AUTHORITY_WAVE_B_BRANCH)
    ):
        raise QueueError(
            "Wave A/B owner, executor, and both branches are not bound in one origin/main NOW entry"
        )
    return entry
...
def _validate_origin_main_launch_authority(
    ...
    return {
        ...
        "human_owner": "franco",
        "executor": "Codex",
        "wave_a_branch": AUTHORITY_WAVE_A_BRANCH,
        "wave_b_branch": AUTHORITY_WAVE_B_BRANCH,
    }
...
def _validate_mechanism(name: str, mechanism: dict[str, Any]) -> None:
    ...
    pose = _finite_number(
        ...
        f"mechanisms.{name}.lower_body_pose_imitation_weight",
    )
    bundle = _nonpositive(
        mechanism["lower_body_stability_bundle_weight"],
        f"mechanisms.{name}.lower_body_stability_bundle_weight",
    )
    expected_pose, expected_bundle = EXPECTED_MECHANISMS[name]
    if pose < 0.0 or pose != expected_pose or bundle != expected_bundle:
        raise QueueError(
            f"mechanisms.{name} must be pose={expected_pose}, bundle={expected_bundle}"
        )
    expected_scalars = {
        "lower_body_pose_imitation_std": 0.35,
        "lower_body_pose_imitation_support_pre_s": 0.30,
        "lower_body_pose_imitation_support_post_s": 0.40,
        "lower_body_stability_min_stance_width_m": 0.22,
        "lower_body_stability_stance_scale_m": 0.05,
        "lower_body_stability_leg_velocity_margin_radps": 1.0,
        "lower_body_stability_leg_velocity_scale_radps": 0.5,
        "lower_body_stability_support_pre_s": 0.30,
        "lower_body_stability_support_post_s": 0.40,
    }
    for key, expected in expected_scalars.items():
        if _finite_number(mechanism[key], f"mechanisms.{name}.{key}") != expected:
            raise QueueError(f"mechanisms.{name}.{key} must be {expected}")
    if pose > 0.0 and bundle < 0.0:
        raise QueueError(f"mechanisms.{name} enables mutually exclusive B1 and B2")
...
def _validate_queue(queue: dict[str, Any]) -> dict[str, Any]:
    ...
    if base_map.get("task.rewards.action_rate_weight") != "-0.1":
        raise QueueError("base recipe must explicitly hold action-rate weight at -0.1")
    ...
    overlap = set(base_map) & (forbidden_base | FORBIDDEN_PROCESSED_SLEW_KEYS)
    if overlap:
    ...
    if set(mechanisms) != {"B0", "B1", "B2"}:
        raise QueueError("mechanisms must be exactly B0, B1, B2")
    ...
    _exact_keys(
        measurement,
        {
            "enabled_in_all_cells", "sources", "phase", "success_conditioned",
            "moving_teacher_authorized", "required_before_scientific_interpretation",
        },
    ...
    if measurement != {
        ...
        "phase": "inclusive_pre_0p30_or_same_attempt_post_0p40",
        "success_conditioned": False,
        "moving_teacher_authorized": False,
    ...
    # are the eleven explicitly registered lower-body Wave-B keys.
    ...
    for parent_name in ("W", "V"):
        ...
        if not all(item == invariant_maps[0] for item in invariant_maps[1:]):
            raise QueueError(f"parent {parent_name} cells differ outside lower-body factors")
    return queue
...
def _load_launch_manifest(
    ...
    if _require_commit(source["commit"], "launch manifest source.commit") != queue["source"]["commit"]:
        raise QueueError("launch manifest source commit differs from the reviewed Wave-B source")
...
def _mechanism_overrides(mechanism: Mapping[str, Any]) -> list[str]:
    key_map = (
        ("lower_body_pose_imitation_weight", "lower_body_pose_imitation_weight"),
        ("lower_body_pose_imitation_std", "lower_body_pose_imitation_std"),
        (
            "lower_body_pose_imitation_support_pre_s",
            "lower_body_pose_imitation_support_pre_s",
        ),
        ...
        ("lower_body_stability_bundle_weight", "lower_body_stability_bundle_weight"),
        (
            "lower_body_stability_min_stance_width_m",
            "lower_body_stability_min_stance_width_m",
        ),
        ("lower_body_stability_stance_scale_m", "lower_body_stability_stance_scale_m"),
        (
            "lower_body_stability_leg_velocity_margin_radps",
            "lower_body_stability_leg_velocity_margin_radps",
        ),
        ...
        ("lower_body_stability_support_pre_s", "lower_body_stability_support_pre_s"),
        ("lower_body_stability_support_post_s", "lower_body_stability_support_post_s"),
    )
    return [
        f"++task.rewards.{hydra_key}={_hydra_number(mechanism[field])}"
        for hydra_key, field in key_map
    ]
...
def _stage_run_name(job: Mapping[str, Any], stage: str) -> str:
    ...
    return f"phase1_lbwave_probe2_{job['id']}_seed3_20260720"
...
def _build_claim(
    ...
    *,
    ...
    if stage == "probe":
        content.update(
            {
                "purpose": "lower_body_stability_probe_not_science",
                "not_science": True,
                "attestable": False,
                "promotable": False,
...
def detached_head():
    completed = subprocess.run(
        ["git", "-C", source_checkout, "symbolic-ref", "-q", "HEAD"],
        check=False, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
        env=environment,
    )
    if completed.returncode == 0:
        return False
    if completed.returncode != 1 or completed.stdout.strip() or completed.stderr.strip():
        raise SystemExit("probe source detached-HEAD check failed")
    return True
if (
    git("rev-parse", "HEAD") != expected_commit
    or git("status", "--porcelain", "--untracked-files=all")
    or not detached_head()
):
    raise SystemExit("probe source is not the exact clean detached claimed commit")
    ...
    if b"Learning iteration" in line and not marker_observed:
        ...
        binding_content = {
            ...
            "source_state_at_binding": {
                "head": expected_commit, "clean": True, "detached_head": True,
            },
            "run_name": run_name,
            "run_dir": run_dir,
...
def detached_head():
    ...
    if completed.returncode == 0:
        return False
    if completed.returncode != 1 or completed.stdout.strip() or completed.stderr.strip():
        raise SystemExit("source detached-HEAD check failed")
    return True
...
if not detached_head():
    raise SystemExit("source checkout is not detached")
...
if content.get("source_state_at_binding") != {
...
if bound.get("purpose") != "lower_body_stability_probe_not_science" or bound.get("not_science") is not True:
...
fatal = re.compile(r"(^|[^A-Za-z0-9_])(Fatal|Traceback|OutOfMemoryError|OutOfMemory|out of memory|OOM|bad_alloc|Segmentation fault|NaN|Inf|Infinity)([^A-Za-z0-9_]|$)", re.I | re.M)
...
pose_contract = hard_contract.get("lower_body_pose_imitation_reward") if isinstance(hard_contract, dict) else None
bundle_contract = hard_contract.get("lower_body_stability_bundle_reward") if isinstance(hard_contract, dict) else None
if not isinstance(pose_contract, dict) or not isinstance(bundle_contract, dict):
    raise SystemExit("hard training contract lacks the paired lower-body Wave-B sections")
if hard_contract.get("processed_qdes_slew_hinge_reward") is not None:
    raise SystemExit("Wave-B hard contract unexpectedly enables processed-qdes slew")
for key, expected in spec["expected_pose_contract"].items():
    if pose_contract.get(key) != expected:
        raise SystemExit(f"hard training contract lower-body pose mismatch: {key}")
for key, expected in spec["expected_bundle_contract"].items():
    if bundle_contract.get(key) != expected:
        raise SystemExit(f"hard training contract lower-body bundle mismatch: {key}")
motion_clips = hard_contract.get("motion_clips")
if (
    not isinstance(motion_clips, list)
    or [item.get("sha256") for item in motion_clips if isinstance(item, dict)]
    != spec["expected_motion_sha256"]
):
    raise SystemExit("hard training contract motion clip SHA mismatch")
question_bank = hard_contract.get("question_bank")
if not isinstance(question_bank, dict) or question_bank.get("sha256") != spec["expected_bank_sha256"]:
    raise SystemExit("hard training contract question-bank SHA mismatch")
accumulator = EventAccumulator(str(rsl_dir), size_guidance={"scalars": 0})
accumulator.Reload()
def counter_rows(group, names, signed_names):
    rows = {step: {} for step in spec["expected_steps"]}
    for counter in names:
        tag = f"Live/lower_body_wave/{group}/{counter}"
        if tag not in accumulator.Tags().get("scalars", []):
            raise SystemExit(f"missing activation counter tag: {tag}")
        events = accumulator.Scalars(tag)
        if [event.step for event in events] != spec["expected_steps"]:
            raise SystemExit(f"activation counter has wrong/duplicate steps: {tag}")
        for event in events:
            value = float(event.value)
            if not math.isfinite(value) or (counter not in signed_names and value < 0):
                raise SystemExit(f"activation counter is invalid: {tag}")
            rows[event.step][counter] = value
    return rows
pose_rows_by_step = counter_rows("pose", spec["pose_counter_names"], [])
bundle_rows_by_step = counter_rows(
    "bundle", spec["bundle_counter_names"], ["gated_signed_stance_width_m_sum"]
)
pose_integer_counters = {
    "observed_sample_count", "support_eligible_sample_count",
    "reward_enabled_eligible_sample_count",
}
bundle_integer_counters = {
    "observed_sample_count", "support_eligible_sample_count",
    "reward_enabled_eligible_sample_count", "narrow_or_crossed_sample_count",
}
for step in spec["expected_steps"]:
    pose = pose_rows_by_step[step]
    bundle = bundle_rows_by_step[step]
    for counter in pose_integer_counters:
        if abs(pose[counter] - round(pose[counter])) > 1e-6:
            raise SystemExit(f"pose counter is not integral at step {step}: {counter}")
        pose[counter] = int(round(pose[counter]))
    for counter in bundle_integer_counters:
        if abs(bundle[counter] - round(bundle[counter])) > 1e-6:
            raise SystemExit(f"bundle counter is not integral at step {step}: {counter}")
        bundle[counter] = int(round(bundle[counter]))
    observed = pose["observed_sample_count"]
    eligible = pose["support_eligible_sample_count"]
    if (
        observed != bundle["observed_sample_count"]
        or eligible != bundle["support_eligible_sample_count"]
        or observed != spec["num_envs"] * spec["num_steps_per_env"]
        or not 0 < eligible <= observed
    ):
        raise SystemExit(f"paired lower-body denominator inconsistency at step {step}")
    expected_pose_enabled = eligible if spec["mechanism"] == "B1" else 0
    expected_bundle_enabled = eligible if spec["mechanism"] == "B2" else 0
    if (
        pose["reward_enabled_eligible_sample_count"] != expected_pose_enabled
        or bundle["reward_enabled_eligible_sample_count"] != expected_bundle_enabled
    ):
        raise SystemExit(f"reward-enabled activation mismatch at step {step}")
    bounded_tolerance = max(1.0, float(eligible)) * 1.0e-6
    for name in ("gated_kernel_sum",):
        if pose[name] > eligible + bounded_tolerance:
            raise SystemExit(f"pose bounded sum exceeds eligible denominator at step {step}: {name}")
    narrow = bundle["narrow_or_crossed_sample_count"]
    if not 0 <= narrow <= eligible:
        raise SystemExit(f"narrow/crossed count exceeds eligible denominator at step {step}")
    for name in ("gated_bundle_sum", "gated_stance_tail_sum", "gated_leg_velocity_tail_sum"):
        if bundle[name] > eligible + bounded_tolerance:
            raise SystemExit(f"bundle bounded sum exceeds eligible denominator at step {step}: {name}")
    expected_bundle_sum = (
        bundle["gated_stance_tail_sum"] + bundle["gated_leg_velocity_tail_sum"]
    ) / 2.0
    identity_tolerance = max(1.0, abs(expected_bundle_sum)) * 2.0e-6
    if abs(bundle["gated_bundle_sum"] - expected_bundle_sum) > identity_tolerance:
        raise SystemExit(f"bundle component identity mismatch at step {step}")
pose_rows = [{"step": step, **pose_rows_by_step[step]} for step in spec["expected_steps"]]
bundle_rows = [{"step": step, **bundle_rows_by_step[step]} for step in spec["expected_steps"]]
pose_totals = {
    counter: sum(row[counter] for row in pose_rows)
    for counter in spec["pose_counter_names"]
}
bundle_totals = {
    counter: sum(row[counter] for row in bundle_rows)
    for counter in spec["bundle_counter_names"]
}
if pose_totals["gated_reference_motion_l1_mean_sum"] <= 0.0:
    raise SystemExit("two-update probe did not observe nontrivial lower-body motion reference")
receipt_content = {
    "schema_version": 1,
    ...
    "activation": {
        ...
        "pose": {"rows": pose_rows, "totals": pose_totals},
        "bundle": {"rows": bundle_rows, "totals": bundle_totals},
    },
...
def _validate_activation_payload(
    ...
    *,
    ...
    _exact_keys(activation, {"expected_steps", "pose", "bundle"}, label)
    ...
    def validate_group(
        ...
        integer_counters: set[str],
        signed_counters: set[str],
    ) -> tuple[list[dict[str, float]], dict[str, float]]:
        group = _mapping(activation[group_name], f"{label}.{group_name}")
        _exact_keys(group, {"rows", "totals"}, f"{label}.{group_name}")
        rows = _list(group["rows"], f"{label}.{group_name}.rows")
        if len(rows) != len(PROBE_STEPS):
            raise QueueError(f"{label}.{group_name}.rows must contain exactly two updates")
        calculated = {counter: 0.0 for counter in counter_names}
        normalized: list[dict[str, float]] = []
        for expected_step, raw_row in zip(PROBE_STEPS, rows):
            row = _mapping(raw_row, f"{label}.{group_name}.rows[{expected_step}]")
            _exact_keys(
                row,
                {"step", *counter_names},
                f"{label}.{group_name}.rows[{expected_step}]",
            )
            if row["step"] != expected_step:
                raise QueueError(f"{label}.{group_name} has wrong or reordered steps")
            result: dict[str, float] = {"step": expected_step}
            for counter in counter_names:
                number = _finite_number(
                    row[counter], f"{label}.{group_name}.{expected_step}.{counter}"
                )
                if counter not in signed_counters and number < 0:
                    raise QueueError(
                        f"{label}.{group_name}.{expected_step}.{counter} must be nonnegative"
                    )
                if counter in integer_counters:
                    if not number.is_integer():
                        raise QueueError(
                            f"{label}.{group_name}.{expected_step}.{counter} must be integral"
                        )
                    number = int(number)
                result[counter] = number
                calculated[counter] += number
            normalized.append(result)
        totals = _mapping(group["totals"], f"{label}.{group_name}.totals")
        if set(totals) != set(counter_names):
            raise QueueError(f"{label}.{group_name}.totals counter set changed")
        for counter, expected in calculated.items():
            observed = _finite_number(
                totals[counter], f"{label}.{group_name}.totals.{counter}"
            )
            if not math.isclose(observed, expected, rel_tol=0.0, abs_tol=1e-6):
                raise QueueError(
                    f"{label}.{group_name}.totals.{counter} does not sum the rows"
                )
        return normalized, calculated

    pose_rows, pose_totals = validate_group(
        "pose",
        POSE_PROBE_COUNTERS,
        {
            "observed_sample_count", "support_eligible_sample_count",
    ...
    bundle_rows, bundle_totals = validate_group(
        "bundle",
        BUNDLE_PROBE_COUNTERS,
        {
            "observed_sample_count", "support_eligible_sample_count",
    ...
    for expected_step, pose, bundle in zip(PROBE_STEPS, pose_rows, bundle_rows):
        observed = int(pose["observed_sample_count"])
        eligible = int(pose["support_eligible_sample_count"])
        if (
            observed != int(bundle["observed_sample_count"])
            or eligible != int(bundle["support_eligible_sample_count"])
            or observed != expected_observed_sample_count
            or not 0 < eligible <= observed
        ):
            raise QueueError(f"{label} paired denominator inconsistency at {expected_step}")
        expected_pose = eligible if mechanism == "B1" else 0
        expected_bundle = eligible if mechanism == "B2" else 0
        if (
            pose["reward_enabled_eligible_sample_count"] != expected_pose
            or bundle["reward_enabled_eligible_sample_count"] != expected_bundle
        ):
            raise QueueError(f"{label} reward-enabled counter mismatch at {expected_step}")
        tolerance = max(1.0, float(eligible)) * 1.0e-6
        if pose["gated_kernel_sum"] > eligible + tolerance:
            raise QueueError(f"{label} pose kernel exceeds denominator at {expected_step}")
        if not 0 <= bundle["narrow_or_crossed_sample_count"] <= eligible:
            raise QueueError(f"{label} narrow/crossed count is invalid at {expected_step}")
        for counter in (
            "gated_bundle_sum", "gated_stance_tail_sum", "gated_leg_velocity_tail_sum",
        ):
            if bundle[counter] > eligible + tolerance:
                raise QueueError(f"{label} {counter} exceeds denominator at {expected_step}")
        expected_sum = (
            bundle["gated_stance_tail_sum"] + bundle["gated_leg_velocity_tail_sum"]
        ) / 2.0
        identity_tolerance = max(1.0, abs(expected_sum)) * 2.0e-6
        if abs(bundle["gated_bundle_sum"] - expected_sum) > identity_tolerance:
            raise QueueError(f"{label} bundle identity mismatch at {expected_step}")
    if pose_totals["gated_reference_motion_l1_mean_sum"] <= 0.0:
        raise QueueError(f"{label} did not observe a nontrivial lower-body reference")
...
def _validate_probe_receipt(
    ...
    _validate_activation_payload(
        content["activation"],
        job["mechanism"],
        f"probe receipt {job['id']} activation",
...
def _probe_verifier_spec(
    ...
    claim_path, binding_path = _claim_paths(queue, job, "probe")
    mechanism = queue["mechanisms"][job["mechanism"]]
    pose_weight = _hydra_number(mechanism["lower_body_pose_imitation_weight"])
    pose_std = _hydra_number(mechanism["lower_body_pose_imitation_std"])
    pose_pre = _hydra_number(mechanism["lower_body_pose_imitation_support_pre_s"])
    pose_post = _hydra_number(mechanism["lower_body_pose_imitation_support_post_s"])
    bundle_weight = _hydra_number(mechanism["lower_body_stability_bundle_weight"])
    stance_min = _hydra_number(mechanism["lower_body_stability_min_stance_width_m"])
    stance_scale = _hydra_number(mechanism["lower_body_stability_stance_scale_m"])
    qdot_margin = _hydra_number(
        mechanism["lower_body_stability_leg_velocity_margin_radps"]
    )
    qdot_scale = _hydra_number(
        mechanism["lower_body_stability_leg_velocity_scale_radps"]
    )
    bundle_pre = _hydra_number(mechanism["lower_body_stability_support_pre_s"])
    bundle_post = _hydra_number(mechanism["lower_body_stability_support_post_s"])
    return {
        "schema_version": 1,
        "queue_id": queue["queue_id"],
        "job_id": job["id"],
        ...
        "pose_counter_names": list(POSE_PROBE_COUNTERS),
        "bundle_counter_names": list(BUNDLE_PROBE_COUNTERS),
        "expected_motion_sha256": [
            manifest.content["assets"]["motion_forehand"]["sha256"],
            manifest.content["assets"]["motion_backhand"]["sha256"],
        ],
        "expected_bank_sha256": manifest.content["assets"]["training_question_bank"]["sha256"],
        "expected_pose_contract": {
            "schema_version": 1,
            "enabled": float(mechanism["lower_body_pose_imitation_weight"]) > 0.0,
            "probe_enabled": True,
            "activation_ledger": "weight_independent_control_step_counters",
            "weight": float(mechanism["lower_body_pose_imitation_weight"]),
            "std_rad": float(mechanism["lower_body_pose_imitation_std"]),
            "support_pre_s": float(mechanism["lower_body_pose_imitation_support_pre_s"]),
            "support_post_s": float(mechanism["lower_body_pose_imitation_support_post_s"]),
            "racket_command_name": "racket_target",
            "motion_command_name": "motion",
            "joint_count": 12,
            "joint_names": list(EXPECTED_LEG_JOINTS),
        ...
        "expected_bundle_contract": {
            "schema_version": 1,
            "enabled": float(mechanism["lower_body_stability_bundle_weight"]) < 0.0,
            "probe_enabled": True,
            "activation_ledger": "weight_independent_control_step_counters",
            "weight": float(mechanism["lower_body_stability_bundle_weight"]),
            "min_stance_width_m": float(
                mechanism["lower_body_stability_min_stance_width_m"]
            ),
            "stance_scale_m": float(mechanism["lower_body_stability_stance_scale_m"]),
            "leg_velocity_margin_radps": float(
                mechanism["lower_body_stability_leg_velocity_margin_radps"]
            ),
            "leg_velocity_scale_radps": float(
                mechanism["lower_body_stability_leg_velocity_scale_radps"]
            ),
            "support_pre_s": float(mechanism["lower_body_stability_support_pre_s"]),
            "support_post_s": float(mechanism["lower_body_stability_support_post_s"]),
            "racket_command_name": "racket_target",
            "motion_command_name": "motion",
            "leg_joint_count": 12,
            "leg_joint_names": list(EXPECTED_LEG_JOINTS),
            "foot_body_names": list(EXPECTED_FOOT_BODIES),
            "joint_order": "runtime_articulation_subsequence",
        ...
        "expected_applied_markers": [
            "[train.py]     rewards.action_rate_l2.weight=-0.1",
            f"[train.py]     rewards.lower_body_pose_imitation.weight={pose_weight}",
            f"[train.py]     rewards.lower_body_pose_imitation.params.std={pose_std}",
            f"[train.py]     rewards.lower_body_pose_imitation.params.support_pre_s={pose_pre}",
            f"[train.py]     rewards.lower_body_pose_imitation.params.support_post_s={pose_post}",
            "[train.py]     rewards.lower_body_pose_imitation_probe.weight=1.0",
            f"[train.py]     rewards.lower_body_stability_bundle.weight={bundle_weight}",
            (
                "[train.py]     rewards.lower_body_stability_bundle.params."
                f"min_stance_width_m={stance_min}"
            ),
            (
                "[train.py]     rewards.lower_body_stability_bundle.params."
                f"stance_scale_m={stance_scale}"
            ),
            (
                "[train.py]     rewards.lower_body_stability_bundle.params."
                f"leg_velocity_margin_radps={qdot_margin}"
            ),
            (
                "[train.py]     rewards.lower_body_stability_bundle.params."
                f"leg_velocity_scale_radps={qdot_scale}"
            ),
            (
                "[train.py]     rewards.lower_body_stability_bundle.params."
                f"support_pre_s={bundle_pre}"
            ),
            (
                "[train.py]     rewards.lower_body_stability_bundle.params."
                f"support_post_s={bundle_post}"
            ),
            "[train.py]     rewards.lower_body_stability_bundle_probe.weight=1.0",
        ],
...
def _remote_launch_body(
    ...
    fatal_pattern = (
        r"(^|[^[:alnum:]_])(Fatal|Traceback|OutOfMemoryError|OutOfMemory|out of memory|OOM|"
        r"bad_alloc|Segmentation fault|NaN|Inf|Infinity)([^[:alnum:]_]|$)"
    )
        ...
        "jobs": [
            {
                ...
                "run_dir": job["run_dir"],
                "weights": {
                    "lower_body_pose_imitation": queue["mechanisms"][job["mechanism"]]["lower_body_pose_imitation_weight"],
                    "lower_body_stability_bundle": queue["mechanisms"][job["mechanism"]]["lower_body_stability_bundle_weight"],
                },
            }
            for job in queue["jobs"]
        ],
