...
def _cross_engine_probe(output_root: Path, ready_pose: Path, plant_xml: Path) -> dict:
    ...
    runner._verify_full_a_runtime_postimport(runtime)
    augmented_runtime_mjb = (
        runner._plant_contract_module().serialize_augmented_mjb_identity(
            runner._mujoco_module(), env.mj_model
        )
    )
    ...
    summary = cross_engine_tape.write_probe_record(
        ...
        runtime_identity={
            ...
            "augmented_runtime_mjb": augmented_runtime_mjb,
