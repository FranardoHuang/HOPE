...
- 相机帧率 **≥ 120 Hz**（在球速超过 5 m/s 的竞技性球体跟踪中，推荐 240–360 Hz；HOPE 的青瞳场地在对打期以 **300 Hz** 运行）
- 在跟踪体积内达到亚毫米级的重建精度
...
**在竞赛与对打（play）过程中，动作捕捉系统不得跟踪乒乓球拍（paddle）。** 对打期间不应在球拍上放置或贴附任何反光标记点或跟踪资产。这是一项与 HITTER 框架及 HOPE 竞赛设计相一致的、刻意的架构性决策。需注意：该禁令仅是竞赛/对打阶段的规则——在内部数据采集/物理标定阶段，球拍**会**被动捕跟踪（连同球台 4 个角点与球网 2 个角点），用于模型拟合：
...
**执行（仅限竞赛/对打）：** 在竞赛布置过程中，裁判将核实球拍、机器人手部，以及超出机器人躯干/骨盆上最后一个被跟踪刚体标记点的腕部连杆上，均无回射材料。内部数据采集/物理标定阶段贴附于球拍的标记点必须在对打前移除。
...
PPT/球台位姿用于布置与标定锚定（世界系原点与漂移检测），在对打过程中为可选项。HOPE 场地已确认的对打期实时流集合为：**机器人基座（骨盆）位姿 + 球体位置，300 Hz**。
...
| 信息 | 来源 | 使用方 |
|-------------|--------|---------|
| 300 Hz 的球体位置 [x, y, z] | 动捕 → ROS 2 话题 | 规划器（阶段 1–3） |
...
> **说明（2026-07-03）：** 旋转测量计划在即将开展的物理建模阶段实施。它需要带图案/可刚体跟踪的球，且动捕中继（当前将球以仅含位置的 `PointStamped` 发布）届时必须同时转发姿态信息。
...
> **状态（2026-07-03）：** 实际实现并用于 sim-to-real 阶段的技术栈是青瞳/VRPN 路径——`vrpn_mocap` 客户端（第 6.5.1 节）加 `hope_bringup` 的 `avatar_pro_vrpn_relay`。`motion_capture_tracking` 仍是面向 OptiTrack/Vicon 场地的未使用备选方案。
...
| 话题 | 消息类型 | 内容 | 频率 |
|-------|-------------|---------|------|
| `/poses` | `geometry_msgs/PoseArray` | 所有被跟踪刚体及自定义跟踪标记点 | 相机帧率（HOPE 场地：300 Hz） |
| `/tf` | `tf2_msgs/TFMessage` | 变换树：world → PPT、world → P1、world → P2、world → Ball | 相机帧率（HOPE 场地：300 Hz） |
...
/vrpn_mocap_client:
  ros__parameters:
    ...
    frame_id: "world"
    update_freq: 300.0            # 与 HOPE 场地已确认的 300 Hz 对打期流速率一致
    refresh_freq: 1.0
```
...
**在竞赛/对打期间，球拍不由动作捕捉系统跟踪**（在内部数据采集/物理标定阶段，球拍**会**被跟踪，连同球台 4 个角点与球网 2 个角点，用于模型拟合）。每台人形机器人必须通过关节编码器与被跟踪的 `base_link` 经正运动学来推断自身的球拍位姿。这是根本性的传感架构：外部感知（球体轨迹）馈入基于模型的规划器，而内部本体感受（关节状态 + `base_link`）驱动定位球拍的全身控制器。完整的从 `base_link` 经 7 自由度手臂到 3D 打印球拍支架的正运动学链，参见配套文档《HOPE WBC 仿真训练参考设计》（第 2.8 节）。
