...
| L2 机制信号 | `4096 env`、一个阻断 seed、每卡四个不同机制单元 | 单变量/小因子配对在相对 `+200/+500/+1000` checkpoint 是否产生方向一致的增长 | 不复制 seed；整组止损，让位给其他机制 |
| L3 复现确认 | 胜者及其匹配对照补第二个独立 seed | 增益是否大于 seed 噪声；正反手较差侧是否也改善 | 不宣称赢家；不解锁第三、第四 seed |
| L4 正式门禁 | 仅正式候选补足 `3–4` seed、terminal、双引擎、连续卷与 Gate 3B | 候选是否可迁移、可恢复并满足完整 24 切面 | 不晋级部署 |

L0/L1 是 fail-closed 机制检查，不因“曲线看起来好”而豁免。L2/L3 可以省 GPU；L4 不能省证据。

### Seed 是晋级税，不是首轮并发单位

2026-07-13 的回看结论是：旧 `SZ`（共用拍面语义、零关节摩擦）在第 2000 次迭代已经因最差
seed `20/100` 失去稳定性资格；把四个 seed 都继续买到第 4000 次迭代，确实补强了“不是晚熟”以及
拍面符号盲区的取证，但对“拒绝该 baseline”购买了过量证据。以后不再把这个历史四-seed 全格点
当成默认模板。

新实验的 seed 预算逐门解锁：

1. **L0/L1 不买 seed 复现。** 先用确定性负控、一个 seed 的小环境冒烟证明开关真的 applied、
   量尺能区分 `n/-n`、tensor finite、合同和日志字段存在。
2. **L2 一张卡的四槽优先放四个不同的因果单元。** 同一阻断 seed、同一父 checkpoint/动作/题库/
   plant/预算，只展开能回答机制主效应和关键交互的 `2×2`；每个单元到固定预算即停，不因卡有空位
   增发同配方 seed。
3. **L3 只给胜者和它的匹配对照补第二个 seed。** 单独给“胜者”补 seed 会失去 paired effect，
   所以每次复现购买的是一对，而不是排行榜上的一条臂。第二个 seed 不复现增益，就回到机制层，
   不再用第三、第四 seed 投票救活配方。
4. **L4 才补足 `3–4` seed。** 只有两 seed 的方向一致、较差侧不退化、同卷 MuJoCo 没抹掉增益，
   且候选确实可能成为 accepted baseline 时，才估计 tail risk、terminal 稳定性和正式门禁。

这条制度不允许挑好 seed：所有已运行 seed 仍全量归档并进入结论。它只把“是否值得再采一个 seed”
变成前一层证据的显式结果。
...
- 已冻结的 2026-07-11 fresh from-scratch 长跑 `0 -> 16999`：`0`、`1000`、`2000`，之后每
  `2000` 一次直到 `16000`，再判 terminal `16999`；该批预注册的“`<8000` 绝对低分不止损”只约束
  这批历史长跑，不再外推成未来机制 canary 必须白跑 8000 次；
- 2026-07-13 起的新机制 canary：只跑到相对 checkpoint `+1000`，存档点为现有 save cadence 能直接
  产生的 `+200/+500/+1000`。由于 runner 从 0 起数且 terminal 是 `max_iterations-1`，fresh 必须显式
  用 `max_iterations=1001` 才会真的写出 `model_1000.pt`；热启动则把三个相对偏移加到父迭代号，
  同样用 1001 次调用预算。`+200/+500` 看训练内有符号拍面、较差侧、finite/KL/跌倒和 paired delta；
  `+1000` 才消费同一冻结小卷。若需要延长，必须先把延长条件和下一判卷点写入原实验纸；
...
- **硬止损**：合同/哈希错、NaN/Inf、不可恢复 crash、开关未生效、train/exam 泄漏、结果删失或
  分母错误。先保留日志，不自动换配方重试。
- **证据止损**：正式长跑仍要求至少两个相邻里程碑和至少一个 `50/侧` 判卷点都显示候选在较差侧
  被对照支配，且没有其他预注册主指标补偿，才停整对/整组。固定预算机制 canary 在相对 `+1000` 自动
  终止，本来就不承诺长跑；它只能决定“是否购买第二 seed/延长”，不能据此宣称 family 永久失败。
- **晋级**：至少两个独立 seed 的差值方向一致，较差侧不倒退，且 MuJoCo 没有把 Isaac 增益全部
  抹掉。峰值 checkpoint 与 terminal 都保存；terminal 不是自动最佳模型。
- 两个 seed 是 L3 复现门，不再作为 L2 第一轮的入场费；额外 seed 只在 L4 估计 tail risk。

## 六卡使用：历史满池结果与下一轮纠偏

两台 Pod 各有三张 5090，广度阶段容量布局是 `4/4/4 + 4/4/4 = 24` 个并发槽；2026-07-11 的
历史矩阵也恰好预注册 24 个实验。最初六条只占每卡一个槽，不能称为“跑满”。但“24 个预注册实验”不自动等于
...
这套完整因子阵现在属于**已结案的历史设计**，不是下一池的资源模板。它成功暴露了 seed 不稳定和
拍面符号盲区，但在量尺已经推翻 baseline 后继续复制 seed 的边际信息过低。下一轮拍面修复只先占
一张卡四槽，四个单元共享 seed 3、动作、题库、零摩擦 plant 与总预算：

| 单元 | 初始化 | 线性拍面引导 | 回答的问题 |
| --- | --- | --- | --- |
| A | 从历史强但正手反面的 checkpoint 热启动 | 关 | 仅修诚实尺后，旧局部最优会不会自己恢复 |
| B | 与 A 同一父 checkpoint | `-0.4`，角度上限 `pi` | 非饥饿角度梯度能否把反面拉回 |
| C | 从零训练 | 关 | 旧配方在修正 telemetry 后的干净对照 |
| D | 与 C 同一随机初始化 seed | `-0.4`，角度上限 `pi` | 引导是否从零就能避免进入反面局部最优 |

A/B 是几天内演示的快速修复路径，C/D 是不洗谱系的机制证据；二者同时存在也能检验“引导效果是否
依赖热启动”这一交互。首轮只到相对 `+1000`，不发第二 seed。正式 run 名、源提交、父 checkpoint SHA、
训练/考试题 SHA、启动命令与 stopping paper 必须在拍面修复进入 `main` 后物化并通过 validator，
因此本段不是发射授权。
...
补同一配对缺口；证据止损释放的槽优先补队首的**不同机制单元或已过离线门的动作处理**；下一 seed
只有在 L2 晋级纸允许后才进入队列。没有合法队首时宁可短暂空槽，也不临时发一条
