...
| 2026-07-15 | 随机横向躯干推力先有可审计事务合同，再接模拟器 | Franco | Codex | 旧草案允许调用方伪造 ACK、异步写后提前签收、坏回执遗留非零力，并在击球/窗口中断时丢失冲量账。现 source-only 状态机要求无副作用 preflight、写前 host-visible 校验、原子 commit、异常后永久 `DIRTY/UNKNOWN`，逐环境保存 sampled/commanded/applied/abandoned 守恒；默认关闭且 `launch_authorized=false`。 | `4a4ecd9`、[扰动卷宗](experiments/2026-07/EXP-P1-LATERAL-BALANCE-PERTURBATION.md)、[adapter 合同](interfaces/lateral_perturbation_adapter_contract.md) | 仍无真实 Isaac adapter、zero readback、no-sync/吞吐门、runtime ledger 或 held-out 球路行为；mock/source 通过不授权训练。 |
