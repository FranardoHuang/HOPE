...
| --- | --- | --- | --- | --- | --- | --- |
| 2026-07-13 | schema-3 题库可做严格加法式物理合同重绑定 | franco | Codex | 目标源码只新增 signed-face helper 时，旧 loader 会正确拒绝整文件 SHA；新 consumer 不走 legacy，而以 Git/AST、全部问题数组 raw bytes、精确四-leaf metadata 和全题 old/new bitwise physics replay 发布新制品。 | [signed-face 漏斗](experiments/2026-07/EXP-P1-SIGNED-FACE-RESCUE-FUNNEL.md) | consumer 尚未在 Pod 物化 bank；新 train family 的 exam 也未重绑定，不能启动 exact judge/L2。 |
