...
| --- | --- | --- | --- | --- | --- | --- |
| 2026-07-15 | post-swing 外生 teacher cold-start 进入源码合同 | Franco | Codex | 旧配对让每个 policy 只有活到自然 clip wrap 才自己填恢复 buffer，base-decel treatment 因此会改变 curriculum ready 时刻，不能再作主效应归因。main 现支持由 MotionCommand 自持 O_EXCL namespace 从 live natural-wrap articulation 采集 immutable teacher，restricted checkpoint attestor 绑定 fresh lineage/claim/hard contract，并让首个 reset 的采用率与状态 readback fail closed；公共数组 writer/callback 自报和 `weights_only=False` 已由红队关闭。 | `fde26f3`、[teacher artifact 合同](interfaces/post_swing_teacher_artifact.md)、[运行操作](operations/run_post_swing_teacher_capture.md)、[measurement 卷宗](experiments/2026-07/EXP-P1-V1V2-BASE-DECEL-MEASUREMENT-RERUN.md) | 只完成 source-only 合同；仍缺 Pod2 4096-env natural-wrap capture、实际 checkpoint attestation、first-reset readback 与 default-off runtime probe，`launch_authorized=false`，不能发科学训练。 |
