...
| --- | --- | --- | --- | --- | --- | --- |
| 2026-07-13 | checkpoint 与 detached-worktree 发射闭包对齐实际运行布局 | franco | Codex | 首版只扫顶层 tensor/合同键；次版未把 source-first 环境传给 child；第三版又在 `SimulationApp` 前执行 Isaac 包。现递归审计 `checkpoint["infos"]`，绑定 tracked setup/确定性环境 SHA，拒绝 local override，并用不执行包的 `find_spec` 验 exact module origin；旧失败证据均保留。 | [signed-face 漏斗](experiments/2026-07/EXP-P1-SIGNED-FACE-RESCUE-FUNNEL.md) | 仅修发射前闭包；L1 行为、directional paper、MuJoCo/Gate3 仍未通过。 |
