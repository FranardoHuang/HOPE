...
| --- | --- | --- | --- | --- | --- | --- |
| 2026-07-13 | checkpoint lineage/finite 审计对齐 RSL-RL 实际布局 | franco | Codex | 首版只扫顶层 tensor、从顶层找合同键，会把正常 RSL-RL checkpoint 假判为无浮点/无 lineage；现递归扫描嵌套容器，并只从 runner 写入的 `checkpoint["infos"]` 绑定 schema/SHA/lineage，旧 v1 证据保留。 | [signed-face 漏斗](experiments/2026-07/EXP-P1-SIGNED-FACE-RESCUE-FUNNEL.md) | 仅修发射前审计；L1 行为、directional paper、MuJoCo/Gate3 仍未通过。 |
