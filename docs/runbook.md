...
| 同卡 3–4 条广度消融 | 每条约慢 25–45%；07-11 实测 4 条/卡总显存约 `22.9–23.2 GiB`、GPU util `87–97%`。只用于“多测配方”优先的广度波。**2026-07-20 警告：此行标定自旧任务代际。physical_ball+task-revision 代际实测：独占 4.6 s/iter、6 条/pod 无衰减、12 条/pod 恶化 3.3×（CPU 线程挤爆 128 核）——该代任务 pod 总并发 ≤8 条（约 2–3/卡）才是吞吐最优；换代后先做并发短测再定容量。** |
