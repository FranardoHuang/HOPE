...
| 同卡 3–4 条广度消融 | 每条约慢 25–45%；07-11 实测 4 条/卡总显存约 `22.9–23.2 GiB`、GPU util `87–97%`。只用于“多测配方”优先的广度波。**2026-07-20 修订（同日两次实测后的最终结论）：physical_ball+task-revision 代际独占 4.6 s/iter（旧代 2.0–2.2 的 2 倍多）；单条延迟 ≈ 独占 × 同卡条数 × ~0.85（4 条/卡 ≈ 15 s/iter），瓶颈是同卡 SM 分时不是显存也不是 pod CPU（7 条/pod 与 12 条/pod 时同卡 4 条都 ≈15 s）。堆叠只多买 ~20–30% pod 总吞吐、却把每条到里程碑的墙钟拖长 ~4×——要快出单格结论就少堆，要整批总量就多堆；换代后先做并发短测。** |
...
3. 3–4 条/卡不是默认权利；只有广度消融、短测确认显存/利用率后才可用。四槽是四个可辨识机制
   ...
   长期执行规则（2026-07-20 立，同日按实测修订）：只要存在已预注册的广度矩阵，两 pod 都应保持
   **该任务代际吞吐最优的并发数**满载（本代 physical_ball+task-revision 实测为 ≤8 条/pod；旧代
   任务才是 3–4 条/卡），空出的槽从唯一队列最前的就绪格补位，不允许长时间整卡闲置；换代先短测。
...
> 完整的波级发射工序（引号完好的命令文件、2-iter 冒烟、串行等首迭代、no-clobber、精确停止、
> 判死条件）见 [消融波发射工序](operations/run_ablation_wave_launch.md)。本节是原始十条核对单。
...
> 判读与报数的常驻工序已收编到 [结果判读与报数](operations/read_and_report_results.md)；
> 本节保留原始裁定原文。
