...
| signed-face C2/D2 `v1r2` | 第二版 D2-only one-shot continuation：严格接受 trainer 实际发出的五键 compact `question_bank`，并独立解析 exact NPZ metadata 来绑定 physics contract。它在写 C2 attestation 或 claim D2 前都要求 v1r1 evidence root/C2 receipt/pair receipt、D2 arm 和 exact D2 run name 仍全部不存在；历史 absence 快照不构成发射授权。它不是新 seed、L2 或 causal continuation。 |
