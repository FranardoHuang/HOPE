...
| <a id="action-ball-table-pose-twist-contract"></a>`action_ball_table_pose_twist_n<N>` / 带桌面位姿与底座线速度的 N 动作球优先 actor 合同 | `hitter_footwork(177) + base_position_table(3) + base_orientation_table_6d(6) + base_lin_vel_heading(3) + signed-face/rho(4) + action_one_hot(N)`，总宽 `193+N`，所以 N1 为 194。绝对 9 值只回答 A3 相对桌体的 XYZ 与完整 roll/pitch/yaw；base/racket task 继续是 robot-relative residual。`base_lin_vel_heading` 是 root COM 的三轴线速度，经 base yaw 旋到 heading frame；部署 producer 以 OptiTrack 位置作无漂移锚，可选 IMU accelerometer 作高频传播，并处理 marker→COM offset 与 `omega × offset`。`base_ang_vel` 仍是三轴 pelvis IMU gyro。旧 182/191-D checkpoint 不能换名复用；C++/MuJoCo producer、噪声/延迟和外参 parity 未闭合前不得接真机。 |
