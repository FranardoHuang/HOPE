...
| <a id="a3-vendor-v1-profile"></a>`HOPEPingPongActionBallA3VendorV1` / 智元 A3 新一代 ActionBall 任务 profile | 以 2026-07-31 最新智元 A3 训练 setting 作训练侧权威的不可变 task leaf：采用新 29-DoF nominal 参数与 armature（head 保留仓内值）、startup Kp/Kd DR、`[0,2]` 控制步延迟和 `axis_box_6d_v2` push。它是 fresh 训练身份，旧 checkpoint/bundle/receipt 不得续成；选中 profile 也不等于 formal launch 授权。 |
| <a id="n1-vendor-baseline-diagnostic"></a>`N1 vendor baseline diagnostic` / 智元基线 N1 单卡诊断 | `launch_n1_vendor_baseline_diagnostic.py` 的窄发射身份：首轮动作固定且只允许 `bh_loop_c`（`bh_block` 机械拒绝），seed 只允许 `0/1/2`。authority 完成后才可发 `smoke=1×2`、`probe=4096×5`；`long=4096×20001` 在当前 revision 仍机械拒绝，未来还须命名的 `vendor_probe_gate_receipt`。每条 run 独占一张空物理 GPU 并使用 fresh no-clobber namespace；它不接受任意 Hydra override，也不产生 formal evaluator、promotion、resume、export 或 judge authority。 |
