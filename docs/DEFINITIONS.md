...
| <a id="action-ball-a211-c211"></a>`A211/C211` / fresh ActionBall A/C N1 合同 | A211 与 C211 actor 均为 `211-D = historical 225 - teacher_base15 + task_valid1`，但同宽 v2 的前15列已改成 localizer world `position3+orientation6D+linear_velocity3` 与 pelvis/body-frame IMU gyro3；不含 projected gravity 或第二份 world angular velocity。critic 均为内容未变的 historical `318 + task_valid1 = 319-D`。A 的 9-D task 是 desired contact p/v/face，C 的 9-D task 是 incoming-ball-at-contact p/v/spin。`task_valid=0` 时 task/base-goal/两钟及 task/contact/outcome reward 与其分母全部不记账，平衡、非任务 mimic 和 safety 保留。A/C actor normalizer/trainability 均为 v2，pre-IMU 同名211、历史225/318与A/C彼此 checkpoint/normalizer 均不得互用。 |
| <a id="action-ball-immutable-tape"></a>`immutable_tape` / ActionBall 单行固定中心题消融夹具 | 历史 target-information ablation 的单行、内容寻址题源：reset 恒取同一行，question/task online inverse 为零，同时冻结题分布和 curriculum authority。它不是“当前档题没变就复用答案”的 cache，不能进入 formal A211/C211 lineage；A 用正常 sampler/curriculum/RNG 加完整语义 answer cache，C 用 `direct_ball`。旧 tape 工具/收据只保留追溯。 |
