...
| <a id="balance-probe-generations"></a>Wave A `probe5 / probe6` / 第五代失败批与第六代替换批 | 两代都只是非科学的六格完整场景探针，不是训练结果。`probe5` 使用 fresh v4 namespace，得到五份 exact receipt；W-H 子进程处于 `fork→exec` 的短暂 argv 身份过渡时被一次性精确身份检查拒绝，监督器按 fail-closed 退出且没有科学结论，五份收据不得续用。`probe6` 使用 fresh v5 namespace 和新 manifest；`Popen` 后在首次 `/proc` 读取前先不可覆盖地保存 child PID，再做同一 PID/PGID/starttime 的有界 exec 身份等待，并把 launcher 与全部 postcheck 包进同一失败残留审计 transaction。当前尚未发射，只有同代六份 exact receipt 全齐才能生成训练命令。这里的 v4/v5 只是隔离运行目录的 namespace 代次，不是算法版本。 |
