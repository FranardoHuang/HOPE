...
| <a id="balance-probe-generations"></a>Wave A `probe5 / probe6 / probe7` / 第五至第七代探针 | 三代都只是非科学的六格完整场景探针，不是训练结果。`probe5` 使用 fresh v4 namespace，得到五份 exact receipt；W-H 在 `fork→exec` argv 身份过渡窗口被一次性检查拒绝，五份收据不得续用。`probe6` 使用 fresh v5 namespace，加入同一 PID/PGID/starttime 的有界 exec 身份等待和失败残留审计，但 W-C 在 trainer 前因 transaction wrapper 给 shlex-quoted multiline Python 参数逐行加空格而触发 `IndentationError`；其余五格未发，v5 永久只作基础设施失败历史。`probe7` 使用 fresh v6 namespace 和新 manifest，把 transaction body 改为逐字节保留、不做逐行缩进，并以 multiline payload byte-equality 回归锁定；当前尚未发射，只有 probe7 同代六份 exact receipt 全齐才能生成训练命令。这里的 v4/v5/v6 只是隔离运行目录的 namespace 代次，不是算法版本。 |
