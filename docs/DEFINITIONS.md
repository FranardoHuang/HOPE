...
| <a id="action-ball-teacher-start-contract"></a>`action_ball_table_pose_twist_heading_task_teacher_start_v2` / 无动作槽位、显式老师启动倒计时的 ActionBall actor 合同 | 当前 fresh N1 合同，固定 **194-D**：frame 一致合同的非身份列后增加 `time_to_teacher_start_s(1)`，并删除无语义的 `action_one_hot`。动作 UID/slot 仍只在 sampler/solver/curriculum/receipt 控制面冻结，绝不作为 policy 输入。该 v2 只授权 N1；formal N5/N73 必须先增加固定宽、由 reference 内容生成的连续 future-motion intent，再切新合同。绝对 9 值只回答 A3 相对桌体位姿，task 继续是 robot-relative residual；部署 pose 由 OptiTrack 锚定，`base_ang_vel(3)` 由 pelvis IMU gyro 提供。旧同宽 194-D/195-D checkpoint 都不能按宽度猜测或 exact resume；C++/MuJoCo producer、实测噪声延迟和外参 parity 未闭合前不得接真机。历史 `...teacher_start_n<N>`（`194+N`）仅保留旧 receipt/schema 解析，不再用于 fresh ActionBall 发射。 |
...
| <a id="a3-vendor-v1-profile"></a>`HOPEPingPongActionBallA3VendorV1` / 智元 A3 新一代 ActionBall 任务 profile | 以 2026-07-31 最新智元 A3 训练 setting 作训练侧权威的不可变 task leaf：采用新 29-DoF nominal 参数与 armature（head 保留仓内值）、startup Kp/Kd DR、`[0,2]` 控制步延迟和 `axis_box_6d_v2` push。首轮入窗诊断后，它还在原 `weight=4/std=0.075 m` 精位置核外独立开启 `weight=1/std=0.30 m` 粗位置核，两者使用同一 swing-through 目标和紧击球窗；其他 ActionBall task 的粗核默认为零。它是 fresh 训练身份，旧 checkpoint/bundle/receipt 不得续成；选中 profile 也不等于 formal launch 授权。 |
| <a id="n1-vendor-baseline-diagnostic"></a>`N1 vendor baseline diagnostic` / 智元基线 N1 单卡诊断 | `launch_n1_vendor_baseline_diagnostic.py` 的窄发射身份：动作固定且只允许 `bh_loop_c`（`bh_block` 机械拒绝），seed 只允许 `0/1/2`，所有阶段强制 stable-ready plant。authority 完成后才可发 `smoke=1×2`、`probe=4096×5` 与 `push_evidence=4096×32`；最后一项精确覆盖 `15.36 s` policy time，并钉住 interval scheduler 与六轴 velocity-push 实现，用来机械证明每个 env 都经历过至少一次 push。`long=4096×20001` 在当前 source commit 仍机械拒绝，后续只能由同 policy/source/seed 的健康 probe + push-evidence 具名 receipt 放行。每条 run 独占一张空物理 GPU 并使用 fresh no-clobber namespace；它不接受任意 Hydra override，也不产生 formal evaluator、promotion、resume、export 或 judge authority。 |
...
| <a id="axis-box-6d-v2"></a>`axis_box_6d_v2` / 六轴速度增量推撞 | schema v2 的对称六轴 root-velocity delta：`x/y=±0.25 m/s`、`z=±0.10 m/s`、`roll/pitch=±0.26 rad/s`、`yaw=±0.39 rad/s`，ActionBall vendor profile 每 `5–15 s` 触发一次。当前版本没有 strike/recovery phase gate，因此可在击球窗发生；不得冒充已实现门控。 |
| <a id="strike-window-entry-distance"></a>`strike-window entry racket distance` / 入击球窗拍距 | 每拍只在第一个 strike-window tick 记一次 `||racket_FK-racket_target||`；八个互斥 finite bin 由 `0.075/0.15/0.20/0.30/0.50/0.70/1.00 m` 七个边界切分，另记 nonfinite、总数与 finite sum。它只是诊断 ledger，不改 Reward/observation/reset/curriculum；若多数 entry `>0.20 m`，先修粗+细核，不继续烧 dead-kernel long。 |
| <a id="vendor-runtime-json-markers"></a>`HOPE_RSL_RL_RUNTIME_ABI_JSON` / `HOPE_POLICY_STD_UPDATE_JSON` / `HOPE_CONTROL_STEP_ACTION_DELAY_RUNTIME_JSON` | 分别在 fresh learn 前钉住 RSL-RL/normalizer ABI，每次 optimizer update 后报 realized policy std 与 learning rate，以及首个 rollout 前报 delay 的 initialized env 数和 lag histogram。任一状态非 finite、std `<=0`、normalizer 缺失/形状错误或 delay 分母不守恒都 fail-loud；这些是运行收据，不是学习成功证据。 |
