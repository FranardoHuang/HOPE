...
| <a id="action-ball-table-pose-n-contract"></a>`action_ball_table_pose_n<N>` / 带桌面相对底座位姿的 N 动作球优先 actor 合同 | `hitter_footwork(177) + base_position_table(3) + base_orientation_table_6d(6) + signed-face/rho(4) + action_one_hot(N)`，总宽 `190+N`，所以 N1 为 191。task 的 base/racket 位置仍是 robot-relative residual；新增 3+6 只表达 A3 相对桌面中心的完整 6-DoF 站位。部署时所有 actor 姿态/角速度相关项都以完成 marker→base 与 venue→table SE(3) 标定后的 ChingMu mocap 为权威，IMU 只可作独立安全冗余。当前 arbitrary-N/191-D C++ consumer 与旋转外参未闭合，不能接真机。 |
