...
| <a id="finite-qdes-execution-projection"></a>`finite q_des execution projection` / 有限目标投影执行 | 对超出运行目标包络、但数值仍有限的 raw `q_des`，执行端取最近的包络内目标；包络内映射严格恒等，不改变 policy sample 或 PPO log-prob，也不因此 reset。非有限请求、实际关节越过 hard limit、子步 ballistic crossing、桌碰和摔倒仍走 hard reset。 |
