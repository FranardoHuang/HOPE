...
| <a id="balance-probe-generations"></a>Wave A `probe5 / probe6 / probe7 / probe8` / 第五至第八代探针 | 四代都只是非科学的六格完整场景探针，不是训练结果。`probe5`/fresh v4 得到五份 exact receipt，W-H 在 `fork→exec` 身份过渡窗口被拒绝；`probe6`/fresh v5 的 W-C 因 transaction wrapper 破坏 multiline Python 参数而在 trainer 前失败，其余五格未发。`probe7`/fresh v6 的 W-C、V-C、V-N 得到三份 exact receipt；W-N 在 scene boot 冻结，由 180 s locked watchdog 对 exact 组 TERM/KILL 后 rc=`125`，W-H/V-H 未发。成功的 W-C/V-N 排除了 W parent 与 `action_rate=0` 各自作为必要失败原因，所以只记 infrastructure transient，不改 Reward 结论或 timeout。v4/v5/v6 均 immutable，禁止重试、补格或跨代混收据。`probe8` 使用 fresh v7 namespace；当前尚未发射，W-N 是唯一首发 canary，只有它先发布 exact receipt 才能发其他五格，最终仍须 probe8 同代六份收据全齐。这里的 v4/v5/v6/v7 只是隔离运行目录的 namespace 代次，不是算法版本。 |
