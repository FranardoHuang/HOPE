...
| <a id="action-ball-n-contract"></a>`action_ball_n<N>` / N 动作球优先 actor 合同（历史 181+N） | `N` 由 exact action-ball manifest 的动作数决定；actor 在共同 hitter/footwork 与拍面项之后追加 N 维 one-hot 动作身份。它保留为旧 checkpoint/诊断兼容，不是 fresh N5 的首选合同；fresh 训练使用下述 frame-consistent teacher-start `194+N` successor。不同 N、不同合同名或不同列语义都不能只换 motion 列表续跑。 |
...
| <a id="action-ball-teacher-start-contract"></a>`action_ball_table_pose_twist_heading_task_teacher_start_n<N>` / 显式老师启动倒计时的 ActionBall actor 合同 | fresh ActionBall 首选合同，宽 `194+N`，N1/N5/N73 分别为 195/199/267。frame 一致合同的全部列保持语义不变，在 heading face 后、final action one-hot 前增加 `time_to_teacher_start_s(1)`，直接读取 Motion phase governor，不让 MLP 重建逐动作时间常数。绝对 9 值只回答 A3 相对桌体位姿，task 继续是 robot-relative residual；部署 pose 由 OptiTrack 锚定，`base_ang_vel(3)` 由 pelvis IMU gyro 提供。旧 checkpoint 不 exact resume；C++/MuJoCo producer、实测噪声延迟和外参 parity 未闭合前不得接真机。 |
