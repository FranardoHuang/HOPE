...
| <a id="action-ball-teacher-start-contract"></a>`action_ball_table_pose_twist_heading_task_teacher_start_v2` / 无动作槽位、显式老师启动倒计时的 ActionBall actor 合同 | 当前 fresh N1 合同，固定 **194-D**：前 177 不是“本体状态”，而是 `reference/teacher 68 + robot/runtime 99 + task/clock 10`；后 17 是 `table pose/twist 12 + demanded face/rho 4 + time_to_teacher_start_s 1`。`base_ang_vel(3)` 是 pelvis 三轴角速度，`base_lin_vel_heading(3)` 是独立的 root-COM 线速度；合起来才是 6 个 twist 标量，`base_orientation_table_6d(6)` 则是姿态编码。frame 一致合同的非身份列后增加 `time_to_teacher_start_s(1)`，并删除 `action_one_hot`；动作 UID/slot 仍只在 sampler/solver/curriculum/receipt 控制面冻结，绝不作为 policy 输入。该 v2 只授权 N1；formal N73 必须冻结 ball-conditioned teacher/task/validity/history 的固定宽 ABI，但不增加 motion ID、one-hot 或换名后的 motion-intent code。不同动作由各自 `q_ref/dq_ref`、body reference 与 measured-paddle teacher trajectory 表达；只有未来出现“当前 teacher 状态相同但必要未来不同”的实证反例时，才考虑短时 future-teacher preview。绝对 9 值只回答 A3 相对桌体位姿，task 继续是 robot-relative residual；部署 pose 由 OptiTrack 锚定，角速度由 pelvis IMU gyro 提供。旧同宽 194-D/195-D checkpoint 都不能按宽度猜测或 exact resume；C++/MuJoCo producer、实测噪声延迟和外参 parity 未闭合前不得接真机。历史 `...teacher_start_n<N>`（`194+N`）仅保留旧 receipt/schema 解析，不再用于 fresh ActionBall 发射。 |
| <a id="action-ball-split-ready-teacher"></a>`action_ball_diagnostic_split_ready_teacher=true` / 实测单次挥拍的物理出生与老师首帧分离诊断 | 仅用于 `diagnostic_unauthorized` 的 measured N1：true reset 写入已通过 nominal-hold 的 physical-ready 根位姿、31 关节、hold action/qdes 与 delay history；随后在 receipt 给定的 pre-swing wait 内固定 teacher frame 0 并保留全身 imitation，让 policy 学 physical-ready 到专业动作首帧的过渡；原速播放非循环 clip 一次，末帧以正常 timeout 触发 true reset，禁止 wrap、复制首帧到末帧或把运动中的末帧当 ready。formal ready-to-ready 路径必须保持该 flag 为 `false`；这个开关不构成机械准入、训练成功、promotion 或真机授权。 |
| <a id="mujoco-direct-frame0-teacher-replay"></a>`--diagnostic-teacher-replay-handoff-mode=direct_frame0_playback` / MuJoCo teacher frame0 直接交接反事实诊断 | 只在 N=1、zero-PPO、`diagnostic_unauthorized` teacher replay 中，在合法题已接受、球仍为canonical park、尚未 launch 且准备等待刚结束的唯一边界，原子安装Motion frame0 root/q和零速，清robot solver/controller/qdes history，保留ball/task/lifecycle。安装后、真实transition前只读记录table keepout与backend-resolved robot/table contact，不写lifecycle latch。当步命令q0，下一步只允许自然frame1；必须exactly-one intervention。默认`split_ready_bridge`路径不变。该模式只回答bridge是否阻断teacher replay，不是physical birth、训练、physics parity、promotion或部署授权。 |
...
| <a id="fullmdp-fixed-action-first-divergence"></a>FullMDP fixed-action first-divergence / 完整任务固定动作首差诊断 | Isaac与MuJoCo从各自真实FullMDP构造边界消费同一tracked `512×H48×31` raw action tape，按共同关节顺序保存reset后与逐tick的root、q/dq、拍面几何及terminal。比较器只报告initial/post-step首个exact差异与数值包络；它是`diagnostic_unauthorized`定位工具，不自带容差、physics PASS、安全Gate、promotion或训练授权。 |
...
| <a id="action-ball-dr-l0-learnability"></a>`ActionBall DR-L0 learnability leaf` / 全新零随机可学性叶子 | 在 `stable-ready plant` 已关 torso CoM/link-mass/PD 的基础上，另关 robot `physics_material`、joint-default `±0.01 rad` 和 A/C actor 观测 corruption；delay/push/reset/task-target noise 也继续为零，但 PPO action-distribution 探索噪声不属于观测 corruption，不因 DR-L0 自动改变。它只用 fresh lineage 回答 nominal plant 能否学会 balance→mimic→hit→landing，不是 main/READY/robustness 配方。shared finalizer 已把缺失 joint-default event 编码为 ordered 31-D exact-zero decoder delta，并对所有 L0 轴 fail closed；当前剩余 blocker 是 A/C launcher+lineage 显式绑定 leaf/manifest/hard-contract 和 exact Pod 验证，完成前仍不得发射 causal long。 |
...
| <a id="vendor-v2-gpu-colocation"></a>`--allow-vendor-v2-colocation` / VendorV2 诊断双进程共驻开关 | `launch_n1_measured_vendor_v2_diagnostic.py template` 的显式、写入 exact claim 的布尔开关；默认不写即仍要求空 GPU。打开时仅允许同一物理 GPU 上最多两个 compute PID，且已有进程与新 claim 双方都必须 opt-in；已有 PID 必须用 `/proc` starttime/cwd/environment/exe/cmdline、同一 checkout+commit、dedicated VendorV2 namespace 的 no-clobber receipt、完整 claim schema、launcher bytes 与 exact training argv 交叉证明。每次 admission 还必须保留至少 `8192 MiB` 空闲显存；收据记录 PID、GPU UUID、total/free/used memory 和 namespace receipt pin。dead 历史 reservation 先按 PID+starttime/runtime handoff 判 stale 后忽略；任一 live 未知/不可证 co-resident 直接拒绝。该开关只是 `diagnostic_unauthorized` 算力 admission，不授权 formal/promotion/export/deploy/hardware。 |
...
| <a id="action-ball-delay-curriculum"></a>`DELAY-L0/L1/L2` / ActionBall 控制步延迟档 | 三个 fresh launch/resume-boundary recipe：L0 固定 `d=0`，L1 抽 `{0,1}`，L2 抽 `{0,1,2}`。每次升档都换 argv/recipe SHA/namespace，不在同一 optimizer 运行中热改 support。`d=2` 前必须先闭合两步 raw-action history/recurrence/known-lag alias。它不是仓内其它 D0/D1/D2 代号。 |
| <a id="action-ball-a211-c211"></a>`A211/C211` / 历史fixed-question ActionBall A/C N1合同 | A211/C211是203/219 FullMDP语义观测之前的历史`211-D actor / 319-D critic` lineage：A的9维task是desired contact p/v/face，C的是incoming-ball-at-contact p/v/spin。它们保留独立旧checkpoint/normalizer解析身份，但不再定义当前FullMDP观测，也不能按宽度或“同为v2”重标成semantic Observation V2。历史完整布局见[`policy_observation_action.md`](interfaces/policy_observation_action.md#historical-fixed-question-a211c211-contracts-not-current-fullmdp)。 |
| <a id="action-ball-immutable-tape"></a>`immutable_tape` / ActionBall 单行固定中心题消融夹具 | 历史 target-information ablation 的单行、内容寻址题源：reset 恒取同一行，question/task online inverse 为零，同时冻结题分布和 curriculum authority。它不是“当前档题没变就复用答案”的 cache，不能进入历史formal A211/C211 lineage；A 用正常 sampler/curriculum/RNG 加完整语义 answer cache，C 用 `direct_ball`。旧 tape 工具/收据只保留追溯。 |
| <a id="action-ball-banded-question-bank"></a>`banded_question_bank` / ActionBall 分档预解题库 | 可选的未来离线 producer 优化：按 exact curriculum domain-level/action/profile/solver identity 分块，reset 索引当前档已解行，缺块 fail closed。当前 module/builder 仍是 construction draft，但不再是 fixed 或 expanding long 的硬前置；A 可按完整语义 cache cold Q/Q'，C 不反解。只有真的采用 bank 时才必须补齐跨 owner RNG/tape-root、连续 base coverage 与 checkpoint lineage。 |
...
| <a id="ppo-num-steps-per-env"></a>`H` / `algo.runner.num_steps_per_env` / PPO rollout长度 | `H`是每个environment在一次PPO update中采集的control step数；一般每轮样本数=`num_envs × H`。当前FullMDP typed recipe取`H=48`，所以4096环境每轮是`196608` transitions；历史`H=24`才是`98304`。`H`不是墙钟目标，也不是MuJoCo-Warp EPA边缓冲的horizon。`algo.num_steps_per_env`不是现有key；强行加它只会产生trainer不读的死字段。 |
| <a id="vector-policy-step"></a>`vector policy step` / 并行策略步 | 一次policy推理后让全部并行环境各前进一个控制周期。每个update含`H`个vector policy step；`update_wall_s / H`是每个并行策略步墙钟，不是单个机器人的20 ms仿真时长。当前FullMDP用`H=48`。 |
| <a id="environment-step-throughput"></a>`environment-steps/s` / 环境步吞吐 | 所有并行环境产生的控制步总数除以墙钟时间；每轮分子是`num_envs × H`。当前4096环境、`H=48`时为`196608`，必须同时报告`H`和环境数，不能把旧H24墙钟直接当同尺度目标。 |
...
| <a id="qdes-projection-penalty"></a>`qdes_projection_penalty` / 投影前目标超限惩罚 | 从 raw `q_des` 到执行投影值的**投影前**归一化超出量计算的非正 Reward；不能读取投影后的零超差，否则 policy 学不到收回均值。历史设计曾把 `-20` 列为候选，但 current measured-VendorV2 learnability override 明确只接受 `qdes_projection_penalty_weight∈[-5,0]`：`-5` 是最强允许剂量，`0` 是显式零权对照，任何更负值都 fail-loud。显式 override 把 RewardManager 权重固定为 `-1`，并把真实 objective weight 作为受限 callable 参数；callable 返回 `(-objective_weight)×cost`，总 Reward 仍精确等于 `objective_weight×cost`。因此 `0` 权仍会执行、返回严格零 Reward，同时按 observed/projected sample 及逐关节 count/distance 记录 hypothetical unweighted penalty。 |
...
| <a id="action-specific-dynamic-ready"></a>`action-specific dynamic ready` / 动作专属动态准备合同 | teacher/reference 永远是该动作不可改写的 motion frame 0；physical spawn 优先直接使用 frame 0，若它在当前 plant 不可执行，则使用满足同一组具名 robust 物理裕量、并在该可行域内尽量接近 frame 0 的动作专属状态。历史 ready 只能作优化起点，不能继承 plant、hold 或授权。控制器出生目标、`last_action` 和 fresh actor 输出共同绑定所选 physical birth 的 live-prefix 已验证 `hold_qdes`；policy 从该物理状态学习跟踪独立 teacher，而不是脚本化瞬移。action、motion bytes、31 关节顺序、physical birth、hold candidate、prefix receipt 与 runtime decoder 必须一次性绑定，true reset 必须原子写入或整体回滚。该合同不改变 observation/action 宽度，不是 failure-buffer reset，也不授权训练成功、physics parity、部署或真机。 |
...
<a id="a211-frame0-exact-hold-receipt"></a>
`A211 frame0 exact hold receipt`：**SUPERSEDED/COUNTEREXAMPLE** 的 historical physical-birth
receipt schema。它把 measured frame0 root/q 当 physical birth 并推导 `62 policy tick / 248 physics
substep`；Pod direct screen 后同门槛为 `0/73`，所以 current A/C 不消费这份 schema。current physical
reset 是 tracked split-ready，`60/240/1.2 s` nominal hold只覆盖最长25 tick hidden WAIT；reveal 后
measured frame0只作teacher authority，bridge由policy学习。`200/800`也不是first-long前置。

<a id="threshold-first-safe-ready"></a>
`threshold-first safe-ready` / **SUPERSEDED/REJECTED** 的 direct-frame0出生尝试：曾先把 exact
measured frame0 root/q与零速度送入13项物理余量和fresh static-hold LP，通过才选零transition；
Pod 73动作同门槛结果=`0/73`，故它不再是current TODO或发车门。保留该术语只为解释历史artifact/
失败收据。current route是split-ready physical reset、WAIT中teacher也停split-ready、reveal后原子切
measured-frame0 teacher并用dense mimic学习bridge，不手写connector也不teleport physical state。
...
| <a id="vendor-v2-oracle2"></a>`oracle2` / VendorV2 两回合 teacher-qdes 动力学诊断 | VendorV2 code-owned launcher 的 fresh、单环境、零 PPO 阶段：用现有 dynamic-ready/fixed-tape/termination plant 连续完成两个 terminal episode，把 teacher joint target 经 live action decoder 执行并产 no-clobber JSON。它只验证诊断链与动力学可跟随性，不授权训练晋级、导出、部署或 formal judge。 |
| <a id="vendor-v2-oracle32"></a>`oracle32` / VendorV2 三十二回合 teacher-qdes 前置门 | 复用 `oracle2` 的 fresh `1 env / 0 PPO`、完整 lineage、no-clobber 输出、exact process-group cleanup 与 post-completion GPU admission，但固定 `32` 个 single-stroke terminal episode。code-owned 门要求每回合 exact-strike 且 p/v/face 分别严格小于 `0.075 m / 0.5 m·s⁻¹ / 15°`，`32/32` capture、零 reject、零 unknown、零 unexpected termination、pre-clamp qdes 误差 `≤2e-6 rad`，并要求 projection/qdes-soft/actual-soft/reference exposure 分母完整。reference-only 和 soft/projection intrusion 只分账，当前没有伪造通过上限。通过仍只是 `diagnostic_unauthorized` 的 fixed-question 学习前置，不代签 A225/C225、PPO 学习、formal judge 或部署。 |
