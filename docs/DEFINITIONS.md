...
| <a id="action-ball-n-contract"></a>`action_ball_n<N>` / N 动作球优先 actor 合同 | `N` 由 exact action-ball manifest 的动作数决定；actor 观测在共同 hitter/footwork 与拍面项之后追加 N 维 one-hot 动作身份。`N5` 表示当前五动作候选，`N93` 表示 93 动作候选；二者是不同 shape/identity，不能只换 motion 列表续跑同一 checkpoint。 |
