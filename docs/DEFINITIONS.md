...
| <a id="action-ball-n-contract"></a>`action_ball_n<N>` / N 动作球优先 actor 合同 | `N` 由 exact action-ball manifest 的动作数决定；actor 观测在共同 hitter/footwork 与拍面项之后追加 N 维 one-hot 动作身份。fresh exact `N5` 的固定顺序是 `bh_loop_c, v12_forehand_block, bh_block, s0_highpress, fh_loop_high`，旧 `fh_loop / fh_block_syn` 禁入；`N93` 只保留给未来 exact ordered 93 件 manifest。两者是不同 shape/identity，不能只换 motion 列表续跑同一 checkpoint；截至 2026-07-29 还没有可 formal 发射的 exact N93。 |
| <a id="action-ball-table-pose-n-contract"></a>`action_ball_table_pose_n<N>` / 带桌面相对底座位姿的 N 动作球优先 actor 合同（历史 191-D 候选） | `hitter_footwork(177) + base_position_table(3) + base_orientation_table_6d(6) + signed-face/rho(4) + action_one_hot(N)`，总宽 `190+N`，所以 N1 为 191。task 的 base/racket 位置仍是 robot-relative residual；新增 3+6 只表达 A3 相对桌面中心的完整 6-DoF 站位。姿态由完成 marker→base 与 venue→table SE(3) 标定后的 OptiTrack 负责；三轴角速度仍由 pelvis IMU gyro 负责，不能把 mocap 姿态差分冒充 gyro。该合同已由下述 194-D successor 取代；当前 arbitrary-N/191-D C++ consumer 与旋转外参也未闭合，不能接真机。 |
...
| <a id="action-ball-table-pose-twist-heading-task-contract"></a>`action_ball_table_pose_twist_heading_task_n<N>` / frame 一致的桌面位姿、底座速度与相对击球任务合同 | fresh ActionBall 首选合同。宽度仍为 `193+N`，N1 为 194，但 target-minus-current-racket position、demanded racket velocity 与 raw-A signed face normal 全部统一到 base yaw-heading frame；`rho` 不旋转。绝对 9 值 `base_position_table(3)+base_orientation_table_6d(6)` 只回答 A3 相对桌体的 XYZ 与完整 roll/pitch/yaw；base/racket task 继续是 robot-relative residual。`base_lin_vel_heading(3)` 是 root COM 三轴线速度；部署 pose 由 OptiTrack 锚定，`base_ang_vel(3)` 由 pelvis IMU gyro 提供。旧同宽 194-D checkpoint 因列语义不同不能 resume；C++/MuJoCo producer、实测 OptiTrack/IMU 噪声延迟和外参 parity 未闭合前不得接真机。 |
