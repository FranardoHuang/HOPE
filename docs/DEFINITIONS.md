...
| one-shot continuation / `v1r1` | 旧 trainer 已按原配方完成，但外层验证器假拒绝后使用的**一次性续接控制器**。它只消费内容绑定的旧 claim/log/hard-contract/checkpoint，并且只能 claim 尚未存在的配对臂；不能覆盖旧证据、重跑已完成臂、改变训练配方或自动 retry。这里的 `v1r1` 只表示 C2 只读取证后补跑未 claim 的 D2，不是新 seed、L2 或 causal continuation。 |
