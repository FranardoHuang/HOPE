# ActionBall 双后端长跑 TODO 历史归档

> 状态：`historical / superseded / read-only evidence`
>
> 本页收纳 2026-08-19 至 2026-08-28 已被取代的双后端 TODO 章节。现役唯一可执行清单只看
> [双后端长跑当前 TODO](../operations/action_ball_dual_backend_longrun_todo_20260819.md)；项目优先级仍只认
> `origin/main:docs/NOW.md`。归档正文从原页原样迁移，仅把两个原本指向同级 `operations/` 文件的相对链接
> 改为移动后的路径，不重写历史结论。
...
### HISTORICAL — V5执行清单（现役局部步骤只认§0.6；不改变`origin/main:docs/NOW.md`）
...
## HISTORICAL / FROZEN — 2. V5已采用的 successor 合同（current只认§0.6）

### 2.1 HISTORICAL — PPO（近端策略优化）V3（current见§0.6）
...
维护；恢复与调用见[`setup_local_sync`](../operations/setup_local_sync.md#bind-the-exact-epa48--rsl-rl-312-site-for-portable-full-a)。

## HISTORICAL / SUPERSEDED — 3. 旧依赖顺序（现役只认§0.6清单）
...
其余训练操作边界见[训练工序](../operations/run_training.md)。
