...
| [EXP-P1-BALANCE-ACTION-SLEW-20260720](2026-07/EXP-P1-BALANCE-ACTION-SLEW-20260720.md) | 腿腰恢复期 processed-q_des 突变铰链能否比全身 raw-action dense 平滑更好地降摔而不伤击球？ | Franco | Codex | ready | E3 mechanics / E1 result | [Wave A/B](../DEFINITIONS.md#balance-stability-waves)：probe8/v7 只发 W-N→Pod1 GPU1，4096 env 建立后在 `sim.reset` 以 `malloc(): invalid size (unsorted)` / outer `134` 退出；无 first iteration/binding/RSL/checkpoint/receipt，其他五格未发，两 Pod 已闭合。这是 pre-managers/pre-RewardManager infrastructure-only evidence，v7 immutable。[probe9 fresh v8 crossover 探针](../DEFINITIONS.md#balance-probe-generations)已预注册/manifest-bound 但未发；W-N→GPU0 与 W-C→GPU1 依次通过后才串行其他四格，任一失败即 freeze，结论仍 `inconclusive/not adopted` |
