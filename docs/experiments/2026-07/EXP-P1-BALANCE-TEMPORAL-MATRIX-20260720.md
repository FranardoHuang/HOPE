...
**实测算力推翻排程假设**：本任务代际（physical_ball + planner task-revision）单条独占 4.6 s/iter，
6 条/pod 仍 4.6 s（无衰减），12 条/pod 恶化到 15.2 s（3.3×）——瓶颈是 CPU（12×16=192 线程挤 128 核），
不是显存。2 条/卡的 pod 总吞吐比 4 条/卡高约 60%。旧手册"4 条/卡"标定自更轻的任务代际，对本代失效。
