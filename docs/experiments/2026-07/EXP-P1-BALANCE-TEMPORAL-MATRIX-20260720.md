...
## 2026-07-20 运行态快照（13:40 UTC）

23/24 格健康训练（iteration 7034–7568，零 Traceback，GPU 85–97%、约 22.5 GiB/卡）。实测吞吐约
11–13 s/iter（4 条/卡 + physical_ball + planner revision，比 runbook 单跑基准慢约 4 倍属预期），
10001 updates 全程约 33 小时；+500 观察点已过，+1000（7700）预计当日晚间。早期定性信号：V 父本
全部格子的窗口摔倒计数 ~380–440，W 父本 0–1——父本分层如设计生效，机制内比较待后续里程碑。

**v_h_s2 基础设施失败（2 次，封格）**：原格与逐字 `_r2` 重试都冻结在
`Starting the simulation`（与 probe7 W-N / probe8 / v8 attempt1 同类，均发生在 11 个 Isaac 并发
运行时的冷启动窗口）。按预注册"每格一次逐字重试"规则封格，不作 V×H×S2 机制证据；若 7700
裁剪后出现安静窗口，可另行预注册第三次尝试。当前矩阵按 23/24 运行。两次尝试的进程均已按精确
PID TERM 收口，GPU2 三条在训格不受影响。

## 2026-07-20 预算修订与动态调度（Franco 授权动态清退）

**实测算力推翻排程假设（当日二次修正归因）**：本任务代际（physical_ball + planner task-revision）单条
独占 4.6 s/iter；单条延迟 ≈ 独占 × 同卡条数 × ~0.85（同卡 4 条 ≈ 15 s，且与 pod 总数无关——7 条/pod 和
12 条/pod 时同卡 4 条都 ≈15 s）。瓶颈是**同卡 SM 分时**，不是显存、也不是最初误判的 pod 级 CPU。
堆叠换 ~20–30% 总吞吐、代价是单条到里程碑的墙钟 ~4×；对单 seed 筛选，10001 updates 的原预算在
4 条/卡下要 ~40 小时，不成比例。

**三项决定**：
1. screening 终点从 +10001 前移到 **+4000**（终档约 model_10700）；+2000 中段、+4000 完整单 seed 结论。
   40 小时的单 seed 筛选不成比例。
2. **动态清退**（+700–1100 行为账本快照存档于两 pod `scheduling_ledger_20260720.jsonl`）：
   - 永久停（容量让位，非科学负例）：`{w,v}_n_{s1,s2,s3}` 六格（N=完全无平滑的交互角信息价值最低；
     N×S0 阴性对照保留）。最后档 model_7300–7500。
   - 暂停待续（槽位释放后从最后 checkpoint 续训）：`{w,v}_n_s0`、`{w,v}_h_{s1,s2,s3}` 八格
     （最后档 model_6900–7800）。
   - 保持活跃：C 全行八格 + `{w,v}_h_s0`。
3. 释放算力给 [Wave P push 波](EXP-P1-PUSH-ROBUSTNESS-20260721.md) 六个大小臂（Franco：push 是平衡的希望）。
   目标并发 ~8 条/pod。

**+700–1100 早期信号（只作方向，不是结论）**：V 父本全线 pre-strike fall 0.22–0.24（挥拍前就倒——
push 波正要打的病）；T 轴已分层：V×C 回球/拍 0.86–0.88 显著高于 V×N/H 的 0.62–0.76，W×C 0.32–0.33
亦领先 W×N/H；S 轴四档暂无分离（机制权重小、价值函数尚在适应，符合预期）。
