...
## 2026-07-16 十二臂全表只读复核

单次 SSH 只读审计覆盖全部 12 条接受臂；三张 GPU 仍为 `4/4/4` trainer，利用率
`88%/88%/96%`，fatal 全为 `0`。每条 PID=PGID、GPU context 与 immutable claim/binding 均匹配；
latest checkpoint 的 filename=embedded iteration、finite、schema-3 hard contract、launch claim 和
fresh lineage=`1` 五项全部通过。source `2c2d70d...607e` 与 archive `6d93bcb...80b` 均 clean exact。

| 人话格 | PID=PGID | iteration / latest checkpoint | latest checkpoint 五项 |
| --- | ---: | ---: | --- |
| 10 秒、左臂继续模仿 | 2148472 | 1181 / 1100 | PASS |
| 10 秒、左臂自由 | 2149153 | 1188 / 1100 | PASS |
| 16 秒、左臂继续模仿 | 2149826 | 1243 / 1200 | PASS |
| 16 秒、左臂自由 | 2150538 | 1147 / 1100 | PASS |
| 24 秒、左臂继续模仿 | 2151254 | 1126 / 1100 | PASS |
| 24 秒、左臂自由唯一 retry | 2158534 | 1061 / 1000 | PASS |
| 三项跟踪近似均分 | 2153484 | 1096 / 1000 | PASS |
| 位置主导 | 2154234 | 1075 / 1000 | PASS |
| 速度主导 | 2154953 | 1125 / 1100 | PASS |
| 拍面主导 | 2155850 | 1075 / 1000 | PASS |
| 低强度跟踪 | 2156650 | 1074 / 1000 | PASS |
| 双倍总强度唯一 retry | 2159350 | 1030 / 1000 | PASS |

最近 21 updates 的同口径训练诊断给出以下方向：16 秒自由臂相对 16 秒对照同时改善位置通过率
`0.318→0.896`、composite `0.199→0.317`、virtual return `0.269→0.376`、completion
`0.798→0.825`，pre/post fall 也从 `0.099/0.046` 降到 `0.066/0.023`，是当前最强的 matched
方向信号。10 秒自由臂与对照近似打平；24 秒自由臂没有显示平衡优势。因此“解除非击球臂模仿的收益会随
episode 变长而单调增加”目前不成立，不能只挑 16 秒窗口作结论。

Reward 配比中，三项近似均分当前最均衡。位置主导主要只把位置通过率提高到 `0.940`，速度通过率随之
下降；速度主导、拍面主导和低强度格都出现明显跨项塌陷。双倍总强度虽把位置通过率推到 `0.930`，但
pre/post fall 升到 `0.111/0.055`，高于近似均分的 `0.081/0.030`，没有形成更好的整体方案。

以上都只是单 seed、单末窗的 Isaac 训练诊断。12 条的 exact-hit 均仅约 `0.47%–0.54%`，需要真实击球
后才有意义的稀疏结果样本仍太少；因此本轮不得停臂、晋级或形成最终消融定论，继续按冻结长曲线观察点
运行，最终行为仍以 vendor MuJoCo 同卷为准。
