...
| N1-SPEED | `IN_PROGRESS` | P0a 已把 strict 1.1 倍同题带 `4096×5` 从旧 exact source 的均值 `17.088 s/update` 降到 `10.206 s/update`，改善 `40.3%`；五轮三组深层 update JSON 逐轮相等，五份 checkpoint 全 finite。反手拉同实现 probe 又达到 `2.90/2.87/12.51/5.97/9.83 s`，均值 `6.816 s/update`，证明相同热路径在不同动作/reset 负载下已接近目标，但健康线尚未稳定。diagnostic-only lean Motion timing validator 已在 exact source `2c3a39fe` 通过 Pod focused `22 passed`；三套相关回归为 `66 passed`，另有 1 个父提交 `bd340479` 同样失败的旧 event-contract fixture。唯一下一动作是等自然空闲槽补 `1 env×2`/same-seed `4096×5` parity 与吞吐，同时继续把 reset broker/receipt 的逐 env Python 收敛为 device/batch 事件。精确桌碰固定税仍只有约 `0.22 s/update`，保留 exact 后端；box/prism 仅是 backend 失效或长 trainer 证明非线性放大时的降级 | Reward、Done、采球、solver、动作、RNG、P/A/R、task identity、qdes/brake、raw-hard/table/fall 和 checkpoint finite 必须继续等价；lean validator 必须保留 Motion 消费的 generation、timing、horizon、pending-wait 与篡改负例，且失败零 partial write；4096-env 健康窗稳定达到 `≥15k environment-steps/s` 或等 reset 负载 collection `≤6.5 s` | lean validator 的 Pod focused PASS 不替代 smoke/trainer 吞吐，当前三条 long 不热补。formal checkpoint 粒度 receipt 仍是 N5 前独立 schema 工作，不由 diagnostic fast path 代替 | [设计与加速审计](../../research/design_audit_and_speedup_20260729.md)、[N=1 发射工序](../../operations/run_ablation_wave_launch.md) |
