...
| --- | --- | --- | --- | --- | --- |
| N1-DIAG-PROBE | `READY` | **三份 final pin 已在同一 clean C0=`7587124d…` 自然完成并复算**：loop/block policy=`ddcc1a7c…a09f` / `73d9de68…1e51`，adaptive effective Reward=`6520f153…63db`、receipt file=`fbf1c09c…2960`、31 个非零项且无 `racket_strike_success`；claims=`555aed65…` / `e512c38a…` / `310464fc…`。三者均 0 PPO、0 checkpoint、全 authorization false；PID/lock 已自然释放、source clean。Pod Isaac 相关组 `403 passed`，两轮独立审计 P0/P1=0 | 唯一下一动作：把三 SHA 原子回填 code-owned launcher 形成窄 C1，host/Pod focused 后固定 checkout 切 C1；然后三 lane `1×2` smoke，禁止再插科学 feature | 旧 `0670ad1f` adaptive r1 永久 spent；新证据根 `/workspace/franco/evidence/final_pins_7587124d`。GPU1 属于另一用户 PID `152495`，严禁触碰 | [本轮外部尽调](../../research/dr_reward_external_diligence_20260731.md) |
...
| N1-LONG-GATE | `BLOCKED` | producer/consumer、exactly-once natural completion、runtime loader、v7 mechanical gate 与三份同源 final pin 已收口；旧 6% probe 永久 `REJECTED_BY_POD`。无新 `4096×5`、无 push receipt，因此不得产生 gate receipt 或发 `4096×20001` | 新 root-cause source 上的 probe + push-evidence 同时 PASS 才能物化具名 gate receipt；receipt 绑 exact source/spec/policy/authority、自然完成、finite/ABI/std/delay、actual-hard/table/fall/nonfinite 与 push 运行证据；actual-hard/nonfinite 零容忍，手写 JSON、spent namespace 与 FAIL 运行都无法放行 | 当前唯一链：三 pin 回填 C1 → 三 lane `1×2` → `4096×5` → push `4096×32`；不再加宽 guard，不新增 acceleration/jerk governor。Pod2 GPU0/2 空闲，GPU1 属于另一用户 | [N=1 发射工序](../../operations/run_ablation_wave_launch.md) |
