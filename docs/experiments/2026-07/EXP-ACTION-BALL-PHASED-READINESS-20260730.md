...
| VENDOR-DYNAMIC-RECIPE | `IN_PROGRESS` | wrapper 已于 `2430fbb2` 跟踪并在 Pod 通过 `83` 个相关测试。r1 claim=`e37f8169…e32` 通过 clean authority、schema-v2 pre-scene binding 和 GPU 门，但 `MotionCommand` 仍只消费 schema-v1，因 kind mismatch 在 env 构造期 fail-closed，零 recipe/零 PPO；精确 PGID 已 `TERM`，GPU0 回到 18 MiB。消费端现已接受并严格验证 schema-v1/v2 及 v2 plant/delay 字段，新负例合跑 `28 passed`。唯一下一动作是提交消费端修复并用 fresh namespace 重跑 recipe-only | recipe claim 与新 policy SHA 可复算；authority/candidate/hold/bundle 逐字绑定；篡改任一 pin、非零 PPO 或多 env 均 fail-closed | r1 永久 spent；消费端修复尚未跟踪/Pod 重跑；不授权训练/export/judge/hardware | [N=1 发射工序](../../operations/run_ablation_wave_launch.md) |
