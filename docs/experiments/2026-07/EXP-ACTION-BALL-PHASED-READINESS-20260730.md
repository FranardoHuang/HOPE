...
| RUNTIME-ASSET-LOADER-V2 | `IN_PROGRESS` | **v7 运行再次证实这是 recipe 前真实 P0**：manual stress 必须使用 exact `private OpenGL:private GLU` 才能启动；而 base diagnostic、identity smoke、dynamic recipe 三个正式 exec 入口的 runtime-asset claim 只 pin GLU，并把 `LD_LIBRARY_PATH` 重建为单一 GLU 目录，会丢 `libOpenGL.so.0`。不允许靠继承 ambient 路径碰运气 | 先把 nested runtime-assets 升 schema/kind v2：同时 pin OpenGL/GLU 固定目录、真实 library SHA、direct SONAME 与 USD closure；`LD_LIBRARY_PATH` 必须逐字等于 `OpenGL:GLU`，禁止反序/尾部继承；三个入口只调用同一 claim-owned env helper。host 四组 launcher 回归+tamper 负例、Pod `ldd` exact、plan 负例与 fresh zero-PPO recipe natural completion 全通过 | 这是 final three-pin materialization 的唯一代码前置；旧 v1 claim fail-closed，不兼容迁移。先更新本账本后开发；不改 Reward、plant、动作、观测或训练 setting | [N=1 发射工序](../../operations/run_ablation_wave_launch.md)、[本地同步](../../operations/setup_local_sync.md) |
| LIVE-CONTRACT-MATERIALIZER | `IN_PROGRESS` | **runtime→authority→dynamic-ready→nominal-hold→contact-bundle 的双动作链已真跑通，12 份新工件已合入，loop/block contact bundle 已回填 registry**：两动作均完成 `0.8 s / 160 physics / 40 policy` hold PASS，双脚接触率 1.0、无 terminal、plant/delay 合同闭合 | loop candidate/hold/bundle SHA=`847ffe78…/22010d5d…/bd91b652…`，block=`3692f4c3…/49948c41…/bbfb612a…`；八个 pin 对 tracked bytes 逐项 SHA 一致。host 集成 `238 passed`；Pod Torch contact bundle 全量 `17 passed`；RUNTIME-ASSET-LOADER-V2 后从 final source 零 PPO 重物化三个缺失 pin | action-specific artifact 链已关闭，但旧 runtime-asset v1 不能用于新 recipe。OpenGL v2 修复后按 `recipe/hash → smoke1×2 → probe4096×5 → push4096×32 → gate → long`，不回退旧工件 | [N=1 发射工序](../../operations/run_ablation_wave_launch.md) |
