...
| N1-SPEED | `IN_PROGRESS` | r4 稳态 collection 实测均值 `23.79 s/update`（约 `4.13k environment-steps/s`），learning 仅 `0.299 s`；粗拟合为 `12–14 s` 固定逐步/同步税 + `5.6–6.7 ms × reset`，约 1670 reset/update 又贡献 `9–11 s`。候选 `c0747d59` 已把 diagnostic safety 改为 device aggregate、取消逐 substep dense clone/逐 step identity/每 update 正式收据落盘，并把 reset 的 env/slot/UID/generation 多次 `.item()` 合为一次 host-row D2H；Pod focused suite 为 `134 passed`。源码变化使旧 solver pin 正确 fail-closed，现已重钉 profile `3c978844…762f`、base bundle `d9de51e8…b03d` 与 fast-ball bundle `398287f7…bdf`。唯一下一动作是提交这些工件，fresh exact checkout 重物化 policy recipe，再跑 `1 env×2` 与旧/新同 seed `4096×5` parity/吞吐 | Reward、Done、采球、solver、动作、RNG、P/A/R、task identity、qdes/brake、raw-hard/table/fall 和 checkpoint finite 必须等价；diagnostic 不产生 formal authority；4096-env 健康窗达到 `≥15k environment-steps/s` 或等 reset 负载 collection `≤6.5 s`。若未达，下一唯一热点是剩余 reset broker Python 和 per-step `bool/float` 同步，不优化 PPO | 当前 exact milestone 不热补丁；candidate 尚未通过真实 Isaac 构造/吞吐，不能称加速完成。formal 的 checkpoint 粒度 receipt 仍是 N5 前独立 schema 工作 | [设计与加速审计](../../research/design_audit_and_speedup_20260729.md)、[N=1 发射工序](../../operations/run_ablation_wave_launch.md) |
