...
| LIVE-CONTRACT-MATERIALIZER | `IN_PROGRESS` | venv entry 修复已在 `e22cc301…` 提交并推送：internal exec 保留 claim 已封印的 `/workspace/hope_isaac_venv/bin/python`，不再 resolve 到系统 Python；identity/required-identity 定向 `89 passed` + pycompile/diff-check PASS | 旧 GPU2 loop recipe namespace spent，零 recipe/零 PPO；contract/required-identity 仍 `None`，新 source 未过 Pod 前训练仍 fail-closed | **本次 fresh Pod recipe/smoke 前已先更新账本**。从下一 clean docs commit 建 loop/block 两个 detached worktree；每个动作顺序 recipe→exact PGID 自然退出→smoke→自然退出→独立 required identity，两动作不共用已变脏 checkout | [N=1 发射工序](../../operations/run_ablation_wave_launch.md) |
