...
| LIVE-CONTRACT-MATERIALIZER | `BLOCKED` | identity 工件+registry+consumer 已在 `63e08ea3…` 推送。Pod2 GPU2 的 loop recipe namespace 已在 Kit/PPO 前 fail-closed：identity launcher 将已钉住的 venv Python 做 `Path.resolve()` 后退化为裸 `/usr/bin/python3.10`，formal pinner 因 `ModuleNotFoundError: yaml` 退出 `2`；namespace spent，零 recipe/零 PPO | 这是 launcher interpreter-identity source bug，不是 Reward/DR/动作结果；contract/required-identity 仍为 `None`，训练路径 fail-closed | **本次 launcher 修复前已先更新账本**。internal exec 改用 spec 已验证的 venv 路径（禁止 resolve 到系统 Python），补回归并推新 clean commit；loop/block 及 stress 都只用新 commit/fresh namespace | [N=1 发射工序](../../operations/run_ablation_wave_launch.md) |
