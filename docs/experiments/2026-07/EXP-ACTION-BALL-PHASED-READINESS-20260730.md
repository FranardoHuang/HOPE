...
| DUAL-ENVELOPE-STRESS-HARNESS | `IN_PROGRESS` | v4 单步 false-negative 已独立复核；v2 schema 的 **4×5-ms full-horizon source 已实现**。每 env/tick 记录 q/qdot/qdes；ON 4/4 tick 必须 strict Hctrl，ON/OFF 8/8 tick 必须 strict Hmech，OFF tick1 必须进 `[Hctrl,Hmech)`，qdes=q0/finite/restore exact；旧 20-ms proxy 保持原采样点但标为 telemetry-only。host focused `24 passed`、`py_compile`、`diff --check` 通过 | 独立 source review 收口后提交 clean commit，Pod2 GPU0 用新 confirm token `SIM_ONLY_A3_DUAL_POSITION_ENVELOPE_8ENV_FOUR_TICKS` 生成 no-clobber v2 receipt；canonical 内容验签且严格 PASS 才进 final recipe pins | v4 namespace 永久 spent。full-horizon receipt 前不进 `4096×5`；不改变 2% Hctrl、6% guard、plant、gain 或 Hmech 阈值 | [G05](../../gates/G05_isaac_training_first_loop.md) |
