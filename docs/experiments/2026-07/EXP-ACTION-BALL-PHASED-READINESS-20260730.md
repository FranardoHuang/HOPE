...
| DYNAMIC-READY-PATH-IDENTITY | `IN_PROGRESS` | **采用方案已实现并通过红队**：producer 的 absolute provenance 只按完整 registry relative component suffix 解释；relative 必须逐字等于 registry；`.`/`..`、重复/尾斜杠、双根、控制字符、relative prefix、同 basename 异目录全拒绝。action/frame0/artifact motion SHA/runtime-contract SHA 先验不变，随后对 current commit blob + worktree motion SHA 再双验；training consumer 的 current candidate/hold canonical absolute path + file/content SHA binding 未改。整合回归 `139 passed`、pycompile/diff-check PASS，独立审查 P0/P1=0；真实 tracked loop/block r2 validator 正例均通过 | 唯一下一动作：提交并推送 clean C0，在固定 `/workspace/franco/a3vendor_final_pin` 切到该 C0，重新生成 fresh loop/block specs 并要求 correct `plan` 在不创建 namespace 时产出含 stable-motion path/SHA、v2 loader 与 dynamic binding 的 claim；随后才 launch 两条 zero-PPO recipe。该修复未重物化 candidate/hold/bundle，C0 policy pins 仍未产生 | Pod plan 若仍拒绝则按新错误入账，不复用旧空 plan、旧 spec 或 spent namespace；policy/Reward pins 必须来自该 clean C0 的自然完成结果 | [N=1 发射工序](../../operations/run_ablation_wave_launch.md)、[G05](../../gates/G05_isaac_training_first_loop.md) |
