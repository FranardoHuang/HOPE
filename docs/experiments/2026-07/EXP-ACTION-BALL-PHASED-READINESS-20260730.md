...
| N1-DIAG-PROBE | `IN_PROGRESS` | exact source=`e7787e25`、policy=`e408b845…c65d`、claim=`819e6125…9ccf` 的 `4096 env×5` 已自然完成，五轮 wall=`4.74/9.26/8.81/8.58/8.74 s`。入窗拍距共 `100` 个，**`97% >0.20 m`**、均值 `0.434 m`，直接采用 `std=0.30 m` 粗核并保留 `std=0.075 m` 精核。另有每轮 `875/3684/3143/3193/3191` 次 actual-hard terminal，主因 waist-roll、次因 waist-pitch；qdes-hard 始终 0，最小实际越界仅约 `6.18e-4 rad`。已确认 vendor launcher 漏传 `stable_ready_plant=true`；另外 safety guard 在 delay queue 之后每 physics substep 立即生效，`lag=2` 不是延迟根因，而是现有 2% plant-state trigger 到硬边反应距离不足。host 实现已收口：stable-ready 唯一 override、粗+精核、**2%→5% 只提前制动不放宽硬边**、新 Reward SHA `8220f339…54dc3` 和因果收据均已接线。唯一下一动作是冻结 clean S0 source，在 Pod1 GPU0 重跑 identity recipe→identity smoke 以产生新 schema-3 authority | launcher plan/claim 明确持有 stable-ready；粗核默认关闭、仅新 vendor recipe 启用；actual-hard terminal 仍使用原始机械硬边；新 5 份 checkpoint finite，actual-hard 为 0 或不再持续成风暴，且 std/LR/normalizer/delay 收据继续成立 | 这不是放弃智元 DR：Kp/Kd 目标与其他 vendor DR 仍保留在正式 task，按既有裁定在健康 1000 update 后逐轴恢复。GPU1 无 owner sidecar 保持不动 | [本轮外部尽调](../../research/dr_reward_external_diligence_20260731.md) |
