...
| --- | --- | --- | --- | --- | --- |
| A3-VENDOR-ID | `IN_PROGRESS` | 以最新智元 A3 训练 setting 作 fresh 训练物理权威；29-DoF nominal/armature、派生 action scale、startup Kp/Kd 拆键、`[0,2]` control-step delay 与六轴 push 已进 host 源码门。tracked `required_identity.v1.json` 当前为 `awaiting_runtime_materialization`、contract SHA 为空，`plan` 已机械拒绝。唯一下一动作是用一次性 [`A3 vendor identity smoke`](../../DEFINITIONS.md#a3-vendor-identity-smoke) 固定 vendor task/`bh_loop_c` upper/seed0，先 recipe-only、再 `1 env×2` 产出 schema-3 contract；随后更新 manifest 为 materialized，并从该合同重物化 dynamic-ready、nominal-hold、policy/bundle/spec/claim | identity smoke 自然完成且 checkpoint/contract finite；spec/authority/artifact 三方 SHA 相等；新 nominal-hold 跨 `t_hit+margin`，无 actual-hard/table/fall/nonfinite；receipt/bundle 内含 vendor profile 与新常数 SHA；新 artifact 不与旧 N1 混用 | identity-smoke launcher host focused `15 passed`，但尚未 Pod 执行；旧 nominal-hold v1 只有 `plant_contract_match=true`，已被拒绝，不能作新 plant 证据 | [外部尽调](../../research/dr_reward_external_diligence_20260731.md)、[G04](../../gates/G04_sim_modeling_mujoco_isaac.md)、[G05](../../gates/G05_isaac_training_first_loop.md) |
| N1-VENDOR-POD1 | `BLOCKED` | 新身份不做科学 A/B；用单卡 [`N1 vendor baseline diagnostic`](../../DEFINITIONS.md#n1-vendor-baseline-diagnostic) 跑 seed `0/1/2`。tracked authority 当前为 `awaiting_runtime_materialization` 且 SHA 为空，`plan` 已机械拒绝；先按 A3-VENDOR-ID 物化。之后严格串行 Kit boot，每卡先 `smoke=1 env×2`，再 `probe=4096×5`。若入窗拍距大部分 `>0.20 m`，转粗+细核修复；stage-evidence v4 已能消费 delay receipt，`long=4096×20001` 另须 probe 后生成的 `vendor_probe_gate_receipt` | exact clean source/spec/claim/no-clobber；spec/authority/artifact 三方 SHA 相等；smoke/probe 有真 PPO iteration、finite checkpoint、runtime ABI/std/LR/delay receipt；无 table/fall/nonfinite/raw-hard 风暴；episode 跨 `t_hit`；明确 `diagnostic_unauthorized=true` | 等 A3-VENDOR-ID 新 contract/receipt/bundle。Pod1 GPU2 旧残留已按 exact sidecar `TERM` 释放，GPU0/GPU2 可用；GPU1 缺 sidecar保持未动。formal N1 另因 action-set registry/trust/receipts 缺失而 blocked，不由本诊断绕过 | [N=1 发射工序](../../operations/run_ablation_wave_launch.md)、[设计与加速审计](../../research/design_audit_and_speedup_20260729.md) |
