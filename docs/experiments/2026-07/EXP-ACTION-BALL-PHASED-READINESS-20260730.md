...
| LIVE-CONTRACT-MATERIALIZER | `IN_PROGRESS` | **runtime→authority→dynamic-ready→nominal-hold→contact-bundle 的双动作链已真跑通，12 份新工件已合入 head `1b45e6b9…`，loop/block contact bundle 已回填 registry**：两动作均完成 `0.8 s / 160 physics / 40 policy` hold PASS，双脚接触率 1.0、无 terminal、plant/delay 合同闭合 | loop candidate/hold/bundle SHA=`847ffe78…/22010d5d…/bd91b652…`，block=`3692f4c3…/49948c41…/bbfb612a…`；八个 runtime/identity/authority/contact pin 对 tracked bytes 逐项 SHA 重算一致。host 集成 `238 passed`，upper/contact `11 passed, 6 full-body deselected`（本机无 Torch，Pod 补跑）；交叉动作与篡改 pin 仍 fail-closed | private OpenGL+exact PYTHONPATH 恢复路径已进 runbook；提交/push clean source 后进入 shared stress。action-specific artifact 链关闭不等于 long 放行，`4096×5→×32` 仍是硬门 | [N=1 发射工序](../../operations/run_ablation_wave_launch.md) |
...
| DUAL-ENVELOPE-STRESS-HARNESS | `IN_PROGRESS` | Pod2 GPU0 exact `ddb3fd0c…` 的 hang 已定位在 `gym.make` 的 manager/command 构造阶段，尚未到 `env.reset`、limit 写入或唯一 `sim.step`，因而不是 PhysX FAIL。入口分叉现已修复：harness 通过 Hydra compose `HOPEPingPongActionBallA3VendorV1` 并复用 `train.py::_apply_task_overrides`，N=1 motion/manifest 从 code-owned registry 解析；不再把 raw Gym 默认误当 long 配置 | host focused `20 passed`、`py_compile` 与独立红队通过；delay `[0,2]`、2% Hctrl、6% guard、`max_inward_until_nonoutward_v1` 保留，motion/contact debug visualization 关闭；新增 vendor-bind、`gym.make`、reset、Hctrl/mixed readback、`sim.step` 前后唯一 marker | **本次 Pod 重跑前已更新账本**。先提交 clean source，再用新 no-clobber receipt 跑真实 8-env×1 tick；2% Hctrl、5 ms、ON/OFF 内外关系、qdes=q0、20 ms proxy 与 finally exact restore 全不放宽，不加 acceleration/jerk governor | [G05](../../gates/G05_isaac_training_first_loop.md) |
