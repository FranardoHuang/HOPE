...
| N1-SPEED | `IN_PROGRESS` | 第三批四刀、diagnostic 外围 rollback 裁剪（`096afb7b`）和 simulator/dynamic-ready 内层 rollback 裁剪（`4d631fb3`）均已完成 exact Pod smoke/probe。三组 update JSON 逐轮等价且 checkpoint finite，但 collection 均值分别为 `10.3804 s`、`10.6618 s`，内层裁剪两次 replicate 为 `10.7364/10.2878 s`；相对同 seed 基线 `10.0916 s` 均无可测收益。相同五轮 reset 数 `0/267/3103/875/2101`，粗算仍付 `4.9--8.0 ms/reset-env`。唯一下一动作是加严格 opt-in、仅 diagnostic 可用的分段 profiler，然后按最大段把 birth/broker/pool/solver/install 改为 compact batched reset；不再继续磨 rollback clone | 默认关闭 profiler 时零 wrapper/零 event/零新增同步；开启时每 update 至多一次汇总同步并报告 reset-env 分母。compact 路径仍须保持 Reward、Done、采球、solver、动作、RNG、P/A/R、task identity、qdes/brake、raw-hard/table/fall 与逐 update JSON 等价；关闭 profiler 的 4096-env 健康窗稳定达到 `≥15k environment-steps/s` 或等 reset 负载 collection `≤6.5 s` | 当前 long 不热补。strict 1.1× bundle SHA=`81dee53f…0351` 仍有效；每个 source 都重新物化 policy recipe 与 fresh spec/claim。formal checkpoint 粒度 receipt 仍是 N5 前独立 schema 工作，compact diagnostic 不能冒充 formal exact-resume | [设计与加速审计](../../research/design_audit_and_speedup_20260729.md)、[N=1 发射工序](../../operations/run_ablation_wave_launch.md) |
