...
| --- | --- | --- | --- | --- | --- |
| N1-RUN | `IN_PROGRESS` | fresh milestone1000 正在 Pod1 GPU2 用 claim `2710fd6f…d4f4` 运行，exact PID/PGID=`1134253`；只读快照已到 update 249，最近 40 update 均值 `27.10 s`，`model_200.pt` 的 80 个 tensor 全 finite。唯一下一动作是守护到 `model_300.pt` 做第二个趋势判读 | model_300/1000 finite 且身份不漂移；按里程碑报告 teacher fidelity、strike/capture/return、Reward income、table/fall/qdes-hard/actual-hard 与 iteration wall。前五轮腰 actual-hard 只记基线，不单独判学习失败 | 无外部阻塞；只管理该 exact run，不 signal/kill；NaN、身份漂移或 counter invariant 破坏才按预注册规则处理 | [N=1 发射工序](../../operations/run_ablation_wave_launch.md)、[结果判读](../../operations/read_and_report_results.md)、[G05](../../gates/G05_isaac_training_first_loop.md) |
| N1-SPEED | `IN_PROGRESS` | 第二批 candidate 已完成：diagnostic Motion timing handoff 复用一次 host identity D2H，fixed-18-draw refill overlap 从 `O(K²)` 改为 `O(K)`；Pod focused `187 passed`，fresh `1 env×2` 为 `2.67/2.45 s` 且 checkpoint finite；同 seed `4096×5` wall=`9.00/10.11/25.63/16.81/23.89 s`，三组深层 update JSON 与第一批逐轮完全相同。但均值 `17.09 s` 只比第一批快 `0.06 s`（约 `0.3%`），性能判定 FAIL。唯一下一动作是把 diagnostic 每个普通 step/strike step 的 invariant、finite、partition 与 EMA scalar 合成少量 device→host validation packet；若仍慢，再批量化 table-contact 与 joint-safety 小 kernel | Reward、Done、采球、solver、动作、RNG、P/A/R、task identity、qdes/brake、raw-hard/table/fall 和 checkpoint finite 必须继续等价；diagnostic 不产生 formal authority；4096-env 健康窗达到 `≥15k environment-steps/s` 或等 reset 负载 collection `≤6.5 s`。确定性优化只做 Pod parity/吞吐，不做学习 A/B，也不优化 PPO | 当前 exact milestone 不热补丁；两批 candidate 都正确但只累计改善约 `4.2%`，不能称加速完成。formal 的 checkpoint 粒度 receipt 仍是 N5 前独立 schema 工作 | [设计与加速审计](../../research/design_audit_and_speedup_20260729.md)、[N=1 发射工序](../../operations/run_ablation_wave_launch.md) |
