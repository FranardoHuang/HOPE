...
| LIVE-CONTRACT-MATERIALIZER | `IN_PROGRESS` | **Pod2 exact source `fdc43396…` 的双动作 runtime 身份已完成，四工件已集成并在 Git head `f0a949bc…` 回填 registry**：loop/block 各自 recipe→exact group 自然退出→`1 env×2 update` smoke→自然退出；4 个 checkpoint 均 86 tensors finite，delay/ABI/std marker 各 `1/1/2`，194-D/schema-3/31 关节/action-only 闭包通过 | loop live contract/required identity SHA=`637d1f63…/25833428…`，block=`91187192…/6fead111…`；两个 required identity 均 exact 12 组/31 唯一关节。A3 vendor 工件链 host 回归 `118 passed`；block smoke 的 1 次 table terminal 仅是身份证据，不冒充 safety | **本次 authority 物化前已再次更新账本**。现从 exact clean `f0a949bc…` 为 loop/block 分别生成 runtime-authority receipt，审核入 Git 后再进入 dynamic-ready/nominal-hold/contact-bundle；在 authority/bundle 完成前训练仍 fail-closed | [N=1 发射工序](../../operations/run_ablation_wave_launch.md) |
...
| DUAL-ENVELOPE-STRESS-HARNESS | `BLOCKED` | source/host 仍为 stress `13 passed`；Pod2 GPU0 exact `ddb3fd0c…` 运行已进入 8-env scene/simulation start，但在首个 MotionCommand 初始化警告后 **5 分钟无日志/无 receipt**、owned leader 持续约 111% CPU；按 PID=PGID=`157180`/starttime=`82370651` exact 绑定后 TERM，12 s 内整组自然清空，未 KILL；log/leader/pre-term 证据保留 | 该运行只判 `infrastructure/harness hang`，不得作 PhysX PASS/FAIL；输出路径已 spent，不删除重用；双动作 identity smoke 可继续，long hard gate 仍闭合 | 与 identity-smoke 并行定位 probe 为何在 env construction/reset 前后不返回；修复须新 clean commit + 新 no-clobber receipt，不放宽 Hctrl/tolerance，不加 acceleration/jerk governor | [G05](../../gates/G05_isaac_training_first_loop.md) |
