...
| --- | --- | --- | --- | --- | --- |
| A3-VENDOR-ID | `IN_PROGRESS` | 以最新智元 A3 训练 setting 作 fresh 训练物理权威；29-DoF nominal/armature、派生 action scale、startup Kp/Kd 拆键、`[0,2]` control-step delay 与六轴 push 已进 host 源码门。当前有两层 intentional block：tracked `required_identity.v1.json` 为 `awaiting_runtime_materialization`、contract SHA 为空；code-owned actual authority receipt SHA 为 `None`。唯一下一动作是先用 exact pinner + 窄 repin producer 把 bootstrap-only profile/prototype/manifest/receipt 重物化到当前 solver `146c4d6a…`，再用一次性 [`A3 vendor identity smoke`](../../DEFINITIONS.md#a3-vendor-identity-smoke) 固定 vendor task/`bh_loop_c` upper/seed0，recipe-only 后 `1 env×2` 产出 schema-3 contract；commit B 随后跟踪 exact contract/actual authority receipt，更新两处 code-owned SHA，并从该合同重物化只含 `bh_loop_c` 的 schema-v2 dynamic-ready、nominal-hold、policy/bundle/spec/claim | identity smoke 自然完成且 checkpoint/contract finite；actual authority 在 `plan` 与 internal launch 双重验证，canonical claim 含 authority；spec/authority/artifact 三方 SHA 相等；新 nominal-hold 跨 `t_hit+margin`，无 actual-hard/table/fall/nonfinite；bundle 交叉绑定 full vendor plant 与 `bh_loop_c` stable-motion；新 artifact 不与旧 N1 混用 | Pod1 首次 `plan` 暴露 manifest helper 误用并已修；第二次进入 env 构造后因旧 manifest solver `329ea0a3…` 不等于 runtime `146c4d6a…` 而 fail-closed，owned PGID 已按 sidecar `TERM`，GPU0 回空。physics 仍为 `aa5c9085…`，当前唯一 blocker 是 bootstrap artifact repin；尚无可 `plan PASS` 的 vendor diagnostic | [外部尽调](../../research/dr_reward_external_diligence_20260731.md)、[G04](../../gates/G04_sim_modeling_mujoco_isaac.md)、[G05](../../gates/G05_isaac_training_first_loop.md) |
| N1-VENDOR-POD1 | `BLOCKED` | 新身份不做科学 A/B；首轮单卡 [`N1 vendor baseline diagnostic`](../../DEFINITIONS.md#n1-vendor-baseline-diagnostic) 只允许 `bh_loop_c` 与 seed `0/1/2`，`bh_block` 机械拒绝。先按 A3-VENDOR-ID 完成两层 authority。之后严格串行 Kit boot，每卡先 `smoke=1 env×2`，再 `probe=4096×5`。若入窗拍距大部分 `>0.20 m`，转粗+细核修复；stage-evidence v4 已能消费 delay receipt，`long=4096×20001` 当前仍机械拒绝，未来另须 probe 后生成的 `vendor_probe_gate_receipt` | exact clean source/spec/claim/no-clobber；actual authority `plan`/internal 双验且 canonical claim 含 authority；spec/authority/artifact 三方 SHA 相等；smoke/probe 有真 PPO iteration、finite checkpoint、runtime ABI/std/LR/delay receipt；无 table/fall/nonfinite/raw-hard 风暴；episode 跨 `t_hit`；明确 `diagnostic_unauthorized=true` | 等 A3-VENDOR-ID commit B 解锁；当前 required identity awaiting/null + actual receipt SHA=None，尚无 vendor diagnostic plan PASS。Pod1 GPU2 旧残留已按 exact sidecar `TERM` 释放，GPU0/GPU2 可用；GPU1 缺 sidecar保持未动。formal N1 另因 action-set registry/trust/receipts 缺失而 blocked，不由本诊断绕过 | [N=1 发射工序](../../operations/run_ablation_wave_launch.md)、[设计与加速审计](../../research/design_audit_and_speedup_20260729.md) |
