...
| LIVE-CONTRACT-MATERIALIZER | `IN_PROGRESS` | **物化 epoch `d4036996…` 已提交并推送**：loop/block 的 identity/runtime/required-identity 全部切到 `_r2` fixed paths，所有新产物 SHA 与 source commit 均为 `None`，旧 loop bundle 也已撤权 | materializer 在该中间态仍 focused `38 passed`；而 launcher 测试在 collection 看到 contract SHA=None 即拒绝，这是预期 fail-closed，不是用假 SHA 修测试的理由 | **本次新 artifact 操作前已更新账本**。下一步先从 `d4036996…` 的7-source formal pinner 产生/跟踪 `_r2` profile pins，再从下一 clean commit 一次物化 loop/block identity bootstrap；产物回填前任何训练都必须拒绝 | [N=1 发射工序](../../operations/run_ablation_wave_launch.md) |
