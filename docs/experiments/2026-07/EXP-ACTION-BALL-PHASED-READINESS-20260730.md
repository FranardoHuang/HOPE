...
| LIVE-CONTRACT-MATERIALIZER | `IN_PROGRESS` | **可测工具 source `5028500c…` 已提交并推送；registry 已进入待提交物化 epoch**：loop/block 的 identity/runtime/required-identity 全部切到 `_r2` fixed paths，所有新产物 SHA 与 source commit 均为 `None`，旧 loop bundle 也已撤权 | materializer 在该中间态仍 focused `38 passed`；而 launcher 测试在 collection 看到 contract SHA=None 即拒绝，这是预期 fail-closed，不是用假 SHA 修测试的理由 | 提交并推送该 epoch，然后在 clean checkout 先重物化 loop/block identity bootstrap，再 Pod 产生 live contract/required identity；产物回填前任何训练都必须拒绝 | [N=1 发射工序](../../operations/run_ablation_wave_launch.md) |
