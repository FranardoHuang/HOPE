...
| DUAL-ENVELOPE-STRESS-HARNESS | `IN_PROGRESS` | clean v2 source=`861a7842…` 的 Pod2 v6 已得到完整 4×5-ms 同带差分：ON 最大 solver 级 Hctrl penetration=`6.06e-5 rad`、最小 Hmech gap=`1.392e-2 rad`；相同输入的 OFF 四组均在 tick2 穿过 Hmech，最大 penetration=`3.27e-4 rad`。差分 v3 已在 clean source=`dff36ad4…` 实现：schema/kind/token 全版本化，初态 q/qdot 直接取 PhysX DOF getter，qdes/ON-OFF/restore exact；旧 20-ms proxy 数值彻底退出 verdict。focused `28 passed`，两轮独立 review 无 P0/P1 | **立即在 Pod2 GPU0 用 fresh v7 no-clobber receipt 跑同一 stress**：ON 四 tick finite/qdes exact 且 strict Hmech，Hctrl penetration<完整 cage reserve；OFF tick1 进入 `[Hctrl,Hmech)` 且后续至少一 tick 触/穿 Hmech；live PhysX q0/qdot 同带 exact、restore exact、canonical PASS。PASS 后马上进入 final recipe pins | v4/v5/v6 永久 spent；v6 receipt file/log SHA=`eb93a9f…/8eb17b8c…`。GPU1 他人训练不碰；v7 使用 direct child rc/receipt status，不使用异步 wrapper rc。OFF 穿 Hmech 仅为 positive control，不授权训练或硬件 | [G05](../../gates/G05_isaac_training_first_loop.md) |
| PLANT-DUAL-POSITION-ENVELOPE | `IN_PROGRESS` | 双位置 plant、双动作 runtime→contact artifact 链、Pod `1 env×1` readback/finite 门已闭合；v6 的完整同带差分又证明：ON 留在 Hmech 内，而关闭唯一 live Hctrl 的 OFF 四组全在 tick2 穿 Hmech。只给 waist-roll/pitch 的 live PhysX constraint 每侧内缩 2% 为 `Hctrl`，其余29轴=`Hmech`，soft/Q、actor、delay、Reward 不变 | 当前不重做 plant 或放宽 guard；唯一下一动作是把已经观测到的物理因果写成 DUAL 行的差分 v3 验收并取得 fresh PASS，随后 final policy/reward pins → `4096×5 → 4096×32` | 尚缺版本正确的差分 receipt 和 final recipe/probe/push；Hmech 对 ON 仍零容忍。9% guard 仅独立 fallback，不与 cage 同臂；不引入 acceleration/jerk governor | [G05](../../gates/G05_isaac_training_first_loop.md) |
