...
| DUAL-ENVELOPE-STRESS-HARNESS | `IN_PROGRESS` | clean `07ba61cf…` 的 Pod2 GPU0 v2 已越过 VendorV1 bind、`gym.make`、reset、Hctrl/mixed readback 与唯一 5 ms `sim.step`，原构造 hang 确认修复；但无 receipt，不能判 PASS。原因是 `_run_live.finally` 先调用 `simulation_app.close()`，Isaac 在本环境 `os._exit(0)`，外层 main 永远来不及校验/发布 | v2 log 保留，末行=`sim_step_done`，shell 观察 rc0 但 no receipt；这是 publication 生命周期 bug，不是 PhysX verdict。2% Hctrl、6% guard、`[0,2]` delay、5 ms tape 均未改变 | **本次 lifecycle 修复前已更新账本**。把 receipt 的 validate/no-clobber write/flush 放到 hard-exit 之前，并补“close 不得先于 publication”的回归和独立复核；新 clean commit 用 v3 路径重跑，v2 禁止复用 | [G05](../../gates/G05_isaac_training_first_loop.md) |
