...
| --- | --- | --- | --- | --- | --- |
| DILIGENCE-0731 | `IN_PROGRESS` | 以 07-31 新尽调十二节为当前主审计，逐节把 DR/Reward、误差核、PPO、reset、吞吐、智元及 HITTER/SMASH/PACE/外部先例映射到本表。已直接吸收：智元 nominal/scale、Kp/Kd 拆键、控制步 delay、逐轴 6DoF push 幅值、finite budget、runtime normalizer/std/LR/delay 收据、入窗拍距、reset 与 contact-view 加速门；已拒绝 AMP 权重、`freeze_upper_body`、mid-swing RSI；其余只进入具名诊断或后置 canary。唯一下一动作是完成 §1–12 对抗复核并把遗漏回填本节与决策账本，不另建影子 TODO | 每个报告结论有 `adopt/defer/reject`、最迟边界与代码/运行证据；新报告和智元表均无未归类高优先项；旧 07-29 审计不再冒充本轮权威 | 分节复核正在并行；不阻塞 authority 物化与 Pod 发射 | [本轮外部尽调](../../research/dr_reward_external_diligence_20260731.md)、[上一版审计](../../research/design_audit_and_speedup_20260729.md) |
| A3-VENDOR-ID | `IN_PROGRESS` | 最新智元表已作为 fresh 物理权威写入 host：29-DoF nominal/armature/派生 scale、startup Kp/Kd、`[0,2]` 控制步 delay，以及 vendor exact per-axis 6DoF 幅值 + ActionBall `5–15 s` cadence。bootstrap repin 与 identity recipe/smoke 已在 exact source `5665963e` 自然完成；live schema-3 contract SHA=`98fa3239…1366f`，shared-ready policy contract=`27bf405e…e416`，两份 checkpoint finite，ABI/std/LR/delay 收据齐全。live smoke 揪出 authority producer 错把 controller 顺序当 USD articulation 顺序，已以 exact live 31-joint order 修复并在该提交重跑。新 dynamic-ready v2 SHA=`c831a4e6…794`、nominal-hold receipt SHA=`11c025dc…740` 已在 Pod 通过 `0.8 s/40` 步、双脚接触率 1.0、零 safety terminal；新 N=1 bundle SHA=`9881c52c…ae03` 已物化。唯一下一动作：在 clean `5665963e` 物化 actual authority receipt，更新 `required_identity` 与 launcher 两个 code-owned SHA，跟踪 contract/receipt/dynamic-ready/hold/bundle；随后在最终 clean checkout 另跑 **dynamic-ready recipe-only** 生成新 policy contract。`27bf…` 只描述 identity shared-ready，禁止复用于正式 vendor smoke | actual authority 在 `plan` 与 internal launch 双重验证，canonical claim 含 authority；spec/authority/artifact 三方 SHA 相等；dynamic-ready recipe 绑定 exact candidate/hold/bundle；新 artifact 不与旧 N1 混用；focused + Pod exact tests 通过 | intentional code pin 仍未翻转：`required_identity` 还是 awaiting/null，launcher actual receipt SHA 还是 `None`；vendor launcher 尚无 code-owned dynamic-ready recipe stage | [外部尽调](../../research/dr_reward_external_diligence_20260731.md)、[G04](../../gates/G04_sim_modeling_mujoco_isaac.md)、[G05](../../gates/G05_isaac_training_first_loop.md) |
| N1-VENDOR-POD1 | `BLOCKED` | 新身份不做科学 A/B；首轮 [`N1 vendor baseline diagnostic`](../../DEFINITIONS.md#n1-vendor-baseline-diagnostic) 只允许 `bh_loop_c`、seed `0/1/2` 与 task-owned vendor setting，禁止 argv 覆盖。A3-VENDOR-ID 解锁且 dynamic-ready recipe 物化后，先在 GPU0 跑正式 vendor `smoke=1 env×2`，再跑 `probe=4096×5`；probe 同时读取入窗拍距、actual-hard、episode age、normalizer、std/LR、delay histogram 和 update wall。若入窗拍距主体 `>0.20 m`，直接转粗+细核；若 reset/actual-hard 仍主导，先做 R0/reset 与 termination 恢复，不烧 long。`long=4096×20001` 当前仍机械拒绝，仓内还没有 probe-gate receipt producer/consumer，不能靠手写 JSON 放行 | clean source/spec/claim/no-clobber；actual authority 双验；dynamic-ready policy SHA 逐字一致；smoke/probe 有真 PPO iteration、finite checkpoint、三类 runtime receipt；无 table/fall/nonfinite/raw-hard 风暴；episode 跨 `t_hit`；明确 `diagnostic_unauthorized=true` | 等 A3-VENDOR-ID commit B + dynamic-ready recipe。Pod1 GPU0/GPU2 可用；GPU1 无 owner sidecar，保持不动。identity smoke 的 48-step 单样本曾有 1 次 waist-roll actual-hard，但新 dynamic-ready nominal-hold 为零；是否形成风暴必须由正式 4096×5 probe 判定 | [N=1 发射工序](../../operations/run_ablation_wave_launch.md)、[本轮外部尽调](../../research/dr_reward_external_diligence_20260731.md) |
