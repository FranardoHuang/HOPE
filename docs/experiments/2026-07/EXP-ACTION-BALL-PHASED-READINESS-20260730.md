...
| DUAL-ENVELOPE-STRESS-HARNESS | `IN_PROGRESS` | v2 4×5-ms source=`861a7842…` 已 clean 提交。Pod2 v5 收据=`5c6d09de…` canonical 验签一致但在 `gym.make` 前因 registered ignored USD bundle 路径缺失而 FAIL；无 env、无 restore、无机械 verdict，namespace spent。已从登记团队副本恢复完整 6-file/21,897,893-byte bundle，四层 SHA 精确为 `1b3fecd7…/8e521141…/5b5fc00b…/c76c5bdd…` | 同一 clean source 用 fresh v6 receipt 重跑；必须进入 gym/reset/4 ticks，canonical PASS、ON 全 horizon strict Hctrl、全行 strict Hmech、qdes/restore exact 才进 final recipe pins | v4/v5 永久 spent。GPU1 他人训练不碰，v6 用 GPU0；`kit_boot_lock.sh` 是异步 boot wrapper，不能用 wrapper rc 代替 child/receipt status | [G05](../../gates/G05_isaac_training_first_loop.md) |
