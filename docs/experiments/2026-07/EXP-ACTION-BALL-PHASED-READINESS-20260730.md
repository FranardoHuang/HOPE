...
| --- | --- | --- | --- | --- | --- |
| N1-DIAG-PROBE | `IN_PROGRESS` | 旧 6% containment 已由 update1/2 actual-hard 判 `REJECTED_BY_POD`；其根因修复——waist-roll/pitch `Hctrl⊂Hmech`——现已通过 v7 同带差分 stress。shared mechanical 前置不再阻塞 | 先完成下方 `RUNTIME-ASSET-LOADER-V2`，再在 final clean source 物化 loop/block policy + adaptive Reward 三个 pins；随后跑 fresh `4096×5`：checkpoint finite，nonfinite/qdes-hard/actual-hard 零容忍，std/LR/normalizer/delay 不退化，identity/recipe/completion claim 一致且中位吞吐≤8 s/update | 当前阻塞从机械 cage 转为正式 launcher 会丢 private OpenGL 的 exec-environment P0；Pod2 GPU0/2 空闲，GPU1 被另一用户 PID `152495` 占用，严禁触碰 | [本轮外部尽调](../../research/dr_reward_external_diligence_20260731.md) |
...
| N1-LONG-GATE | `BLOCKED` | producer/consumer、exactly-once natural completion 与 v7 mechanical gate 已收口；旧 6% probe 永久 `REJECTED_BY_POD`。无新 `4096×5`、无 push receipt，因此不得产生 gate receipt 或发 `4096×20001` | 新 root-cause source 上的 probe + push-evidence 同时 PASS 才能物化具名 gate receipt；receipt 绑 exact source/spec/policy/authority、自然完成、finite/ABI/std/delay、actual-hard/table/fall/nonfinite 与 push 运行证据；actual-hard/nonfinite 零容忍，手写 JSON、spent namespace 与 FAIL 运行都无法放行 | 暂阻塞于 loader-v2 Pod 验收、three-pin C0→C1、lane smoke，再到 `4096×5/×32`；不再加宽 guard，不新增 acceleration/jerk governor。Pod2 GPU0/2 空闲，GPU1 属于另一用户 | [N=1 发射工序](../../operations/run_ablation_wave_launch.md) |
| RUNTIME-ASSET-LOADER-V2 | `IN_PROGRESS` | **source 已在 clean `42277708…` 实现并独立复核**：runtime-assets schema/kind v2 同时 pin OpenGL/GLU 固定目录、library SHA、direct SONAME、USD closure 与 exact `OpenGL:GLU` string；missing/reverse/tail/tamper/旧 v1 全 fail-closed，base/identity/dynamic 三入口共享 exec 前重验 helper。四组 focused `170 passed`、py_compile/diff-check PASS、红队无 P0/P1 | Pod 验收：`ldd` 必须把 GLU 的 `libOpenGL.so.0` 解到 exact private OpenGL；正确 plan claim 必须含 v2 双库/双 SHA/exact loader path；missing/reverse/tail plan 均在 namespace 前 rc2；随后在同一绝对 `SOURCE_ROOT` 运行 C0 的 loop recipe、block recipe、adaptive Reward hash 三个 zero-PPO stage并 natural completion | 诚实 threat model 是 `pathname_sha256_revalidated_immediately_before_exec_no_concurrent_local_writers_v1`：launch 窗口 runtime tree 必须 quiescent，禁止并发维护；不宣称抵御恶意本地写者。C0 输出含 absolute dynamic-ready binding，C1 pin/smoke **不得换 checkout 路径**；boot marker/launcher accepted 不等于结束，必须等 exact PGID/GPU lock 释放 | [N=1 发射工序](../../operations/run_ablation_wave_launch.md)、[本地同步](../../operations/setup_local_sync.md) |
