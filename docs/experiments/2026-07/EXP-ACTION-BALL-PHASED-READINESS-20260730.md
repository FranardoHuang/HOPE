...
| N1-SPEED | `IN_PROGRESS` | 第一批 candidate 已完成 Pod parity：focused `134 passed`；`1 env×2` 为 `3.83/1.81 s` 且 checkpoint finite；旧/新同 seed `4096×5` 的 termination、strike、table、Reward 与题带统计逐轮相同。旧 wall=`10.09/10.48/26.63/17.16/24.85 s`，新 wall=`9.25/10.95/25.44/16.17/23.91 s`，均值只从 `17.84` 降到 `17.14 s`（约 `3.9%`），明确未收口。唯一下一动作是按残余 profile 合批 VirtualBall/command 的 per-step host barriers，并继续把 reset broker/receipt 从逐 env Python 降为 batch/device 事件 | Reward、Done、采球、solver、动作、RNG、P/A/R、task identity、qdes/brake、raw-hard/table/fall 和 checkpoint finite 必须继续等价；diagnostic 不产生 formal authority；4096-env 健康窗达到 `≥15k environment-steps/s` 或等 reset 负载 collection `≤6.5 s`。每批确定性优化只做 Pod parity/吞吐，不做学习 A/B，也不优化 PPO | 当前 exact milestone 不热补丁；第一批 candidate 可保留，但只削掉小头，不能称加速完成。formal 的 checkpoint 粒度 receipt 仍是 N5 前独立 schema 工作 | [设计与加速审计](../../research/design_audit_and_speedup_20260729.md)、[N=1 发射工序](../../operations/run_ablation_wave_launch.md) |
