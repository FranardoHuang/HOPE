...
| DYNAMIC-READY-PATH-IDENTITY | `IN_PROGRESS` | **08-01 final-pin C0 首个正确 plan 的 fail-closed blocker与采用裁定**：固定 clean source=`eef4d61e…`，loop registry 是 repo-relative motion path，tracked candidate `847ffe78…` 的 `sources.stable_motion.path` 保留旧 `/workspace/franco/a3vendor_7de10987_dynamic_loop/...`；validator 因 path 不等而拒绝。拒绝在 Kit/GPU/namespace/PPO 前，未启动训练。源码审计已证明 producer 必然把输入 `resolve()` 为绝对 provenance，而真正 runtime consumer 从当前 checkout 读 motion、按实际 bytes SHA 交叉验证，并把当前 candidate/hold 的 canonical absolute path + file/content SHA 写入 runtime/policy binding；因此“重物化后继续与 repo-relative 字符串逐字相等”仍不可能成立。**采纳 path-portable logical identity**：candidate 的 absolute provenance 必须以完整 registry relative path 为 suffix，当前 commit/worktree 的该 relative tracked blob 必须 exact SHA；保留 artifact/hold/bundle absolute runtime binding。拒绝只比 basename、删除 path 检查或用旧绝对 source path作 runtime 输入 | action/frame0、motion SHA、candidate/hold/bundle SHA、runtime-contract SHA、source commit 与 current tracked blob 必须全部继续 fail-closed；cross-action、同 basename 异目录、错误完整 suffix、错误 SHA、untracked/worktree/commit drift、旧 schema/篡改负例通过；repo 内真实 loop/block r2 正例通过；正确 `plan` 能在**不创建 namespace**的 plan 阶段形成 v2 loader+dynamic binding claim。该修复不重物化 candidate/hold/bundle，故 C0 policy pin 尚未产生、不会被“沿用旧 pin” | 主线实现与 focused tests 可立即并行；独立 subagent 仍作对抗复核，若发现 absolute provenance 被任何 runtime consumer 解引用则回退到新 schema 重物化，不以赶车压过证据 | [N=1 发射工序](../../operations/run_ablation_wave_launch.md)、[G05](../../gates/G05_isaac_training_first_loop.md) |
