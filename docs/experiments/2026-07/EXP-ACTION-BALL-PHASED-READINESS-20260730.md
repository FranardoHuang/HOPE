...
| LIVE-CONTRACT-MATERIALIZER | `IN_PROGRESS` | **identity 工件+registry+consumer 已在 `63e08ea3…` 提交并推送**；loop/block 共用 identity source `a2882d68…`，A3 vendor 六组工件链 host `115 passed` | 新 profile/solver/producer/action registry 闭包已通；contract/required-identity 仍为 `None`，所有训练路径仍 fail-closed | **本次 Pod runtime 物化前已先更新账本**。从下一 clean docs commit 建立 Pod2 fresh checkout，两动作分别跑 identity recipe/smoke，然后原子物化 live schema-3 contract + 12 组/31 关节 required identity；任一 FAIL 不回填 SHA | [N=1 发射工序](../../operations/run_ablation_wave_launch.md) |
...
| DUAL-ENVELOPE-STRESS-HARNESS | `IN_PROGRESS` | **source + host 验收已完成**：独立 8-env/1-physics-tick 工具用同 tape 覆盖 waist-roll/pitch × lower/upper × Hctrl ON/OFF；task 已 code-owned 钉死 `HOPE-PingPong-ActionBall-AgibotA3-v0`，operator 不能换任务借收据 | stress host `13 passed`，A3 vendor 全工件链 `115 passed`；exact `dt=5 ms`，q0 在 Hctrl 内 `0.1R`、动力学点在 Hctrl 外 `0.6R`/Hmech 内 `0.4R`；恢复 Hctrl exact readback 是 PASS 必要条件 | **本次 Pod stress 前已先更新账本**。与双动作 identity smoke 并行用 Pod2 空闲 GPU 运行真实 PhysX `set_dof_limits + 1×5 ms`；host 不能代替，FAIL 不放宽容差 | [G05](../../gates/G05_isaac_training_first_loop.md) |
