...
| LIVE-CONTRACT-MATERIALIZER | `IN_PROGRESS` | **loop/block identity bootstrap 已从同一 clean source `a2882d68…` 物化并回填 registry**；loop prototype/manifest/receipt SHA=`1fe96882…/74c1a1a3…/ec13d8a3…`，block=`e53a509b…/6e2f7b15…/b29c640b…`，producer=`b90bac5f…` | 旧 fixture/consumer 已改钉 `df7fe0f0…` profile 与 `af4f6f95…` solver；A3 vendor 六组工件链 host `115 passed`，包括真实跟踪工件闭包、双 action 共存与 cross-action/source/profile/producer 拒绝 | 提交/推送 identity 工件+registry+consumer；然后**进入 Pod runtime-contract/required-identity 前再更新账本**。contract/required-identity 仍为 `None`，训练仍必须拒绝 | [N=1 发射工序](../../operations/run_ablation_wave_launch.md) |
...
| DUAL-ENVELOPE-STRESS-HARNESS | `IN_PROGRESS` | **source + host 验收已完成**：独立 8-env/1-physics-tick 工具用同 tape 覆盖 waist-roll/pitch × lower/upper × Hctrl ON/OFF；task 已 code-owned 钉死 `HOPE-PingPong-ActionBall-AgibotA3-v0`，operator 不能换任务借收据 | stress host `13 passed`，红队整合共 `113 passed`；exact `dt=5 ms`，q0 在 Hctrl 内 `0.1R`、动力学点在 Hctrl 外 `0.6R`/Hmech 内 `0.4R`；恢复 Hctrl exact readback 是 PASS 必要条件 | 唯一下一动作：统一 clean source 上用 Pod2 GPU 运行真实 PhysX `set_dof_limits + 1×5 ms`；host 不能代替，FAIL 不放宽容差 | [G05](../../gates/G05_isaac_training_first_loop.md) |
