...
| N1-DIAG-PROBE | `IN_PROGRESS` | vendor diagnostic smoke claim=`be783ab7…ad54` 已自然完成 `1 env×2`；model0/1 均 83 tensors 且 finite，ABI/delay/std-LR marker=`1/1/2`，无 Traceback/table/fall/qdes-hard/nonfinite，GPU0 回到 18 MiB。update1 有1次 waist-roll actual-hard（episode age=25），48 步内 zero strike 因预算小于 `t_hit_min`而不判核。唯一下一动作是生成并 `plan` 同 policy/authority/bundle 的 `4096 env×5` spec，plan PASS 后在 GPU0 发射 | 5 份 checkpoint finite；episode 可跨 `t_hit`；读取入窗拍距、actual-hard、episode age、normalizer、std/LR、delay histogram 和 update wall；无 table/fall/nonfinite/raw-hard 风暴 | 入窗距离主体 `>0.20 m` 则转粗+细核；reset/actual-hard 主导则先 R0/R1。GPU1 无 owner sidecar 保持不动 | [本轮外部尽调](../../research/dr_reward_external_diligence_20260731.md) |
