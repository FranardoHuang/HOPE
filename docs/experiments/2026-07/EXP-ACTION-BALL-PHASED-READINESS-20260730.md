...
| DUAL-ENVELOPE-STRESS-HARNESS | `IN_PROGRESS` | clean v2 source=`861a7842…` 的 Pod2 v6 已得到完整 4×5-ms 同带差分轨迹、restore/qdes exact 和 canonical 可验签 FAIL receipt=`da977d6…`。FAIL 只说明旧 validator 的“ON 每 tick strict Hctrl”假设错误：ON 最大 solver 级 Hctrl penetration=`6.06e-5 rad`，但最小 Hmech gap 仍=`1.392e-2 rad`；相同 q0/qdot/qdes 的 OFF 四组均在 tick2 穿过 Hmech，最大 penetration=`3.27e-4 rad`。这已经正向证明 Hctrl cage 保护 Hmech，不是 plant 失败 | **先版本化差分 v3、再用 fresh receipt 重跑**：ON 四 tick 必须 finite/qdes exact 且 strict Hmech，允许的 Hctrl solver penetration 必须小于该侧完整 cage reserve；OFF tick1 必须进入 `[Hctrl,Hmech)` 且后续至少一 tick 触/穿 Hmech，作为同带 positive control；两组初态/速度/qdes 必须一致、restore exact。不得继续用 strict-Hctrl 误杀，也不得把 OFF 穿 Hmech 写成安全放行 | v4/v5/v6 永久 spent；v6 receipt file SHA=`eb93a9f…`、log SHA=`8eb17b8c…`。GPU1 他人训练不碰；新 schema/kind/confirm token 必须与 v2 隔离，旧 20-ms proxy 继续仅作 telemetry | [G05](../../gates/G05_isaac_training_first_loop.md) |
