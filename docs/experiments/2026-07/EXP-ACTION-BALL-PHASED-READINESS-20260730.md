...
| LIVE-CONTRACT-MATERIALIZER | `IN_PROGRESS` | **source 已实现并复核**：fixed-path 双输出原子 materializer 由 live `training_contract` 安装 contract 并派生 12 组/31 显式关节 `required_identity`；输出只取 action registry，禁止 operator 手填 SHA、原地覆盖或半发布 | focused `38 passed`，launcher/materializer/authority 整合 `107 passed`；已覆盖 exact deploy 归一化、tamper、错 source/动作/顺序、`O_EXCL/O_NOFOLLOW`、partial-write rollback；不改训练数值 | 唯一下一动作：先提交可测工具 source，再把 registry 切到 `_r2` fixed paths + `sha256=None` 的明确物化 epoch，Pod 同时物化 loop/block 后回填 | [N=1 发射工序](../../operations/run_ablation_wave_launch.md) |
...
| DUAL-ENVELOPE-STRESS-HARNESS | `IN_PROGRESS` | **source + host 验收已完成**：独立 8-env/1-physics-tick 工具用同 tape 覆盖 waist-roll/pitch × lower/upper × Hctrl ON/OFF；开机前/发布前复验 clean HEAD，FAIL 也只能 no-clobber 发布 | host `12 passed` + pycompile/diff-check；exact `dt=5 ms`，q0 在 Hctrl 内 `0.1R`、动力学点在 Hctrl 外 `0.6R`/Hmech 内 `0.4R`；恢复 Hctrl exact readback 是 PASS 必要条件 | 唯一下一动作：统一 clean source 上用 Pod2 GPU 运行真实 PhysX 压测；若 ON/OFF 不分离或 Hmech 触边就停，不放宽容差 | [G05](../../gates/G05_isaac_training_first_loop.md) |
