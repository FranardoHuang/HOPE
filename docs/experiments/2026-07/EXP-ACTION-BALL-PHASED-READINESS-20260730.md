...
| --- | --- | --- | --- | --- | --- |
| N1-RUN | `BLOCKED` | exact recipe `165645f5…bd9` 已物化，fresh r4 smoke/probe/long spec 已生成；唯一下一动作是提交/发布工件，等待 launcher watchdog 对 r3 做 exact-PG 自然收口与全 Pod Kit boot lock 释放，再 canonical plan/launch r4 `1 env × 2 updates` | fresh smoke 有真实 PPO update 与 finite checkpoint；teacher-start 首帧等于完整 wait、每 tick 减一个 `policy_dt` 且不为负；随后 probe 跨 `t_hit` 且无 NaN/identity 漂移/持续 table、fall、raw-hard 爆炸，通过即发 long | r3 PID `1127383` 与 wrapper 自然退出（不得手工 signal/kill）；全 Pod Kit boot lock 和自然空闲 GPU 槽 | [N=1 发射工序](../../operations/run_ablation_wave_launch.md)、[结果判读](../../operations/read_and_report_results.md)、[G05](../../gates/G05_isaac_training_first_loop.md) |
