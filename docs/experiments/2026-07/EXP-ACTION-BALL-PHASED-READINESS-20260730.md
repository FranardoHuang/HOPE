...
| N1-TONIGHT-3LANE | `IN_PROGRESS` | **07-31 今晚发射裁定**：不等“传统 baseline 跑完”，共用 hard gate 放行后尽快发 Pod2 GPU0/1/2 三条 fresh `4096×20001`。主臂 A 是单动作 N=1 `bh_loop_c`，主臂 B 是单动作 N=1 `bh_block`，两者使用同一最强 vendor nominal/delay/push/Reward/observation/safety 配方；第三臂 C 是较难的 `bh_loop_c` adaptive-sigma **`IN_PROGRESS` profile**。仅打开 adaptive 三旗会从 live minimum `0.075/0.5/0.262`（position/velocity/normal）起步，`min(current, EMA)` 将永久 no-op，故该伪 canary 已拒绝。真正 C 必须由 code-owned profile 从 maximum `0.20/1.0/0.52` 起步，让 additive 与 `racket_strike_success` 的 position/velocity/normal 三参数同值锁步，再单调收紧到 `0.075/0.5/0.262`；共同 coarse position 仍为 `0.30`，权重/DR/reset/seed 全不变。最大初态是 adaptive schedule 本身必需的初始条件，不是额外第二变量。三条都不是 N=2 policy，**禁止恢复 N 维 `action_one_hot`** | A/B 各自的 action-specific candidate/hold/bundle 和 common probe/push hard gate PASS；C 须共用 A 的 action/source/asset/spec，launcher/compose 必须物化并校验 `0.20/1.0/0.52 → 0.075/0.5/0.262` 双 term 三参数锁步合同。C 只能 fresh 发射、禁止 resume；须有自然完成的 Pod 构造/运行证据，不用其他随机变量混出不可解释比较 | 6% 身份/dynamic smoke 的机械链 PASS，但 GPU0 probe 仅得 infrastructure hang，GPU2 push 已用 update1/2 actual-hard 将 6% 判为 `REJECTED_BY_POD`；common hard gate 未放行，C 不发。08-01 已确认 Pod2 GPU0/1/2 均空闲，三 lane 无卡位阻塞 | [N=1 发射工序](../../operations/run_ablation_wave_launch.md)、[观测合同](../../interfaces/policy_observation_action.md) |
| N1-LONG-GATE | `BLOCKED` | producer/consumer 与 exactly-once 自然完成 marker 的 source 门已收口；6% successor 的 identity/dynamic smoke 只证明机械链可运行。GPU0 claim=`ca2e6423…` 未进 update0 即 infrastructure hang 并已 spent；GPU2 claim=`55fd8bd…` 则用 update1/2 actual-hard 将 6% 明确判为 `REJECTED_BY_POD`。无 probe PASS、无 push receipt，因此不得产生 gate receipt 或发 `4096×20001` | 新 root-cause source 上的 probe + push-evidence 同时 PASS 才能物化具名 gate receipt；receipt 绑 exact source/spec/policy/authority、自然完成、finite/ABI/std/delay、actual-hard/table/fall/nonfinite 与 push 运行证据；actual-hard/nonfinite 零容忍，手写 JSON、spent namespace 与 FAIL 运行都无法放行 | 暂阻塞于 actual-q/implicit-PD/qdes-projection 根因定位与新 source 复验；不再加宽 guard，不新增 acceleration/jerk governor。Pod2 GPU0/1/2 均空闲，但空闲不改变 gate 的 fail-closed 结论 | [N=1 发射工序](../../operations/run_ablation_wave_launch.md) |
| LIVE-CONTRACT-MATERIALIZER | `READY` | **ADOPT source-only feature 前置**：增加 live `training_contract` + `required_identity` 的 canonical no-clobber materializer，由实际 runtime contract/authority 物化 code-owned pins；禁止 operator 手填 SHA、原地覆盖旧工件或只改其中一份 | 同一输入 bytes 产生唯一 canonical payload/SHA；旧路径存在时 no-clobber 拒绝；contract/authority/required-identity 三方交叉 pin；tamper、错 source/动作/顺序与 partial write 负例 fail-closed；不修改训练数值 | 无外部输入、无 Pod 依赖；必须在下一次 loop/block 身份 DAG 重物化前完成，本轮只入账、不实现 | [N=1 发射工序](../../operations/run_ablation_wave_launch.md) |
| VENDOR-DIAG-TEMPLATE | `READY` | **ADOPT source-only feature 前置**：为 vendor diagnostic canonical launcher 增加 `template` 子命令，从 code-owned action/profile/lane 生成 canonical smoke/probe/long spec 模板，禁止为三 lane 手写 JSON spec | `template`→canonicalize→`plan` 在同一输入下字节稳定；A/B/C 的 action/source/asset/policy/authority/bundle/Reward/sigma/lane 字段齐全；未物化 pin、错 GPU UUID、错 adaptive profile 与 namespace 冲突均在 Kit/PPO 前拒绝 | 无外部输入、无 Pod 依赖；必须在三 lane 正式 spec 冻结前完成，本轮只入账、不实现 | [N=1 发射工序](../../operations/run_ablation_wave_launch.md) |
| PLANT-DUAL-POSITION-ENVELOPE | `IN_PROGRESS` | source commit `1e44be7b…` 已实现：URDF `H_mech` 继续作 raw terminal/ledger 真值，只给 waist-roll/pitch 的 live PhysX constraint 每侧内缩 hard span 2% 为 `H_ctrl`，其余29轴=`H_mech`；soft/Q、actor、delay、Reward 均不变。host `364 passed`；最终 source 的 Pod `1 env×1` 自然完成，startup/首 reset readback SHA 同值，`120` sample 的 actual-hard/terminal 与全部 cage proxy 为零 | 下一唯一动作是把该提交与 nominal 反转、block/adaptive source 合并；之后重签 plant contract，跑两腰×双侧5ms ON/OFF stress 与 fresh `4096×5`，要求 mechanical gap>0、actual-hard/nonfinite全零，并记 attempt/near/penetration/capture-proxy/dwell/side-flip/Δqdot | 尚缺统一 clean source、stress、identity/authority/recipe 与 4096 probe；9% guard 仅独立 fallback，不与 cage 同臂；不引入 acceleration/jerk governor | [G05](../../gates/G05_isaac_training_first_loop.md) |
