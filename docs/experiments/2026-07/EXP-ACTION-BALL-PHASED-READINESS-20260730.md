...
首个 194-D N1 保持最小可学习合同，不在没有真实 dropout 分布时追加常量 freshness 列。最终
...
宽度将由 194 变为 196，必须使用新 contract 名并重钉 Python/C++/MuJoCo producer；不能在
194-D 名称下静默追加。是否再加短 history 仍由实测延迟/别名 canary 决定。
...
| 首条 milestone1000 | Pod1 GPU1 已接受反手挡 `4096×1001`，fresh namespace `n1hr_milestone1000_f2c54fc3_block_gpu1_r1`，claim `8dc4dcb2…ff080`；2026-07-30 快照已到 update 81，`model_0.pt` 共 80 个 tensor、全部 finite；mean episode `67.17` steps、`1176` 个 strike opportunity、`1183/1538` swing completion，qdes forbidden `0` | swing completion 已到 `76.92%`，但 actual-hard 仍有 `1420`、table/fall 为 `79/2`，virtual capture 仍为 `0`；exact strike hit `3.45%`、位置误差约 `0.242 m`，按 update100/300/1000 判趋势，反手拉同一卡自然排在其后 |
...
3. **分段 profiler 与 all-mode 热路径。** 分开 physics rollout、metrics/D2H、termination、
   ...
   不写成精确归因；
4. **generic formal launcher 余项。** 任意 N launcher 对 194-D family 的 name/width 识别已完成；
...
1. C++/ONNX/MuJoCo 全部支持最终版本化 ordered terms、normalization、metadata 和 stale/dropout
   语义；若采用 `localization_age/valid`，必须使用新名称/宽度，不能伪装成 194-D；
