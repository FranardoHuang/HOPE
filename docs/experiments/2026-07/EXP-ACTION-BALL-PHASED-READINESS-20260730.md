...
| --- | --- | --- | --- | --- | --- |
| N1-DIAG-PROBE | `IN_PROGRESS` | **07-31 当前点**：source/artifact/pin 谱系已推送；Pod1 GPU0 的 identity-only recipe + smoke 、新 runtime authority、dynamic-ready candidate、nominal-hold 与 r3 bundle 均 PASS，vendor live contract=`38974f1b…68fba` 未漂移。第一条 replacement smoke 在零 PPO update 前 fail-closed：配置 SHA=`9fbc61ad…`、实际=`f76df202…`；根因是 recipe wrapper 错用 r2/v3 bundle，而 smoke 正确绑 r3/v4。已在 source=`469d5e56…` 将 r3 bundle 升为 vendor launcher 唯一 code-owned pin，recipe wrapper 直接引用，stale bundle 在 compose 阶段即拒绝；host 回归=`49/79 passed`。Pod fresh canonical recipe 已精确产生 `f76df202…`，证明 recipe/smoke 身份已一致。**第二条 smoke 暴露了下一个纯门控缺口**：在 PPO 前构造 natural-completion payload 时，trainer 取不到 launcher 已验证的 claim SHA，因 `training_launch_claim_sha256=None` fail-closed；仍为零 update/零 checkpoint，exact-owned PGID 已 TERM、GPU0 已释放。下一修复是由已验证的 internal launcher 通过受控 env 把 claim SHA 传给 completion marker，不把 claim 自引用地塞进 scientific argv，不改 PPO/Reward/DR。旧 `89082b7c…079` probe 仍因 actual-hard=`4873`、table=`2025` FAIL，不可放行 long。新 `max_inward_until_nonoutward_v1` 是 emergency containment，不是 nominal acceleration governor；raw actor、delay queue、Reward 源、actual-hard terminal 不变，exact-resume 已覆盖 `delay OR containment` | checkpoint finite；nonfinite/qdes-hard/actual-hard **零容忍**；预测 crossing 只记 containment；table/fall 用有界率 + mean episode age + strike/swing>0 门；std/LR/normalizer/delay 不退化；r3/v4 canonical recipe-only 与 smoke 的 policy SHA 必须一致，completion 又必须绑 exact stage/vendor contract/claim | 当前闭合 internal-launcher→trainer completion claim 传递后，fresh smoke→GPU0 `4096×5` probe。GPU1 所有权不明的旧进程不动 | [本轮外部尽调](../../research/dr_reward_external_diligence_20260731.md) |
...
| 区间 | 项 | 维度 | 语义 |
| --- | --- | ---: | --- |
| `[0,62)` | `command` | 62 | 当前 teacher/reference phase 的 31 关节位置 + 31 关节速度；pre-swing wait 期间固定在 ready frame 0 |
...
| **小初始探索的 PPO / 模仿论文组**（报告§12.C 原论文与开源配置核查） | 外部栈多以 `σ=1.0` 且不清零 actor head 起步；没有直接分析 adaptive-KL 与我们 `σ=0.02` zero-head 的交互 | `RETAIN` 我们的 0.02 和运行时 std/LR 收据；`REJECT` 盲抄 1.0；`DEFER` log-std/熟化改动 | 新 probe 继续强制 std finite/positive 与 LR 轨迹；只有 floor-lock 或穿零证据才重开 noise parameterization |
...
| 三处 legacy 配置真值清理 | **ADOPT / IMPLEMENTED** | 已完成 | 否；post-compose 已对账 | PD 注释/实际 startup 范围、Hitter IdealPD 旧 rationale 与 implicit-A3 名义 torque reward 均已改成组装真值；详细证据在§2/G05/PROGRESS |
...
| 现有 action smoothing + q/qdes/projection 安全链 | **RETAIN / 不降级** | 当前 | 只在抖动量尺异常时调当前剂量 | 保留 active action-rate/二阶差分平滑项与三条限位分账：qdes `-40` 读 processed/executed 目标，actual-q `-40` 读真实关节，pre-clamp 请求超出由 projection-distance `-5` 计价；不把三者混称为 pre-clamp barrier。bang-bang 当前只调这类 action-change penalty，不叠新 governor |
...
| 智元 joint-zero + proprio noise | **RETAIN（已对齐）** | 当前 | 否；做 compose/runtime receipt | joint default offset `±0.01 rad`与 action offset 同源；`ang_vel ±0.2 / gravity ±0.05 / q ±0.01 / qdot ±0.5` 已与厂商一致，不为“学新东西”重复改 |
