...
| DUAL-ENVELOPE-STRESS-HARNESS | `IN_PROGRESS` | clean `9a08d979…` 的 Pod2 GPU0 v3 已产生真实 FAIL receipt `ec1eebc…`，证明 lifecycle 修复有效；FAIL 为 telemetry 计数预期 `2/1/1`、实测 `3/1/1`，不是 mechanical penetration verdict。三次来源是手工 pre、正常 sim hook、手工 post；不得把阈值直接改 3 制造 PASS | v3 receipt/log spent；同时暴露 FAIL receipt 在 validate 异常后丢失 finally 已完成 restore/raw rows，错误写成默认 `restore.attempted=false`。2% Hctrl、6% guard、`[0,2]` delay、5 ms plant 均未改变 | **本次 telemetry/FAIL-evidence 修复前已更新账本**。先核对 action-term 正常 hook，删除重复采样而非放宽判据；让 validation FAIL 仍携带真实 restore、raw observations/diagnostic。新 clean source 用 v4 路径重跑，v3 不复用 | [G05](../../gates/G05_isaac_training_first_loop.md) |
