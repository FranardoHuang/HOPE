...
| LIVE-CONTRACT-MATERIALIZER | `IN_PROGRESS` | **runtime→authority→dynamic-ready→nominal-hold→contact-bundle 的双动作链已真跑通**：runtime/authority pins 在 `4af953c6…`；Pod2 用 `7de10987…` 同源 clean worktree 生成 candidate，loop/block 都完成 `0.8 s / 160 physics / 40 policy` nominal-hold PASS，双脚接触率 1.0、无 terminal、plant/delay 合同闭合 | loop candidate/hold/bundle SHA=`847ffe78…/22010d5d…/bd91b652…`，block=`3692f4c3…/49948c41…/bbfb612a…`；bundle 的 landing_claim=false，solver/physics pins exact。loop 前两次只在 Kit 启动环境缺 `libOpenGL.so.0` 和 worktree `PYTHONPATH` 时失败，无 scene/无 receipt；retry3 修正环境后自然 rc=0 | **本次十二工件集成前已更新账本**。审核并分动作提交 candidate+hold+四件 bundle，合并后回填 registry contact-bundle SHA；这只关闭 action-specific artifact 链，shared stress/`4096×5→×32` 未过前 long 仍 fail-closed | [N=1 发射工序](../../operations/run_ablation_wave_launch.md) |
...
| DUAL-ENVELOPE-STRESS-HARNESS | `IN_PROGRESS` | Pod2 GPU0 exact `ddb3fd0c…` 的 hang 已定位在 `gym.make` 的 manager/command 构造阶段，尚未到 `env.reset`、limit 写入或唯一 `sim.step`，因而仍非 PhysX FAIL。同时审计确认 harness 直接 `parse_env_cfg` raw Gym class，绕过 long 使用的 `HOPEPingPongActionBallA3VendorV1` Hydra profile；action-ball target/canonical-ready/manifest 与 vendor guard/Hctrl/delay 都未完整同源 | 这个入口分叉无论是否是 hang 唯一原因都必须修；旧 log 及 bounded 探针路径全部 spent，不作科学证据 | **本次 harness 修复前已更新账本**。改为与 long 同源 VendorV1 compose，显式关 debug visualization，补 `gym.make/reset/readback/sim.step` 阶段 marker 与回归；修复后必须新 clean commit+新 no-clobber Pod receipt。2% Hctrl、5 ms、ON/OFF 内外关系、qdes=q0 及 finally exact restore 全不放宽，不加 acceleration/jerk governor | [G05](../../gates/G05_isaac_training_first_loop.md) |
