...
| --- | --- | --- | --- | --- | --- |
| N1-RUN | `IN_PROGRESS` | Pod1 三卡现在各有一条有用但互不重标的 `4096×1001` diagnostic long：GPU0 是 exact source `6557390f` 的反手挡 1.1 倍来球 seed1（claim `9fafe753…e8c`）；GPU1 是 exact source `bd340479` 的反手拉原中心来球 seed0，已依次通过 recipe、`1 env×2` smoke 与 `4096×5` probe 后 fresh 发射；GPU2 继续只读守护旧 exact source `8729104e` 的反手挡 seed0，已越过 update600。三条都已进入真实 PPO，不互相 resume，也不因 source 不同合并报数。唯一下一动作是按各自自然 `100/300/600/1000` milestone 守护 finite、teacher fidelity、strike/capture/return、Reward income 与 unsafe 分账 | 三条 exact PID/PGID/cwd/GPU UUID/spec/claim 持续匹配，checkpoint finite；按动作与 seed 分开报告；NaN、身份漂移、Traceback 或 counter invariant 破坏才按预注册停止规则处理 | 无外部阻塞；运行中的 exact source 不热补、不 signal/kill。tracked smoke/probe/milestone spec 见 `configs/n1_speed_hotpath_launch_20260730/` | [N=1 发射工序](../../operations/run_ablation_wave_launch.md)、[结果判读](../../operations/read_and_report_results.md)、[G05](../../gates/G05_isaac_training_first_loop.md) |
| N1-SPEED | `IN_PROGRESS` | 第三批 candidate 已完成四刀确定性优化：diagnostic birth callback 整批化（`d2ec91e9`）、joint-safety update aggregate 收敛（`5f85cc58`）、broker/pool diagnostic proof 裁剪（`dbb7ce04`）及 metric/validation D2H 合包（`60a8e219`）。最后一刀把已知普通 step host barriers `5→1`、strike step 约 `15→2`；formal 同步 fail-fast、Reward/Done/solver/RNG 不改。Pod metric focused `126 passed`，joint ledger `88 passed`，runtime/wiring/highwater 分组回归均通过；新 profile/base/strict 1.1× bundle 已在 exact source 上重物化并以 config commit `056625be` 入库，fast-ball bundle SHA=`81dee53f…0351`，固定 tape 仍为 `2763/4096` admitted。唯一下一动作是 GPU2 自然空闲后在 exact clean checkout 做 recipe-only → `1 env×2` → same-seed `4096×5`，比较完整 update JSON、finite checkpoint 与 wall；未测前不宣称加速收口 | Reward、Done、采球、solver、动作、RNG、P/A/R、task identity、qdes/brake、raw-hard/table/fall 和 checkpoint finite 必须继续等价；CUDA async invariant 另做坏谓词 fail-closed 负控；4096-env 健康窗稳定达到 `≥15k environment-steps/s` 或等 reset 负载 collection `≤6.5 s` | 当前三条 long 不热补。新 source 改过 `hope_commands.py`，故旧 bundle/spec/claim 全部作废；已完成 profile/bundle 重钉，policy recipe/spec/claim 等自然空闲槽物化。formal checkpoint 粒度 receipt 仍是 N5 前独立 schema 工作，不由 diagnostic fast path 代替 | [设计与加速审计](../../research/design_audit_and_speedup_20260729.md)、[N=1 发射工序](../../operations/run_ablation_wave_launch.md) |

### 0.3 Next — long 已运行后的判读与 formal N=5 前置

| ID | 状态 | 当前交付 / 触发条件 | 完成验收 | 阻塞输入 | 证据入口 |
| --- | --- | --- | --- | --- | --- |
| N1-REVIEW | `IN_PROGRESS` | 三条 milestone1000 分别按尚未越过的下一个自然里程碑复核；GPU2 旧反手挡继续到 1000，GPU0 1.1 倍来球反手挡与 GPU1 反手拉按 100/300/1000 判读。不得把 GPU1 probe 的 `6.816 s/update` 当学习结论，也不按一次均值临时改 Reward/课程 | finite/exact identity 持续；动作与 teacher 偏差可解释；strike opportunity 非零；capture/return、table/fall/hard、Reward income 与 curriculum failure 分账；各自 1000 结论进入附录 | 运行数据正在生成；若需动作语义/视频晋级，届时由 Franco 人工裁定 | [结果判读](../../operations/read_and_report_results.md)、[G05](../../gates/G05_isaac_training_first_loop.md) |
...
| 第三批 update-wall candidate（Pod trainer 待验） | commits `d2ec91e9 / 5f85cc58 / dbb7ce04 / 60a8e219` 分别整批化 diagnostic birth callback、裁掉 diagnostic safety/receipt 冗余、收敛 broker/pool proof，并把 ordinary/strike metric host barriers 约从 `5/15` 降到 `1/2`。Pod metric focused=`126 passed`、joint ledger=`88 passed`；新 profile raw SHA=`2c1c91c…9b2c`、base bundle=`d28a5b12…4246`、strict 1.1× bundle=`81dee53f…0351`，config source=`056625be` | 只完成 source/parity/fresh artifact 门；CUDA 坏谓词、recipe-only、`1×2` 和 same-seed `4096×5` 尚未运行，因此没有 wall 改善声明。旧 active long 不热补，旧 spec/claim 不复用 |
| P0a 反手拉 probe 与三卡 long | exact source `bd340479` 的反手拉原中心来球先完成 recipe、`1 env×2` 和 `4096×5`；probe wall=`2.90/2.87/12.51/5.97/9.83 s`，均值 `6.816 s/update`，五份 checkpoint finite。strike opportunity=`0/0/921/0/1`，table=`0/0/15/77/22`，fall=`0/0/0/0/6`，actual-hard=`0/264/3111/803/2059`。随后 GPU1 fresh milestone1000 已进入真实 PPO；GPU0 同时运行 `6557390f` strict 1.1 倍反手挡 seed1，GPU2 继续旧 `8729104e` 反手挡 seed0 | 三卡均产出不同动作、来球或 seed 证据；probe 的速度与前五轮计数只证明构造/吞吐/击球窗，不是学习晋级。当前 long exact source 均不热补 |
...
| explicit table-contact v3 与固定税定价 | 旧的 32 sensor × 5 filter 方向会触发 pinned PhysX 的 filter backend 警告，单个 Robot wildcard 又不能代表 32 个 filter slot，均已拒绝。当前 source `f068555b` 改为 5 个 table-source sensor × 32 个显式有序 A3 body filter；Pod focused `214 passed`，1-env 真实 top/keepout/net/two-post 正控覆盖 wrist/elbow/ankle 与四个 physics substep，五 probe reset 后零泄漏，unsupported/did-not-match/FAIL/Traceback 均为 0，出现 `main_completed` 且 shell rc=0；注入失败另以 source `2d9997a6` 证明 shell rc=1，Kit teardown 不再伪装 PASS。source `2bddb440` 的两次 4096-env 短稳态 A/B：table on=`67.577/72.000 ms/step`，off=`60.233/61.400 ms/step`，平均固定税约 `8.97 ms/step` 或 `0.22 s/24-step update` | exact pair identity 与训练 fail-safe 语义成立；table 不是当前 17–25 秒 update 的主瓶颈，故保留精确后端。用户提出的 table-frame conservative geometry prism 已完成只读设计，仅在后端再次失效或后续实测税显著放大时启用，不在当前关键路径切换 |
