...
| N1-SPEED | `IN_PROGRESS` | diagnostic compact receipt、batch sampler、update-boundary telemetry、pose-OBB v4 与 host-only solver result 已依次落地。最新 exact source `7f77ae5c` 的 same-seed `4096×5` wall=`2.864/2.753/11.292/6.341/10.249 s`，均值 `6.700 s/update`、约 `14.67k environment-steps/s`；相对旧 `12.341 s/update` 累计快 `45.7%`，相对 pose-OBB `7.258 s/update` 再快 `7.7%`。五份 checkpoint 均 80 tensor/76 浮点且 finite，joint-safety 与 exact-behavior JSON 和 pose-OBB 基线逐轮相等；reset=`0/267/3110/884/2085`。最新一刀保留同一公式初值、12 次 LM、authoritative replay 与 host packet，只不再构造 caller 不消费的 public result/ledger，并合并五次 H2D | Pod focused `104 passed`；packet/reason float32-bit parity、恶意字段宽度、反序 quaternion 关联、三组 update JSON 与 finite checkpoint 全过。当前距 `6.5 s / 15k` 健康线只差约 `0.20 s/update`；下一唯一高杠杆是 fixed-tape `cq_n_iters=4/6/8/12` 数值 canary，先检查残差/admit/reason/task 再选最小充分迭代数，不把学习 A/B 当数值验收 | source=`7f77ae5c`，policy contract=`b150a395…3703`，bundle=`c5d8a01f…74b9d`，smoke/probe claim=`72cc4bb9…a70d` / `7c8f7554…94f63`。formal checkpoint-granularity receipt 与 pose-OBB v4 consumer 仍在 N5 前独立完成；不阻塞当前 N1 diagnostic | [设计与加速审计](../../research/design_audit_and_speedup_20260729.md)、[N=1 发射工序](../../operations/run_ablation_wave_launch.md) |
...
| N5-TABLE | `LATER` | formal N=5 前让 `canonical_motion_admission.py`、`action_ball_stage_evidence.py` 与 generic curriculum launcher 消费 pose-OBB v4，并把 path-free runtime USD tree/content identity 写入 hard contract | producer/consumer schema 一致；真实五组件/四子步正负控、nonfinite、reset 零泄漏、formal admission 与 exact-resume identity 全过；不得回退到 pinned backend 不支持的 pair-filter matrix | 不阻塞 N=1 diagnostic；formal consumer 迁移尚未实现 | [table safety smoke](../../operations/run_action_ball_table_safety_smoke.md)、[G05](../../gates/G05_isaac_training_first_loop.md) |
...
| table pose-OBB truth | 直接修 | N=1 long；formal consumer 在 N=5 前 | 否；做 Pod 正负控/吞吐 | diagnostic 已采用 43 collision-component + blade OBB、五桌体 AABB、四子步 sticky；filtered v3 与 sphere 中间态退役。formal consumer/schema 与 path-free USD identity仍开放 |
