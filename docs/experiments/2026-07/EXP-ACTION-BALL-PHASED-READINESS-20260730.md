...
| --- | --- | --- | --- | --- | --- |
| N1-DIAG-PROBE | `IN_PROGRESS` | fixed clean C0=`0670ad1f…` 的 loop/block zero-PPO policy 已自然完成：policy SHA=`ddcc1a7c…a09f` / `73d9de68…1e51`，0 PPO/0 checkpoint/authorization 全 false。adaptive hash claim=`b96a7275…` 随后在 0 PPO materialization 后 fail-closed；逐字段诊断已定谳为 **validator 错读**：有效 Reward receipt 按设计只列非零项，故包含 `racket_position/velocity/normal=0.20/1.0/0.52` 与 coarse=`0.30`，正确省略 weight=`0` 的潜在 `racket_strike_success`；train 的 hash-only 入口在写 receipt 前已另行精确验证四项锁步、完整 schedule 与三旗，runtime scheduler 也原子校验/更新四项 | 唯一下一动作：保持 `racket_strike_success.weight=0`，修 wrapper 对四项非零有效图（含 coarse）验宽度、同时反向拒绝 receipt 意外激活 zero-weight success；用 source pin + receipt-before-write gate 保留潜在 success 锁步证明。focused tests + 独立复核后提交新 clean C0，并在 fresh namespaces 重新物化 loop/block/adaptive 三 pin；三 SHA 同源后才回填 C1 | r1 namespace `...0670ad1f...r1` 永久 spent；禁止编辑 receipt、把 zero-weight term 塞入有效图、提高 success 权重或手填 Reward SHA冒充通过。GPU1 属于另一用户，严禁触碰 | [本轮外部尽调](../../research/dr_reward_external_diligence_20260731.md) |
...
| LIVE-CONTRACT-MATERIALIZER | `IN_PROGRESS` | **双动作 policy recipe 已真跑通；adaptive r1 false negative 根因已定位**：loop/block claims=`d1b6c54f…` / `bc3c7fd8…`，policy SHA=`ddcc1a7c…` / `73d9de68…`；0 PPO/0 checkpoint、全部 unauthorized。adaptive receipt 的有效图正确省略 weight=0 success，wrapper 把“潜在调度状态”误当“非零奖励项”才拒绝 | 按上一行 N1-DIAG-PROBE 的裁决修 validator/tests；新 clean C0、新 spec/plan/new namespaces 重跑三份 hash-only，自然退出且 policy/Reward SHA 可复算后，三 SHA 才原子回填 C1 | r1 namespace spent；禁止编辑其 receipt 冒充通过。`launch_a3_vendor_identity_smoke.py` 不是 post-pin lane smoke；C1 必须用 vendor baseline diagnostic | [N=1 发射工序](../../operations/run_ablation_wave_launch.md) |
