...
| --- | --- | --- | --- | --- | --- |
| N1-RUN | `IN_PROGRESS` | fresh milestone1000 正在 Pod1 GPU2 用 claim `2710fd6f…d4f4` 运行，exact PID/PGID=`1134253`；只读快照已到 update 558，`model_500.pt` 的 80 个 tensor（76 个数值 tensor）全 finite，SHA-256=`fad45f21…8536`。最近 iteration 的 mean episode 为 `500`、strike opportunity=`944`，strike-window hit rate 首次出现 `0.0042`，但 virtual capture/return 仍为 `0/0`；该轮 face/geometry reject=`631/313`，table/fall/actual-hard=`4/1/1`。唯一下一动作是守护到 `model_600.pt` 再做 teacher fidelity/face-gate 趋势判读，随后自然到 1000 | model_600/1000 finite 且身份不漂移；按里程碑报告 teacher fidelity、strike/capture/return、Reward income、table/fall/qdes-hard/actual-hard 与 iteration wall。前五轮腰 actual-hard 只记基线，不单独判学习失败 | 无外部阻塞；只管理该 exact run，不 signal/kill；NaN、身份漂移或 counter invariant 破坏才按预注册规则处理 | [N=1 发射工序](../../operations/run_ablation_wave_launch.md)、[结果判读](../../operations/read_and_report_results.md)、[G05](../../gates/G05_isaac_training_first_loop.md) |
| N1-SPEED | `IN_PROGRESS` | 两批确定性 candidate 已保持深层行为 parity，但同 seed 均值仍为 `17.09 s/update`，累计只改善约 `4.2%`。精确桌碰后端现已改为 5 个 table source × 32 个显式 A3 body filter；Pod 五 role/四子步正控、reset 零泄漏与异常 shell rc 均通过。两次 4096-env 短稳态 A/B 的 table on/off 均值为 `69.79/60.82 ms per policy-step`，桌碰固定税约 `8.97 ms/step ≈ 0.22 s/24-step update`，所以不切 box，也不再把桌碰当 12–14 秒固定税。唯一下一动作是把 diagnostic 普通 step/strike step 的 invariant、finite、partition 与 EMA scalar 合成少量 device→host validation packet，并继续批量化 reset broker/receipt 的逐 env Python；box 只保留为传感器后端失效时的降级设计 | Reward、Done、采球、solver、动作、RNG、P/A/R、task identity、qdes/brake、raw-hard/table/fall 和 checkpoint finite 必须继续等价；diagnostic 不产生 formal authority；4096-env 健康窗达到 `≥15k environment-steps/s` 或等 reset 负载 collection `≤6.5 s`。确定性优化只做 Pod parity/吞吐，不做学习 A/B，也不优化 PPO | 当前 exact milestone 不热补丁；table A/B 是短稳态 backend 定价，不替代 same-seed `4096×5` trainer 验收。formal 的 checkpoint 粒度 receipt 仍是 N5 前独立 schema 工作 | [设计与加速审计](../../research/design_audit_and_speedup_20260729.md)、[N=1 发射工序](../../operations/run_ablation_wave_launch.md) |

### 0.3 Next — long 已运行后的判读与 formal N=5 前置

| ID | 状态 | 当前交付 / 触发条件 | 完成验收 | 阻塞输入 | 证据入口 |
| --- | --- | --- | --- | --- | --- |
| N1-REVIEW | `IN_PROGRESS` | milestone1000 已启动；唯一下一动作是在 update 100 首次检查 teacher fidelity、strike denominator、capture/return、Reward income、unsafe 和 iteration wall，再按 300/1000 复核；不按一次均值临时改超参 | finite/exact identity 持续；动作与 teacher 偏差可解释；strike opportunity 非零；table/fall/hard 与 curriculum failure 分账；1000 结论进入附录 | 运行数据正在生成；若需动作语义/视频晋级，届时由 Franco 人工裁定 | [结果判读](../../operations/read_and_report_results.md)、[G05](../../gates/G05_isaac_training_first_loop.md) |
