...
| `CONTACT-GUIDANCE-ABC` | `IN_PROGRESS / 512 PROBES NEXT` | 同一对拉向 Take_061 fixed question，ready=frame0，prepared-core/tape SHA=`c5212ce9…0370/22052606…9e66`，所有臂 `reset_inverse_solve=false`。A=`current_lm 111`；A-fast=`analytic_full 111`；B=`teacher_pos_face_no_velocity 101`，从源头不求目标速度；C=`outcome_dense_only 000`。held teacher 修复后 A 1x2 PASS；shared reward/policy materialization 在 exact `ad4ba3f4` PASS。固定 birth frontier 错误已修，但 4096 完整 scene 冷启动触发 1800 s watchdog，未进 PPO；先跑 fresh `512x5` 比较，4096 另作 scaling gate。尚缺 varying-ball robot-frame ball/goal ABI、profiler-off 长程学习和 physical-ball outcome bridge；纯 sparse C 仍禁止 |
