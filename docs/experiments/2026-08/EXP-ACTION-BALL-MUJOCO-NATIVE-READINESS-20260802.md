...
> **就地更正（2026-08-07，§5.6.22 取证）：上面这段把"被钳关节常驻软带内、恒扣 `-0.0844`"
...
> （严格窄于内沿，被钳关节确定性零罚），同批把地板从软带挪到机械硬限位。见 §5.6.22。
...
电机反而是宽裕的：限幅 `118.2 N·m`，只用到 `41.6%`。
...
这时轮到电机限幅 `118.2 N·m` 当边界——对需求 `49.2` 仍然**够用还有一倍多**。
...
**变异测试 `14 passed / 0 skipped`**（`test_audit_position_hold_authority.py`），两头都开火：
...
#### 6. 被这堵墙挡住、因此**本轮没有做**的三件事（不是忘了，是不该由 subagent 代签）
...
- **A/C 两族同改**：本轮没有改任何出生姿态，所以两族**天然仍然只差 obs 和 reward**，
  `53040fb0` 那道门不会被触发（也确认过：本轮改动不碰任何族标签、配方或出生来源）。
...
2. **权重上我们在这条轴上比 `build_1` 重 `7.7` 倍，而且多一条。**
   ...
   > **就地更正（2026-08-07，§5.6.22 取证）：原文这里写的"只在 rally 窗内付款"是错的。**
   ...
   > 真正有窗的只有 2 个踝关节那条（占它 qdes 轴 ~5%）。逐字取证见 §5.6.22。
...
2. **qdes 轴的剂量差 15 倍。** 尽调 S6 要 `-10`；`build_1` 实跑 `-0.65`。
   ...
   > 聚合方式与量纲，不是占空比。Franco 2026-08-07 裁定二已按开源形状定案，见 §5.6.22。
...
1. ~~**`build_1` 的 reward 源码我们从来没读过。**~~ **已作废（2026-08-07）：源码已逐字读过，
   ...
   **无门控、全相位**；"门控差异是我们 3.5 倍的头号解释"这个假设**不成立**。
   真正的解释是核函数形状（饱和 vs 线性尾）+ 聚合（SUM vs top4 混合）+ 量纲。取证见 §5.6.22。
...
#### 5.6.22 三条裁定落地：形状照开源、验收看深度、硬超限只记不拦（2026-08-07 落地）
...
| **qdes 轴合计** | **`-0.2504`** | **`-0.022 ~ -0.034`**（中心） | **降 `7~11` 倍** |
| **负收入合计** | `-0.3085` | **`-0.080 ~ -0.092`** | |
| **`\|负\|/正`** | **`16.48`** | **`4.3 ~ 4.9`**（全带 `3.3 ~ 5.8`） | |
| **qdes 轴占罚金** | `75.6%` | **`27% ~ 37%`** | |

对三方表：`build_1` `830xw9hy` @iter2 峰值 `7.12`、@iter4 `6.93`；`i4dxpbwy` @4 `10.87`；
收敛 `0.554`/`0.307`。**改后我们 `4.3~4.9` 落在 `build_1` 早期之下、收敛之上。**
qdes 轴占比 `27~37%` 与 `build_1` 早期 `16.2%`/`29.3%` 同档，**份额倒挂消失**。
