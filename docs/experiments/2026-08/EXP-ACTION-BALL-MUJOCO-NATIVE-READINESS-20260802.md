...
| `CONTACT-GUIDANCE-ABC` | `IN_PROGRESS / WRIST-GUARD FIX` | 同一对拉向 Take_061 fixed question，ready=frame0，prepared-core/tape SHA=`c5212ce9…0370/22052606…9e66`，所有臂 `reset_inverse_solve=false`。A=`current_lm 111`；A-fast=`analytic_full 111`；B=`teacher_pos_face_no_velocity 101`，从源头不求目标速度；C=`outcome_dense_only 000`。exact `56a7d373` 四臂都完成 `512x5`，但都是 `strike_opportunity=0/virtual_capture=0`，约 88% reset 由 `ee_body_pos` 导致，不能按总 reward 排名。successor 已将 striking wrist 从 generic pos/ori/lin/ang mimic 和 reference-body termination 全部释放，由 measured paddle task space 独占；待 fresh Pod 复验非零 strike opportunity 后才选 long。4096 仍是独立 scene-scaling gate。尚缺 varying-ball robot-frame ball/goal ABI、profiler-off 长程学习和 physical-ball outcome bridge；纯 sparse C 仍禁止 |
