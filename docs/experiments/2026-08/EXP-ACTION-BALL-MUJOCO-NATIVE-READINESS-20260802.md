...
| `ISAAC-FOUR-ARM-FIXED-QUESTION` | `A211 V2 CODE IMPLEMENTED / NO 200-TICK SAFE READY AUTHORITY` | fresh A211 是当前四臂 consumer：211/319、actor localizer12+body-gyro3 ordered-layout v2、task-valid WAIT、full-body/measured-paddle/contact/outcome eligibility 和 fresh actor normalizer v2 已实现；critic内容/norm仍v1。tracked frame0 candidate file SHA=`ad17d984…bc54`。exact Pod commit `ea8c7e1d` 用 measured frame0 root/q、零速与同 q hold 真跑时，在 policy step9（`.18 s / 36` substep）触发 `robot_hit_table`。恢复 historical dynamic-ready 后也不能复用旧 60-tick PASS：commit `5482eb54` 的 200-tick延长 canary 在 step62（`1.24 s / 248` substep）因 `waist_roll_joint=-.34910727 < -.34906581 rad` 触发 `joint_actual_forbidden`；无碰桌、双脚仍接触。旧证据末帧已经是 `q≈-.32054, qdot≈-1.1869`，所以根因是把 MuJoCo 静态 LP hold 的短时 Isaac transient 当成长期 ready，而非安全门误报。必须保持所有 hard termination，用预注册候选搜索得到 exact Pod `200/800` PASS 的替代 hold，再验证 physical-ready→frozen teacher-frame0 过渡和 `pre_swing_wait` 下界；此前 lineage/receipt/oracle32/4096/long 全阻断。C211 属于独立 A/C comparison，不是四个 A learnability 臂的前置。 |
