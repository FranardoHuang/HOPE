...
> （站宽差 `0.350 m`，而开环位置指令不会迈步），不在桥；
> ——**注意**：这里原先还列了「`57/57` 帧脚底离地、`35/57` 帧质心出支撑多边形」，
> 2026-08-07 复核后前者归因改成全库共有的一个标量地面偏移、后者证伪（是量具假象），
> 且站宽差那一项异常的是**出生姿态**不是 clip，见 §5.6.7「六」；
...
| --- | --- | --- | --- |
| 脚踩没踩到地（鞋底最低顶点离地板） | `+10.5 .. +12.9 mm`，`57/57` 帧全部悬空 | `-1.5 mm`（压着地，`sole_floor` 门 PASS） | **成立，但不是这条 clip 的事** —— 全库 `73` 条里 `71` 条都不落地，是重定向导出的**一个标量地面高度偏移**（本 clip 需整体下沉 `12.05..14.43 mm`，`57` 帧内只散 `2.38 mm`）。**「只下沉」不够**：地面 LP 实跑
`0/57` 帧接受，因为左右脚差 `3.5..5.7 mm` 而 LP 窗口只有 `4 mm` 宽，必须外加共面收尾，见「七」3 |
| 站不站得稳（质心到双脚支撑多边形的有符号裕度） | **诚实的双支撑多边形上：`+11.1 .. +48.9 mm`，均值 `+27.9 mm`，`0/57` 帧在支撑面外**（原记法：`-25.7 .. +15.6`，`35/57` 在外，frame 0 `-11.9`） | `+123.3 mm`（`support_margin` 门 PASS） | **不成立** —— `-11.9` 是审计的 `SUPPORT_BAND_M = 6 mm` 撞上本 clip 左右脚 `4.8 mm` 高差产生的量具假象 |
| 站姿宽度（两踝水平距离） | `0.6113 m`（`57` 帧内变化 `< 0.1 mm`——**这条 clip 全程不迈步**） | `0.2613 m` | **数字对，异常的是右边那个** —— 动捕真人是 `0.567 m`，按腿长归一化与 clip 差 `0.9%`；全库 `73` 条是 `0.550..0.688 m`；`0.2613` = A3 十二个腿关节全零的 `0.2490 m` 加 `12 mm`，即**厂商 deploy 默认站姿** |
...
**那道门没错，但它拒的是上面第一行那个全库标量偏移**：整体下沉后较低那只脚落地、较高那只脚仍高
`2.2..4.2 mm`，所以它还会拒 —— 差的是几毫米的共面性，不是「这条 clip 站不住」。
...
- **重定向要修的只有一样：地面高度那个标量。**（初稿写的是「retarget 没约束地面接触、没约束静态平衡、
  也没把站姿锚到出生姿态」，后两项经复核不成立，见「六」。）站姿和静态平衡 retarget 都做对了；
  错的是导出时的地面高度约定，全库 `73` 条里 `71` 条整体悬空 `~1 cm`。修法是每条 clip 一个 z 偏移
  加双脚共面收尾，**不动关节角**。**注意不要写「站姿等于出生姿态」这条硬约束** —— 它会把动捕真人的
  预备站位压成立正，是把唯一做对的那一项也毁掉。**§12.3 的 `0/73` 机械准入结论不变**（那是另一套判据），
...
- **该换的是出生姿态，不是 clip。** 但这条路有一个真实的前置约束：出生姿态的 `12` 个腿关节
  ...
  **动作零点**，与 C++ `pp_policy` lockstep；2026-07-26 `EXP-V2-REWARD-FREEZE` §0.13 已明令
  「禁止改资产 `default_joint_pos`」。所以正确做法是**给 ActionBall 谱系一个独立的 ready 姿态**
  （不改资产默认值、不动动作零点），走 fresh 臂，且必须重新过 `sole_floor`/`support_margin`/
  地面 LP/`nominal_hold` 全套静态门。
...
| 现役训练里的活门 `lower_body_stability.min_stance_width_m` | `0.22 m`（`hope_env_cfg.py`）—— 出生姿态离「太窄」罚区只有 `4 cm` |

**按腿长归一化，重定向把真人的 `0.731` 变成 `0.7377`，差 `0.9%`。** 也就是说站宽这一项 retarget
...
但顺手抓到量具的另一处真错，把「六」的所有毫米数往同一个方向挪了 `1.12 mm`。
...
2. **权重上我们在这条轴上比 `build_1` 重 `7.7` 倍，而且多一条、还不门控。**
   `build_1`：`rally_joint_qdes_saturation = -0.65` + `rally_ankle_qdes_saturation = -0.3`，
   只在 rally 窗内付款；我们：投影 `-5.0` + barrier `-5.0`，**两条全相位付款**。
...
1. **投影距离该不该收费。** 尽调 `design_audit:172` 说"投影 + 罚 pre-clamp 超出量"是有文献背书
   ...
   = 0.0253`、`qdes_clamp_fraction = 0.0823`），**收费为零**，只罚 rally 窗内的 saturation，
   而且 `Episode_Termination/actual_q_hard_limit_audit = 0`。我们两条罚合起来 `75.6%`。
2. **qdes 轴的剂量差 15 倍。** 尽调 S6 要 `-10`；`build_1` 实跑 `-0.65`。
   我们取 `-5` 恰好在两者中间 —— **但我们两条都全相位付款，所以实付比 `build_1` 高 `3.5` 倍。**
   先决问题是**要不要照抄 `build_1` 的 rally 窗门控**，而不是调数字。
...
1. **`build_1` 的 reward 源码我们从来没读过。** "`rally_joint_qdes_saturation` 是 rally 窗门控"
   是从项名 + 存在一条独立的 `rally_ankle_qdes_saturation` 推出的**强推断**，不是核实过的事实。
   而门控差异正是"我们 `3.5` 倍"这个数的头号解释。**拍板前应该先读 `build_1` 的 reward 源。**
