...
| --- | --- | --- | --- |
| 脚踩没踩到地（鞋底最低顶点离地板） | `+10.5 .. +12.9 mm`，`57/57` 帧全部悬空 | `-1.5 mm`（压着地，`sole_floor` 门 PASS） | **成立，但不是这条 clip 的事** —— 全库 `73` 条里 `71` 条都不落地，是重定向导出的**一个标量地面高度偏移**（本 clip 只需整体下沉 `12.05..14.43 mm`，`57` 帧内只散 `2.38 mm`） |
