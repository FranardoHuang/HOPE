...
| `MUJOCO-SCENE-CONTACT-HARNESS` | `PARTIAL / SELECTED-RUBBER CONTACT OBSERVED, RECEIPT-V2 POD PENDING` | native ball/table/racket scene、strict contact pairs、portable/backend SHA closure、substep contact/recontact/outgoing latch 已实装。generic `right_racket_collision` 仍只证明 blade contact；versioned classifier 在该 edge 后以 official site frame、URDF 红/黑 planes 与 STL strict safe disk 分类，edge/rim 与两面之间均 fail-closed。exact Pod `592835dc` 同题真实 rollout 得 generic edge=1/table=0/valid outgoing，sidecar 分类正号红面，tick/substep=1/3，切向距 `0.007168732 < 0.044263876 m`，invalid=[]。CLI receipt-v2 已在 host 加入 classification seal 与 classifier/question/scene/backend SHA；无 contact/无 classifier 显式 unknown，host=`15 passed,3 skipped`，exact Pod v2 receipt replay待复核。Reward/PPO/incoming-question parity 仍未授权 |
