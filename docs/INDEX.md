...
| 单动作 FullMDP → Isaac A/C 长跑 → MuJoCo GPU A/C | [唯一执行 TODO](operations/action_ball_single_action_dual_backend_todo_20260817.md) → [Isaac 5.1环境身份](operations/action_ball_isaac51_environment_identity_20260818.md) → [G05](gates/G05_isaac_training_first_loop.md) → [MuJoCo 准备账](experiments/2026-08/EXP-ACTION-BALL-MUJOCO-NATIVE-READINESS-20260802.md)；先跑同一 action slot 的 A，1000 update 内只读里程碑，不为节点重启 |
