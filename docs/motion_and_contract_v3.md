...
- `racket_target_normal_b`(3 维):**随位置通道的惯例走 `yaw(base)⁻¹·n_w`**——方向量
  ...
  **执行机构已备好(R16,franco 2026-07-04)**:`rewards.free_wrist_ori_mimic` 把拍侧手腕
  从姿态模仿里解放(位置/线速度模仿保留)——视频管线的手腕朝向不可靠,模仿它给拍面封顶;
  解放后拍面由 racket_normal/球结果奖励塑形,契约日换指令法线参照即成"按需拍面"的手。
