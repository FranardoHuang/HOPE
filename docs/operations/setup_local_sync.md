...
| RunPod `/workspace/codexschema/motion_video_intake_20260713_{s0,m0}/` | Accepted no-clobber runtime evidence for [S0/M0](../DEFINITIONS.md), the static high-press and four-candidate lateral GVHMR batches | Created on Pod1 on 2026-07-13 through `run_motion_video_gvhmr_prereg.md`; exact file SHA ledger is `configs/motion_video_gvhmr_s0_m0_results_20260713.json`. Copy the complete roots and the five bound GVHMR outputs together; this RunPod path is not a durable artifact service | G08 offline GVHMR preprocessing |
