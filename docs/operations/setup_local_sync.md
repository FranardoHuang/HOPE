...
| RunPod `/workspace/codexschema/motion_video_intake_20260713_{s0,m0}/` | Planned independent no-clobber roots for [S0/M0](../DEFINITIONS.md), the static high-press and four-candidate lateral batches | Not created by the preregistration branch; each exact batch may use the same or a different selected Pod only through `run_motion_video_gvhmr_prereg.md` | G08 offline GVHMR preprocessing |
...
root, GVHMR/GMR output or schema-2 NPZ for this generation. Two GVHMR-only queues are
preregistered but were not run on this branch: S0 contains only `static/pai.mp4`; M0
contains only the four lateral candidates. They bind nominal air-swing event/ready
...
batches, while the four lateral assets are candidates rather than repeated seeds.
