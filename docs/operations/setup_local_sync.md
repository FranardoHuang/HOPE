...
| RunPod `/workspace/codexschema/phase1_signed_face_rescue_20260713/papers/signed_face_exam_k100_v1/` | Future exact-bank-bound K100 schedule and paper-only activation | Not materialized as of the 2026-07-14 source gate. When `consume` succeeds, copy the complete no-clobber root and record schedule file/semantic/order plus activation file/content SHA; a schedule without its last-written activation is partial and must not be reused | G06 signed-face paper activation; never judge/training permission by itself |
