...
解释器必须显式传入，并来自按
[`action_ball_mujoco_environment_identity_20260829.md`](action_ball_mujoco_environment_identity_20260829.md)
的133项lock重建、已安装Torch/TensorDict/MJLab依赖的base venv；当前Pod的
`/workspace/mjlab_venv/bin/python`只是该实例的历史路径，不是跨机器真源。run-local fresh site中的exact
RSL-RL 3.1.2置于`PYTHONPATH`最前，并逐项核版本与distribution origin。
