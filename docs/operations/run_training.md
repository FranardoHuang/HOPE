...
`4096 env × 1001 update`, but v3 rejects every L2 validate/plan/launch before runtime writes until a
separate immutable signed-face directional checkpoint paper path/SHA and reviewed v4 activation
...
child. Preserve v1/v2; run only `control/v3`, whose recursive audit, deterministic environment SHA,
exact-worktree import smoke and frozen file hashes are recorded in the dedicated operation page.
