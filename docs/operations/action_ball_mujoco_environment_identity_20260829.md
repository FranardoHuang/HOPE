...
`2026-08-29`最终在Pod1以checkout=`b0d7d562`建立fresh exact目录，恢复92个mesh并复核root XML
SHA=`7bbda723…bcae1`、payload=`92 files / 25,331,878 bytes`、RSL3 wheel SHA=`40686735…e06d`；完整Mu
successor launcher dry-run RC0。此前`8a57a522`仅Isaac消费playback scale，已撤销训练源资格；真实CUDA
fixed-action因三卡占用仍`未测`。
