...
| `prepare_a3_p1_0803_31d_asset.py` | 从 exact private 0803 raw intake 无覆盖生成独立 ignored 31-action A3 candidate：夹爪 9 轴固定但保留惯量，大小写/mesh 闭包/重复 link 显式归一，`--check` 重算全量 diff/closure；不改现役 pointer、不授权训练 | 0/1 | [0803 归一化记录](../experiments/2026-08/EXP-A3-P1-0803-31ACTION-NORMALIZATION-20260804.md) |
...
| `materialize_action_ball_a225_lineage.py` | 从 fresh A225 bundle/tape/manifest 与已 tracked motion/dynamic-ready 输入生成 canonical、atomic no-clobber 的 commit-required lineage。它允许 explicit fresh 输入以便先审查，但不授权发车；输入文件和 lineage 全部进入同一新 commit 后，launcher 才会接受 | 0 | [successor 账本](../experiments/2026-08/EXP-ACTION-BALL-MUJOCO-NATIVE-READINESS-20260802.md) |
| `launch_action_ball_a225_four_arm_diagnostic.py` | A225 固定题四臂的 code-owned fresh/no-clobber 发车器。主链严格为 `materialize Reward(0 PPO) -> recipe exact policy(0 PPO) -> oracle32 -> scale4096(5 updates) -> long4096(1000 updates)`；并从 runtime effective receipt 验证 balance/mimic/contact/outcome 必需项。`long4096` 只接受同臂 scale 的 finite save/natural clean-exit terminal result。`smoke/probe512/long512` 仅为失败定位支线。单卡双进程需显式 colocation，保持最多两 PID 与 `8192 MiB` headroom，且不进 speed evidence | 0/2 | [successor 账本](../experiments/2026-08/EXP-ACTION-BALL-MUJOCO-NATIVE-READINESS-20260802.md) |
| `launch_action_ball_c225_diagnostic.py` | C225 固定中点 ball-state 路线的独立 no-clobber 发车器；只允许 C-owned 225/318 ABI、incoming-ball p/v/spin、`000`、fresh normalizer/checkpoint 与 immutable-tape zero solver/LM/RNG。当前只开放 `materialize/recipe` 零 PPO 阶段；独立 C oracle consumer 未实现前，`oracle32/scale4096/long4096` 在 GPU/namespace mutation 前 fail closed | 0/2 | [successor 账本](../experiments/2026-08/EXP-ACTION-BALL-MUJOCO-NATIVE-READINESS-20260802.md) |
| `materialize_action_ball_a211_frame0_exact_artifact.py` | 从 measured motion 原样读 frame-0 root/q，强制 root/joint velocity 全零，生成 canonical no-clobber A211 frame0 artifact。它不伪造 Pod nominal-hold receipt，输出仍为 `launch_authorized=false` | 0 | [successor 账本](../experiments/2026-08/EXP-ACTION-BALL-MUJOCO-NATIVE-READINESS-20260802.md) |
| `run_action_ball_a211_frame0_nominal_hold.py` / `consume_action_ball_a211_frame0_nominal_hold.py` | 在 exact clean Pod 上从 tracked frame0+plant 模板派生唯一 zero-velocity/qdes/action probe input，复用 live Isaac nominal-hold 跑足 `200 tick / 800 substep`；consumer 只在 zero terminal/current+substep hard-edge 和五帧截图闭合后 no-clobber 发布嵌入 raw evidence 的 A211 receipt | 0/2 | [exact frame0 hold 工序](run_action_ball_a211_frame0_nominal_hold.md) |
...
| `materialize_action_ball_a211_lineage.py` | 严验 tracked frame0 artifact+Pod hold receipt、motion/tape/manifest/bundle 和 fresh 211/319 ABI；固定 tape 只表示 diagnostic N1/早期 fixed band，不代表最终永久单题 | 0 | [successor 账本](../experiments/2026-08/EXP-ACTION-BALL-MUJOCO-NATIVE-READINESS-20260802.md) |
| `launch_action_ball_a211_four_arm_diagnostic.py` | fresh A211 四臂；主链 `materialize -> recipe -> oracle32 -> scale4096 -> long4096`，512 只定位失败。单卡可显式最多两进程共驻，共驻结果不进 speed evidence | 0/2 | [successor 账本](../experiments/2026-08/EXP-ACTION-BALL-MUJOCO-NATIVE-READINESS-20260802.md) |
| `launch_action_ball_c211_diagnostic.py` | fresh C211 211/319 ABI、incoming-ball p/v/spin、`task_valid`，reward-v2 只有拍心-球心距离与 landing。无 desired-contact 奖励/独立 hit bonus，reset 热路反解为零 | 0/2 | [successor 账本](../experiments/2026-08/EXP-ACTION-BALL-MUJOCO-NATIVE-READINESS-20260802.md) |
