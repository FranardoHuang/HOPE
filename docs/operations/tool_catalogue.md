...
| `scripts/launch_mujoco_full_mdp_successor.py` | clean-Git、fresh `/workspace` root的portable Full-A H48 one-shot；核canonical Python、固定SHA的显式`--ready-pose`、GPU UUID/空compute apps并持flock覆盖child lifetime，固定`4096×12500/save500`、run-local双wheel site与`WARP_CACHE_PATH=<run-root>/warp_cache`，前台等待自然rc。无monitor/retry/resume/signal/`ACCEPT` gate；Pod real仍按每个fresh commit重新验收 | 0/2/child rc | [训练边界](run_training.md#fullmdp-ppo-v3执行配方2026-08-23-branch-candidate)；[实验真源](../experiments/2026-08/EXP-ACTION-BALL-FULLMDP-CURRICULUM-UNBLOCK-20260822.md) |
...
| `mujoco_gpu_ac_full_mdp_wait_rsl3.py --full-a --diagnostic-rate-probe` | 真实portable Full-A H48/4096 PPO热路固定跑61 update（10 warm-up/50 measured/1 tail）；拒profiler-on，不写snapshot/completion，报告raw wall/p50/p90/transitions/s/peak CUDA。正式12500入口不变 | 0/2 | [训练边界](run_training.md#fullmdp-ppo-v3执行配方2026-08-23-branch-candidate)；[实验真源](../experiments/2026-08/EXP-ACTION-BALL-MUJOCO-PORTABLE-FULLA-20260819.md#h48-fixed-tape-and-rate-probes-20260821) |
