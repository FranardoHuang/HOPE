...
| `scripts/launch_isaac_full_mdp_successor.py` | clean-Git、fresh `/workspace` root的Isaac Full-A H48 one-shot；复用现有Kit boot owner，核IsaacLab/Kit/RSL/USD、GPU UUID/空compute apps并让flock、fd16 receipt与fd18 sealed archive覆盖唯一child lifetime。固定`4096×12500/save500`，无monitor/retry/resume/signal/`ACCEPT`或matched-wall启动门；当前host回归PASS，exact Pod dry-run/real仍待执行 | 0/2 | [训练边界](run_training.md#fullmdp-ppo-v2执行配方2026-08-21-branch-candidate)；[hot-path实验](../experiments/2026-08/EXP-ACTION-BALL-FULLMDP-HOTPATH-20260819.md) |
