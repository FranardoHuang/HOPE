...
| `scripts/launch_isaac_full_mdp_successor.py` | clean-Git、fresh `/workspace` root的Isaac Full-A H48 one-shot；核exact runtime/assets、GPU UUID与lifetime flock，绑定run-owned cache/training root。V8候选合同为PPO V6 `512×100000/save2000`、fixed LR`1e-4`、Reward28、V3 `215/231`、milestone schema8和四due/exhausted clock；terminal overlap不public且只在既有CPU drain做跨writer合成。旧V7是negative只读证据 | 0/2 | [现役训练边界](run_training.md#fullmdp-ppo-v6-observation-v3-current)；[课程实验](../experiments/2026-08/EXP-ACTION-BALL-FULLMDP-CURRICULUM-UNBLOCK-20260822.md#109-v7结论固定lr在mimic曝光前耗尽) |
| `mujoco_gpu_ac_full_mdp_wait_rsl3.py --full-a --diagnostic-rate-probe` | 真实portable Full-A PPO V6热路固定跑`512×H48×61`（10 warm-up/50 measured/1 tail）；profiler-off，报告raw wall/p50/p90/transitions/s/peak CUDA且不写snapshot/completion。V8实测待闭合；旧PPO V5 `2048×H48`的Mu p50/p90=`5.468/5.526 s`只作历史，不可代签新shape。probe不能替代fresh学习或成为Gate，正式入口仍100000 | 0/2 | [现役训练边界](run_training.md#fullmdp-ppo-v6-observation-v3-current)；[性能历史](../experiments/2026-08/EXP-ACTION-BALL-FULLMDP-HOTPATH-20260819.md#fullmdp-v6-rate-current) |
