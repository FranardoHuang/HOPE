...
| `scripts/probe_mujoco_full_mdp_h48_tape.py` | 用tracked SplitMix64 `N64×H48×31` tape跑真实portable Full-A env，保存Reward20、203/219 observation、plant和离散生命周期raw arrays；compare只给exact mismatch/difference envelope，缺失自然strata写`未测`，无tolerance/verdict/promotion authority | 0/2 | [当前TODO](action_ball_dual_backend_longrun_todo_20260819.md)；[实验真源](../experiments/2026-08/EXP-ACTION-BALL-MUJOCO-PORTABLE-FULLA-20260819.md#h48-fixed-tape-and-rate-probes-20260821) |
...
| `scripts/launch_mujoco_full_mdp_successor.py` | clean-Git、fresh `/workspace` root的portable Full-A H48 one-shot；核canonical Python、ready pose、plant、EPA48/RSL3/MJLab runtime stack、run-owned augmented `runtime.mjb`、GPU UUID与lifetime flock。最终写schema-6 update/evidence与schema-5 completion，包括`scheduled_due_rows/due_terminal_overlap_rows/reveal_due_rows`及Mu四项cause（R03 nonfinite、R06 source-invalid、R07 sequence/nonfinite）；不再发布`racket_contact_eligible`。固定`4096×12500/save500`，无monitor/retry/resume/signal/业务成功Gate；corrected V5的final SHA、exact Pod与fresh run仍pending | 0/2/child rc | [训练边界](run_training.md#fullmdp-ppo-v3执行配方2026-08-23-branch-candidate)；[实验真源](../experiments/2026-08/EXP-ACTION-BALL-FULLMDP-CURRICULUM-UNBLOCK-20260822.md) |
| `mujoco_full_mdp_longrun_consumer.py` | 独立消费exact schema-6 update/evidence、schema-5 completion并输出schema-5 summary；重验runtime/plant/MJB、Mu四项cause、pure-timeout/table reason及fresh-prefix `launch≤reveal`、`selected≤racket≤launch`、`R03/outcome/retired≤launch`、`landing≤selected`。拒绝`racket_contact_eligible`假分母；selected-contact以launch为分母，`r06_common_per_eligible`是closed landing成功率，`opponent_landing_per_crossing`仅为crossing条件比例。拒绝旧V4伪兼容，只审fresh `diagnostic_unauthorized`证据，不签ready/promotion/deploy | 0/1 | [训练边界](run_training.md#fullmdp-ppo-v3执行配方2026-08-23-branch-candidate) |
| `scripts/launch_isaac_full_mdp_successor.py` | clean-Git、fresh `/workspace` root的Isaac Full-A H48 one-shot；复用Kit boot owner，核exact runtime/assets、GPU UUID与lifetime flock，绑定run-owned cache/training root。保持203/219、history0与reset-ready → post-transition selected frame0 → completed-action frame0 reference；post-settlement due只让仍busy row DEFER，terminal overlap不public，允许same-tick launch+outcome并要求outcome/recovery互斥。Isaac共享ActionEpoch使用独立36-row-fault namespace；pure-timeout与durable ACK后stdout best-effort按训练合同执行。corrected V5 fresh run仍pending | 0/2 | [训练边界](run_training.md#fullmdp-ppo-v3执行配方2026-08-23-branch-candidate)；[课程解阻实验](../experiments/2026-08/EXP-ACTION-BALL-FULLMDP-CURRICULUM-UNBLOCK-20260822.md) |
| `mujoco_gpu_ac_full_mdp_wait_rsl3.py --full-a --diagnostic-rate-probe` | 真实portable Full-A H48/4096 PPO热路固定跑61 update（10 warm-up/50 measured/1 tail）；profiler-off，报告raw wall/p50/p90/transitions/s/peak CUDA且不写snapshot/completion。约6秒只是大幅提速方向，不是probe、launch或safety Gate；该probe不能替代corrected V5 fresh训练结果，正式12500入口不变 | 0/2 | [训练边界](run_training.md#fullmdp-ppo-v3执行配方2026-08-23-branch-candidate)；[实验真源](../experiments/2026-08/EXP-ACTION-BALL-MUJOCO-PORTABLE-FULLA-20260819.md#h48-fixed-tape-and-rate-probes-20260821) |
