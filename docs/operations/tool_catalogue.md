...
| `scripts/launch_isaac_full_mdp_successor.py` | clean-Git、fresh `/workspace` root的Isaac Full-A H48 one-shot；核exact runtime/assets、GPU UUID与lifetime flock，绑定run-owned cache/training root。当前合同为PPO V5 `2048×25000/save500`、Reward24、V3 `215/231`和四due/exhausted clock；terminal overlap不public且只在既有CPU drain做跨writer合成。final SHA与fresh learning canary仍pending | 0/2 | [现役训练边界](run_training.md#fullmdp-ppo-v5-observation-v3-current)；[课程解阻实验](../experiments/2026-08/EXP-ACTION-BALL-FULLMDP-CURRICULUM-UNBLOCK-20260822.md#10-2026-08-25-currentv5反例收口与v6最小桥修复) |
| `mujoco_gpu_ac_full_mdp_wait_rsl3.py --full-a --diagnostic-rate-probe` | 真实portable Full-A PPO V5热路固定跑`2048×H48×61`（10 warm-up/50 measured/1 tail）；profiler-off，报告raw wall/p50/p90/transitions/s/peak CUDA且不写snapshot/completion。约`4.8 s`只是待测迭代目标，不是Gate；probe不能替代fresh学习canary，正式入口仍25000 | 0/2 | [现役训练边界](run_training.md#fullmdp-ppo-v5-observation-v3-current)；[性能真源](../experiments/2026-08/EXP-ACTION-BALL-FULLMDP-HOTPATH-20260819.md#16-2026-08-25-current真实pod反例ppo-v5迭代尺度与未完成rate) |
