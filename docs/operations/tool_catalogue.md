...
| `scripts/launch_mujoco_full_mdp_successor.py` | clean-Git、fresh `/workspace` root的portable Full-A H48 one-shot；核canonical Python、固定SHA的显式`--ready-pose`及必填[`--plant-xml`](../DEFINITIONS.md#mujoco-fullmdp-plant-binding)，绝对路径只作locator。child首次Torch/MJLab import前cold verify单一[`runtime_stack`](../DEFINITIONS.md#mujoco-fullmdp-runtime-stack)的EPA48/RSL wheels与MJLab tree，env后复核loaded modules；schema-5另绑定actual geometry、run-owned augmented `runtime.mjb`、clock/capacity/owner-frame。再核GPU UUID/空compute apps并持flock覆盖child lifetime；固定`4096×12500/save500`、run-local cache，无monitor/retry/resume/signal/`ACCEPT` gate，只产fresh diagnostic | 0/2/child rc | [训练边界](run_training.md#fullmdp-ppo-v3执行配方2026-08-23-branch-candidate)；[实验真源](../experiments/2026-08/EXP-ACTION-BALL-FULLMDP-CURRICULUM-UNBLOCK-20260822.md) |
| `mujoco_full_mdp_longrun_consumer.py` | 独立消费exact schema-5 update ACK与completion；必填[`--expected-plant-xml`](../DEFINITIONS.md#mujoco-fullmdp-plant-binding)，在读`runtime.mjb`/ACK前独立cold verifyruntime stack，从locator重验base/owner-frame并hash+加载run-owned MJB，再逐字段对账geometry/clock/capacity，拒绝V4 `4/4/3`伪兼容并输出schema-4 summary。只审fresh `diagnostic_unauthorized`证据，不签ready/promotion/deploy | 0/1 | [训练边界](run_training.md#fullmdp-ppo-v3执行配方2026-08-23-branch-candidate) |
| `scripts/launch_isaac_full_mdp_successor.py` | clean-Git、fresh `/workspace` root的Isaac Full-A H48 one-shot；复用现有Kit boot owner，核IsaacLab、exact Kit `/workspace/isaacsim-5.1.0/kit/python/bin/python3`、RSL与完整asset package，GPU UUID/空compute apps并让flock、fd16 receipt与fd18 sealed archive覆盖唯一child lifetime。可显式绑定GPU-local `--cpu-affinity`和`0..50`轮有界profiler；checkpoint/WAL/contract写入run-owned `training/`。固定`4096×12500/save500`，无monitor/retry/resume/signal/`ACCEPT`或matched-wall启动门 | 0/2 | [训练边界](run_training.md#fullmdp-ppo-v3执行配方2026-08-23-branch-candidate)；[课程解阻实验](../experiments/2026-08/EXP-ACTION-BALL-FULLMDP-CURRICULUM-UNBLOCK-20260822.md) |
| `mujoco_gpu_ac_full_mdp_wait_rsl3.py --full-a --diagnostic-rate-probe` | 真实portable Full-A H48/4096 PPO热路固定跑61 update（10 warm-up/50 measured/1 tail）；任何Full-A直调均在Torch/MJLab import前cold verifyruntime stack并拒绝ambient `HOPE_GEOMETRY_PY`。rate probe拒profiler-on，不写snapshot/completion，报告raw wall/p50/p90/transitions/s/peak CUDA；正式12500入口不变 | 0/2 | [训练边界](run_training.md#fullmdp-ppo-v3执行配方2026-08-23-branch-candidate)；[实验真源](../experiments/2026-08/EXP-ACTION-BALL-MUJOCO-PORTABLE-FULLA-20260819.md#h48-fixed-tape-and-rate-probes-20260821) |
