...
| `scripts/probe_mujoco_full_mdp_h48_tape.py` | 历史`probe/compare`模式仍保留；新增`cross-engine-probe`使用现役Reward28/V3真实Mu FullMDP边界和同一[`512×H48×31`固定动作首差tape](../DEFINITIONS.md#fullmdp-fixed-action-first-divergence)，记录portable初态/逐tick状态。它先闭合EPA48 pre/post import身份；不训练、不设physics阈值、不授权promotion | 0/2 | [当前TODO](action_ball_dual_backend_longrun_todo_20260819.md#fullmdp-v6-todo-current)；[历史实验真源](../experiments/2026-08/EXP-ACTION-BALL-MUJOCO-PORTABLE-FULLA-20260819.md#h48-fixed-tape-and-rate-probes-20260821) |
| `scripts/action_ball_full_mdp_cross_engine_tape.py` | 唯一tracked SplitMix action tape、portable evidence schema和Isaac/Mu compare；报告initial与post-step exact首差、逐字段max/mean absolute envelope和terminal mismatch，不输出readiness/physics verdict | 0/2 | [定义](../DEFINITIONS.md#fullmdp-fixed-action-first-divergence) |
