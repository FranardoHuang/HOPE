...
| `scripts/launch_isaac_full_mdp_successor.py` | clean-Git、fresh `/workspace` root的Isaac Full-A H48 one-shot；核exact runtime/assets、GPU UUID与lifetime flock，绑定run-owned cache/training root。V9合同为PPO V6、Reward28、V3 `215/231`和六次自然due；`--diagnostic-profile-probe --profile-updates N`只跑N轮inclusive host-wall归因并自然退出、写no-clobber receipt，不用signal，也不作正式速度/学习/安全Gate | 0/2 | [现役训练边界](run_training.md#fullmdp-ppo-v6-observation-v3-current)；[课程实验](../experiments/2026-08/EXP-ACTION-BALL-FULLMDP-CURRICULUM-UNBLOCK-20260822.md#11-2026-08-28-currentv8混源反例与v9自然重叠修复) |
