...
| `mujoco_gpu_ac_full_mdp_wait_rsl3.py --full-a --diagnostic-controller-trace-checkpoint … --diagnostic-controller-trace-output …` | 隔离加载冻结checkpoint，固定`512×240`，从真实Mu plant owner记录`waist_pitch/waist_roll/left_ankle_roll`的q/dq、raw/executable qdes、每子步clamp前后tau包络与hard-edge；首跑保存action tape，`--diagnostic-controller-trace-action-tape`让后续反事实复用完全相同输入。输出root no-clobber，禁止训练evidence/snapshot/completion，不授权checkpoint、训练、physics parity或promotion | 0/2 | [当前TODO](action_ball_dual_backend_longrun_todo_20260819.md#fullmdp-v6-todo-current) |
