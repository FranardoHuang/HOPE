...
| `scripts/launch_mujoco_full_mdp_successor.py` | clean-Git、fresh `/workspace` root的portable Full-A H48 one-shot；核canonical Python、GPU UUID/空compute apps并持flock覆盖child lifetime，固定`4096×12500/save500`和run-local双wheel site，前台等待自然rc。无monitor/retry/resume/signal/`ACCEPT` gate；Pod clean-checkout dry-run已PASS，`96f0ca69` real run已取得连续durable ACK并运行中 | 0/2/child rc | [训练边界](run_training.md#fullmdp-ppo-v2执行配方2026-08-21-branch-candidate)；[实验真源](../experiments/2026-08/EXP-ACTION-BALL-MUJOCO-PORTABLE-FULLA-20260819.md#epa48-fresh-runtime-binding-20260821) |
| `scripts/launch_isaac_full_mdp_successor.py` | clean-Git、fresh `/workspace` root的Isaac Full-A H48 one-shot；复用现有Kit boot owner，核IsaacLab/Kit/RSL/完整asset package、GPU UUID/空compute apps并让flock、fd16 receipt与fd18 sealed archive覆盖唯一child lifetime。可显式绑定GPU-local `--cpu-affinity`和`0..50`轮有界profiler；checkpoint/WAL/contract写入run-owned `training/`。固定`4096×12500/save500`，无monitor/retry/resume/signal/`ACCEPT`或matched-wall启动门 | 0/2 | [训练边界](run_training.md#fullmdp-ppo-v2执行配方2026-08-21-branch-candidate)；[课程解阻实验](../experiments/2026-08/EXP-ACTION-BALL-FULLMDP-CURRICULUM-UNBLOCK-20260822.md) |
| `mujoco_gpu_ac_full_mdp_wait_rsl3.py --full-a --diagnostic-rate-probe` | 真实portable Full-A H48/4096 PPO热路固定跑61 update（10 warm-up/50 measured/1 tail）；拒profiler-on，不写snapshot/completion，报告raw wall/p50/p90/transitions/s/peak CUDA。正式12500入口不变 | 0/2 | [训练边界](run_training.md#fullmdp-ppo-v2执行配方2026-08-21-branch-candidate)；[实验真源](../experiments/2026-08/EXP-ACTION-BALL-MUJOCO-PORTABLE-FULLA-20260819.md#h48-fixed-tape-and-rate-probes-20260821) |
