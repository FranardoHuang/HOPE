...
| `scripts/build_mujoco_warp_epa48.py` | 从exact `mujoco-warp==3.10.0.3` sdist离线应用tracked两文件patch，no-clobber构建/只读复核[`3.10.0.3+hope.epa48.1`](../DEFINITIONS.md#mujoco-warp-epa48-fork) wheel；schema-4 receipt保存patch前后281-file count+manifest digest，复验时重建source逐文件绑定wheel并核ZIP/RECORD。builder环境只作自报telemetry；fixture/GPU/oracle进度与raw provenance SHA不进入build identity。只关闭supply chain，不代签GPU差分或训练授权 | 0/2 | [恢复/构建](setup_local_sync.md#restore-and-build-the-project-owned-mujoco-warp-epa48-fork)；[G06](../gates/G06_isaac_to_mujoco.md) |
| `scripts/replay_mujoco_warp_epa48_fixture.py` | 在两套独立Python/Warp cache与同一physical GPU上重复固定tracked pair；只接受stock24 exact mask256与fork48 zero-overflow、稳定finite active contact。Pod GPU2 10+10 actual已PASS。no-clobber raw+summary；不搜索fixture，不把CPU MuJoCo当oracle，不授权ASan/fixed-tape/training | 0/2/3 | [恢复/调用](setup_local_sync.md#bind-the-exact-epa48--rsl-rl-312-site-for-portable-full-a)；[实验真源](../experiments/2026-08/EXP-ACTION-BALL-MUJOCO-PORTABLE-FULLA-20260819.md#epa48-fresh-runtime-binding-20260821) |
| `scripts/probe_mujoco_full_mdp_h48_tape.py` | 历史`probe/compare`模式仍保留；`cross-engine-probe`在导入任何MuJoCo合同前先绑定显式选择的root MJCF，再用现役Reward28/V3真实Mu FullMDP边界和同一[`512×H48×31`固定动作首差tape](../DEFINITIONS.md#fullmdp-fixed-action-first-divergence)，记录portable初态/逐tick状态、编译后policy clock及Mu solver/integrator选项。它先闭合EPA48 pre/post import身份；不训练、不设physics阈值、不授权promotion | 0/2 | [当前TODO](action_ball_dual_backend_longrun_todo_20260819.md#fullmdp-v6-todo-current)；[历史实验真源](../experiments/2026-08/EXP-ACTION-BALL-MUJOCO-PORTABLE-FULLA-20260819.md#h48-fixed-tape-and-rate-probes-20260821) |
| `scripts/action_ball_full_mdp_cross_engine_tape.py` | 唯一tracked SplitMix action tape、portable evidence schema和Isaac/Mu compare；v3不再复制一份actor center，而是从配置钉住的当前dynamic-ready artifact及其SHA读取`normalized_actor_action`。报告initial与post-step exact首差、逐字段max/mean absolute envelope和terminal mismatch，不输出readiness/physics verdict | 0/2 | [定义](../DEFINITIONS.md#fullmdp-fixed-action-first-divergence) |
...
| `scripts/launch_mujoco_full_mdp_successor.py` | clean-Git、fresh `/workspace` root的portable Full-A H48 one-shot；核canonical Python、ready pose、plant、EPA48/RSL3/MJLab runtime stack、run-owned `runtime.mjb`、GPU UUID与lifetime flock。V8候选合同为PPO V6 `512×100000/save2000`、fixed LR`1e-4`、Reward28、V3 `215/231`与wire`10/5/6`；无monitor/retry/resume/signal/业务成功Gate。旧V7是negative只读证据，不得resume或复用namespace | 0/2/child rc | [现役训练边界](run_training.md#fullmdp-ppo-v6-observation-v3-current)；[实验真源](../experiments/2026-08/EXP-ACTION-BALL-FULLMDP-CURRICULUM-UNBLOCK-20260822.md#109-v7结论固定lr在mimic曝光前耗尽) |
| `mujoco_full_mdp_longrun_consumer.py` | 独立消费exact schema-10 update/evidence、schema-5 completion并输出schema-6 summary；重验runtime/plant/MJB、Reward28/V3/PPO V6身份、paddle真实误差与fresh-prefix因果守恒。拒绝假分母与旧wire伪兼容，只审fresh `diagnostic_unauthorized`证据，不签ready/promotion/deploy | 0/1 | [现役训练边界](run_training.md#fullmdp-ppo-v6-observation-v3-current) |
| `scripts/launch_isaac_full_mdp_successor.py` | clean-Git、fresh `/workspace` root的Isaac Full-A H48 one-shot；核exact runtime/assets、GPU UUID与lifetime flock，绑定run-owned cache/training root。复用机器ignored provisioning中已建立的[EULA/隐私选择](../DEFINITIONS.md#isaac-operator-runtime-authority)，不提供per-run确认flag也不默认替人选择隐私`Y`；调用者显式给OpenGL/GLU目录，launcher在root前核库SHA/SONAME，不再硬编码Pod私有路径。V9合同为PPO V6、Reward28、V3 `215/231`和六次自然due；有限profile入口只做host-wall归因，[`--diagnostic-fixed-action-probe`](../DEFINITIONS.md#fullmdp-fixed-action-first-divergence)在policy runner前消费tracked tape、打印自身具名boot marker并自然退出，不能等待PPO的`Learning iteration`。两者均不用signal，也不作正式速度/学习/安全Gate | 0/2 | [现役训练边界](run_training.md#fullmdp-ppo-v6-observation-v3-current)；[课程实验](../experiments/2026-08/EXP-ACTION-BALL-FULLMDP-CURRICULUM-UNBLOCK-20260822.md#11-2026-08-28-currentv8混源反例与v9自然重叠修复) |
| `mujoco_gpu_ac_full_mdp_wait_rsl3.py --full-a --diagnostic-rate-probe` | 真实portable Full-A PPO V6热路固定跑`512×H48×61`（10 warm-up/50 measured/1 tail）；profiler-off，报告raw wall/p50/p90/transitions/s/peak CUDA且不写snapshot/completion。V8 source `0ad85ae1…`实测Mu p50/p90=`3.796/3.999 s`；Isaac对应入口=`6.835/7.612 s`。probe不能替代fresh学习或成为Gate，正式入口仍100000 | 0/2 | [现役训练边界](run_training.md#fullmdp-ppo-v6-observation-v3-current)；[性能真源](../experiments/2026-08/EXP-ACTION-BALL-FULLMDP-HOTPATH-20260819.md#fullmdp-v8-rate-current) |
| `mujoco_gpu_ac_full_mdp_wait_rsl3.py --full-a --diagnostic-controller-trace-checkpoint … --diagnostic-controller-trace-output …` | 隔离加载冻结checkpoint，固定`512×240`，从真实Mu plant owner记录`waist_pitch/waist_roll/left_ankle_roll`的q/dq、raw/executable qdes、每子步clamp前后tau包络、hard-edge和既有termination bits；首跑保存action tape，`--diagnostic-controller-trace-action-tape`让后续反事实复用完全相同输入。输出root no-clobber，禁止训练evidence/snapshot/completion，不授权checkpoint、训练、physics parity或promotion | 0/2 | [当前TODO](action_ball_dual_backend_longrun_todo_20260819.md#fullmdp-v6-todo-current) |
| `mujoco_gpu_ac_full_mdp_wait_rsl3.py --full-a --num-envs 1 --num-updates 1 --diagnostic-teacher-replay --diagnostic-teacher-replay-output …` | 候选的N=1冻结teacher回放诊断：不构造PPO或checkpoint，仍由真实FullMDP owner自然产生due→task→launch，dynamic-ready hold/frame0 bridge/teacher qdes逆解成raw action后再走现役affine decoder、qdes guard、PD plant与contact classifier。显式加[`--diagnostic-teacher-replay-handoff-mode=direct_frame0_playback`](../DEFINITIONS.md#mujoco-direct-frame0-teacher-replay)才进入唯一frame0原子交接反事实；默认仍为`split_ready_bridge`。输出fresh no-clobber JSON/NPZ和opt-in首个generic contact patch；Mu现役action delay为0-step，不虚构delay。两种模式都是zero-PPO、`diagnostic_unauthorized`；Pod CPU四module union已通过，仍缺fresh CUDA回放，不授权训练、物理或promotion | 0/2 | [当前TODO](action_ball_dual_backend_longrun_todo_20260819.md#fullmdp-v6-todo-current) |
...
| `materialize_action_ball_a211_frame0_exact_artifact.py` | **SUPERSEDED / COUNTEREXAMPLE 工具，但退役理由已被推翻，待重判**：从 measured motion 原样读 frame-0 root/q并构造零速度历史 artifact。退役写的理由是「direct exact-frame0 physical-birth 同门槛扫描已是 `0/73`」；2026-08-07 实测那个 `0` 是**全库共有的一个 `~1 cm` 悬空**被数了 `73` 遍，成因是重定向的脚部目标少了一项 `~31 mm` 的踝高补偿。补一次接地收尾解算（root 与 `19` 个非腿关节逐位不动、只解十二个腿关节）之后，同一批 clip 的 `sole_floor + double_support` 从 `0/73` 变成 `70/73`。**在重判之前，输出仍然只保留 provenance、`launch_authorized=false`，不得进入 current A/C lineage**；重判要连 artifact 重铸一起走，见 successor 账本 §5.6.7「十一」4 与「十」6(b)。**2026-08-08 重判有结果了，但结论不是"可以复活"**：接地补完之后同一条路走到 hold LP 仍然被拒——撑住接地后的 frame 0，`waist_pitch` 要 `-49.155 N·m`，策略增益的位置指令最多出 `-21.704`（`226.5%`）。**"只差接地"这句话已经不成立**；要复活它必须先动腰的保持增益或换出生姿态口径，见 §5.6.7「十二」「十三」 | 0 | [退役 exact-frame0 工序](run_action_ball_a211_frame0_nominal_hold.md) |
