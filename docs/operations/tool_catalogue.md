...
| `scripts/launch_mujoco_full_mdp_successor.py` | clean-Git、fresh `/workspace` root的portable Full-A H48 one-shot；核canonical Python、GPU UUID/空compute apps并持flock覆盖child lifetime，固定`4096×12500/save500`和run-local双wheel site，前台等待自然rc。无monitor/retry/resume/signal/`ACCEPT` gate；Pod clean-checkout dry-run已PASS，real run未测 | 0/2/child rc | [训练边界](run_training.md#fullmdp-ppo-v2执行配方2026-08-21-branch-candidate)；[实验真源](../experiments/2026-08/EXP-ACTION-BALL-MUJOCO-PORTABLE-FULLA-20260819.md#epa48-fresh-runtime-binding-20260821) |
