...
| `materialize_action_ball_a211_frame0_exact_artifact.py` | **SUPERSEDED / COUNTEREXAMPLE 工具**：从 measured motion 原样读 frame-0 root/q并构造零速度历史 artifact。direct exact-frame0 physical-birth 同门槛扫描已是 `0/73`，所以输出只保留 provenance、`launch_authorized=false`，不得进入 current A/C lineage | 0 | [退役 exact-frame0 工序](run_action_ball_a211_frame0_nominal_hold.md) |
