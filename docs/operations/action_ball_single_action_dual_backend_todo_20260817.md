...
| 9 | `PASS-live-step / PASS-host-SAT / HOLD-fresh-snapshot-bind` | commit `e71ee1a…` 的fresh Pod1门已闭合N=1 WAIT step与N=2 masked reset。2026-08-18共卡direct在GPU0完成`16/19`后由同一首错拒绝：table authority把fresh one-shot snapshot的绝对路径误当plant identity，虽其77文件逐项与clean Git canonical A3树相同。fresh修复改用root SHA + portable source closure + live owner-local frame三重identity；不同字节反例和同字节异路径正例=`2 passed` | 提交fresh源码后重跑19-test direct；通过才进入`learn(1)` |
...
| 11 | `PASS-host-callpoint / HOLD-live-RSL3` | 薄launcher只调用upstream RSL-RL 3.1.2 `OnPolicyRunner.learn(1)`并绑定PPO/ActorCritic/RolloutStorage/Adam与ready-pose bytes。共卡one-shot证明GPU/CPU-affinity路径可运行，但direct在16/19后被row9的路径假identity阻断，故RSL未启动（`rsl3_rc=99`） | 用fresh commit/namespace重跑19-test direct；全过后同进程立即跑真实`N=2 × 24`一次update；仍只称WAIT engineering，不称MuJoCo A |
