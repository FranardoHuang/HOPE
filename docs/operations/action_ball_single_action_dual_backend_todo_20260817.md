...
| 9 | `PASS-live-step / PASS-host-SAT / HOLD-attached-name-live` | fresh snapshot的绝对路径假identity已关闭；下一次live direct越过该门后暴露真实scene命名差异：canonical A3 body是`pelvis_link`，MJLab attach后为`robot/pelvis_link`。fresh authority现只接受调用方显式给出的namespace prefix，仍逐项比较32-body父子关系/local frame与portable closure；bare native路径不变。host namespace反例=`7 passed`、device SAT=`4 passed,1 skipped` | 提交fresh源码后用新namespace重跑19-test direct；通过才进入`learn(1)` |
...
| 11 | `PASS-host-callpoint / HOLD-live-RSL3` | 薄launcher只调用upstream RSL-RL 3.1.2 `OnPolicyRunner.learn(1)`并绑定PPO/ActorCritic/RolloutStorage/Adam与ready-pose bytes。GPU0共卡one-shot已真实进入19-test direct；路径identity修复后首错是row9的attached body namespace，故RSL仍未启动（`rsl3_rc=99`） | 用fresh commit/namespace重跑19-test direct；全过后同进程立即跑真实`N=2 × 24`一次update；仍只称WAIT engineering，不称MuJoCo A |
