...
| 9 | `PASS-live-step / HOLD-learn` | commit `e71ee1a…` 的fresh Pod1 GPU2门为`12 passed`：N=1 reset、非零step、timeout、Reward20、park与N=2 selected-reset peer均闭合；result/log SHA=`f4d41aa9…/5fa33b26…`。最小WAIT transition净增396 production LOC；C-frame body-COM velocity、上一拍anchor cache、per-joint nonfinite fallback均已实测。resolved table contact仍只作具名backend bool，不冒充Isaac SAT keepout | 实现portable component-OBB SAT keepout并跑GPU反例；此前禁止`learn(1)` |
