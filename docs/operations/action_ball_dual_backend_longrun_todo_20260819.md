...
| evidence | Isaac milestone schema7按具名episode slice解码；Mu update/completion/summary wire为`9/5/6` | suffix在追加paddle telemetry后不再等于episode字段；wire只绑定可界定事实，不将布局偶然性当合同 |
