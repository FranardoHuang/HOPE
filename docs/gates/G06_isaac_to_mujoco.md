...
teacher replay装了pre-reset source latch：NPZ每tick分列累计keepout、canonical final-forward
resolved与substep resolved-any；中间resolved后分离不与final truth比较。summary只冻结首个
canonical bit16 source的first-positive substep与boundary，substep resolved首见另记为diagnostic。
两类canonical source都无而bit16存在，或source存在而
bit16不存在，均为typed unknown并fail closed。非replay路径不创建buffer、不D2H、不改
termination。更准确地说，默认路径只多一个轻量consumer存在分支，没有buffer或
D2H。body/component/table-role归因暂不新建热路，等首次source分流后再决定。
...
明确表面：Torch tensor本身、或真`warp.array`的Warp-to-Torch zero-copy bridge。
第二个CUDA又证明真proxy无`.torch`，但仍在180-step前fail closed。对pinned MJLab
1.5.3的`sim_data.py`审计后，真类型唯一确认为`TorchArray`，其公开索引返回
共享的底层Tensor。后继因此仅对这一exact class执行`value[...]`，不再根据
`.device`、shape、类似属性或DLPack协议猜类型。spent roots
