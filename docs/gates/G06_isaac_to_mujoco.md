...
`643f03b5`的fresh GPU1 direct-frame0 zero-PPO replay又提供了独立边界证据：29/29 handoff谓词通过，
自然从teacher frame0消费到frame8；随后component51 `right_wrist_roll_link.stl`的single OBB在第18个
physics substep给出`-1.236878 mm`，同时Mu backend resolved contact/substep与generic contact均为false。
