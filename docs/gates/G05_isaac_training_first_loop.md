...
- The active `HOPEPingPong` config now defaults to unified forehand+backhand training (`registry_name_2` enabled), `target_mode: uniform`, fixed strike plane `x=0.4`, per-clip strike phases, actor `swing_type`, and no actor `racket_target_normal_w`. (This note originally recorded `strike_phase_per_clip: [0.36, 0.74]`; the phases were later re-measured for the corrected clips and the current YAML uses `strike_phase_per_clip: [0.47, 0.33]`.)
...
- Merged from `train_1` (2026-06-26) and superseded by the unified HITTER alignment: paddle-contact timing is per clip via `strike_phase_per_clip` (`[0.36, 0.74]` at merge time; `[0.47, 0.33]` in the current YAML for the corrected clips); `episode_length_s: 3.0` caps each episode to about one swing; `scripts/train.py` / `cfg/train.yaml` keep the `checkpoint_path` knob for staged resume.
