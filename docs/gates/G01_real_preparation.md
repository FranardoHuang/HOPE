...
- The B/C schema-2/FK source gate now binds that bijection, the exact 32-body runtime column order,
  the vendor MJCF plus all `74` referenced meshes, and an exact donor-ONNX metadata expectation.
  It remains offline: the donor metadata has not yet been re-extracted from the ONNX in runtime,
  no private motion was read, and no simulator or hardware command was authorized.
