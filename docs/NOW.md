...
| 引导比值臂×2(原拍面加权臂,franco 07-07 升级设计) | 主攻 + **常引导惩罚**:位置/速度/拍面误差给一个小而恒的负分(挥不到也有梯度,不像 exp 核在远处饿死)+ 击中/上台稀疏正奖金;两档罚强度 | 什么比值能"一直引导又说清我们要什么"(高击球率→高上台率→未来高球质);**比值铁律:引导罚每步收入 << 模仿收入(≤10-20%),奖金稀疏而大** |
...
| --- | --- | --- | --- | --- |
| 跑着 | **第一波重跑·结构四臂全部在训**(2026-07-07 深夜点火,每臂冒烟过 CLAMP+WARN 审读):主攻 s1w3_main + 产品线原样 s1w3_baseline 共卡,蒙眼 s1w3_blind、只换题库 s1w3_bankonly 各独卡;9600→13600,约 2.5-3h 到线;已审 WARN=v5 标定对帧 0 朝向未重落地(+17.5°/+19.5°)——沿用第一波裁决:倾斜已被握拍标定吸收,题库/登记/npz 自洽,本波资产不动(claude) | — | 共跑 | 4 臂在训 |
