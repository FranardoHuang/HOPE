...
只是**臂的定义表**,排程一律走下面的统一队列(J 臂和别的臂一样按序抢槽,owner=jiayi)。

| 臂 | 内容(人话) | 现值 vs 基线 |
| --- | --- | --- |
| J1 | 熵系数:训练"乱试劲头"大一半有没有用(他已自退 0.01,降为记录性验证) | 0.015 vs 0.01 |
| J2 | 脚部姿态惩罚剂量:不许拧脚的代价 | -0.3 vs 0(他自己注释:窗口 [-0.5,-0.1]) |
| **J3** | **q_des 限位剪切采纳**:训练=真机的目标角剪切,S1/产品线该不该也开 | on vs off(他谱系已 on 且过三门禁;我方默认 off) |
...
**调度规则(就这三条)**:
1. **任何空槽拉队列里最靠前的"就绪"项**(就绪=依赖列全满足+输入文件已在该 pod);
   卡和 pod 无角色之分,全池 6 卡 × 2 槽 = 12 槽动态分。
2. 任务自带属性,不是卡带属性:`独占`(跑到底类,单卡不共跑)/ `共跑`(消融臂,≤2/卡)/
   `CPU`(判卷/出题,不占槽);错峰 ≥60s、发射过 runbook 十条照旧。
3. 输入不在本 pod = 一条自动前置依赖(rsync 热启存档/题库/npz,几分钟)——由调度器先跑,
   不算人的事。
...
| 1 | W1 判卷:A/B/D 已到线,横比凑齐一对判一对(claude) | 1a-0 bank 题源收尾(半成品在 `wip-bank-exam-source-0706`) | 导出占槽 4min+CPU | **今天** |
| 2 | **W2 奖励结构波 4-5 臂**:对照/击球窗/击球窗+权重/难度课程/J3 剪切(claude) | W1 判卷 + 1b 记账 + 1c 击球窗代码(+1d 课程 loader) | 共跑 | **最高优先;代码今天写** |
| 3 | jiayi Hitter 主线长跑(jiayi) | 他拉 main 后随时 | **独占** | 他的节奏 |
| 4 | W3 巩固波:赢家跑到底+换种子+动作源复验 + 等待/预警起步档搭车臂(claude) | W2 判卷 + 等待/预警①②代码 | 跑到底=独占,其余共跑 | 等 |
| 5 | J1/J2/J4 参数正名臂(jiayi) | 周会对表定档位 | 共跑 | 等 |
| 6 | S2a 拍点框 3 臂(claude) | S1 过线 + 出题器 point_mode(yikang) | 共跑 | 等升段 |
| 7 | S2b 手补/身补/转补 4 臂(claude) | S2a 过线 + 位移卷生成器 | 共跑 | 等升段 |
| 8 | S3 旋转 5 臂(claude) | S2 过线(考卷加旋已有) | 共跑 | 等升段 |
| 9 | J5/J6(jiayi) | J1/J2/J4 出信号 | 共跑 | 等 |
| 10 | 加固包 1-2 臂:感知毛病+相位噪声+预警窗末档 整包(claude) | 回球率>0 谱系 + 相位噪声代码 | 共跑 | 等 |
| 常驻 | checkpoint 抽查(每个新存档:空隙导出+MuJoCo 考卷) | 1j 流水线 | 导出占槽 4min+CPU | 建好即常驻 |
...
- **池 = 2 pod × 3×5090 = 6 卡 12 槽**,统一队列动态分;唯一的物理事实差异:老 pod 卷上有
  全量数据(500GB),新 pod 数据不驻留——所以"输入 rsync"是新 pod 上任务的自动前置依赖
  (调度规则第 3 条),不是分工。
- **D9(最高优先)**:①新 pod 一键 bring-up(同路径 rsync venv + 带补丁 extscache + env.sh
  含 `HOPE_URDF_IMPORTER_NO_UI=1` + 资产软链 + 快速冒烟;实测"登录→可点火"时长记 runbook);
  ②**统一队列调度器**:读一份队列清单,轮询两台 pod 的空槽(nvidia-smi 按卡计数),就绪项
  自动派发+自动 rsync 输入;去掉一切写死卡号。
- 老规则不变:≤2 任务/卡、错峰 ≥60s、独占属性的任务单卡、卡型钉死 5090-32GB。
- 扩缩容 = 池大小跟着队列走:就绪臂数持续 > 空槽+2 → 加卡;持续 < 4 卡占用 → 退新 pod
  (数据不驻留,退了不心疼)。全程纯算力 80-150 卡·时,6 卡下**墙钟 ≈ 2.5-3 天**,
  节拍器是判卷/拍板不是卡。钱:6 卡 × ~$1/h × 3 天 ≈ $400±。
...
| D9 新 pod bring-up 脚本 + **统一队列调度器**(一份队列清单→两台 pod 空槽自动派发+自动 rsync 输入) | ★★★(第二台 3×5090 已拍板) | claude | — | 新 pod 到手即实弹;实测时长记 runbook |
