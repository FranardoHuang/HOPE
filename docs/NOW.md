...
**今晚点火版(2026-07-06 定,franco:收尾+升级新基线,让上台率起飞)**——第一波以 yikang
分支的接线为准:臂 1 主攻(反解题目+拍面观测 179+腕踢除+固定点/无旋/场馆速度档,锚=v5 反手
_cal f17-19)、臂 2 无观测对照(175)、臂 3 速度隐含对照(SMASH 式:不给拍面观测但拍面奖励
锚指令——裁"显式 vs 隐含")、臂 4 产品线原样;判据=阶段 1 考卷(固定点/无旋)回球率——
**第一个非零就是历史,起飞线定 ≥50%**(反事实上限 100%)。SMASH 窗口奖励/相位噪声/区域采样
实现完即插第二波。前置:yikang 反手 bank(重出中)+ mech check;franco 阶段考卷开关(评估器,
今晚)。
