...
## o4 轮计划(2026-07-05 晚定;新 mindset:一切为把「回球率」从 0 拉起来,franco 授权按新发现重排)

北极星 = 真球考试回球率(人话行见术语表;命中率保留)。臂按贡献回球率的机制排序:

| 臂 | 内容(人话) | 状态/依赖 |
| --- | --- | --- |
| **R18 拍面指令(头号)** | franco 方案 = 契约日训练侧先行:观测 175→178(+3 维「这球要的拍面朝向」,由虚拟球采样时反解好),racket_normal 奖励参照从模板改成指令。**真机 C++ 侧不动也能先验证**——eval-B 回球率直接裁决;有效后再排全队契约日(pp_obs_builder 同步) | claude 实现(训练侧代码 + eval harness 支持 178 维);机制检查后走看门狗 |
| R16b 无指令对照 | R16(手腕放开)+ **拿掉 strike 侧拍面锚**(racket_normal_weight 0.5→0)+ 分阶段出题第一阶段(落点框放宽到"现状摸得到")——回答"不给指令、只解锚+点亮奖励,能自己学出拍面吗";R18 赢它 = 指令通道的净贡献 | claude;和 R18 同批对照 |
| 分阶段出题 v1 | franco 方针落地:第一阶段来球/落点域 = 可达性扫描反推的"当前物理上打得回"的域(反手贴网斜线小缝;正手无解,靠 R18);奖励点亮后逐步扩散到真球分布 | claude(扫描代码现成,反向用) |
| 变速考卷 | R14 判读的前提(franco:考卷全原速,变速训练等于没考)——评估器加变速协议,出题速度 0.8-1.2 随机、要求拍速同步 | claude,CPU |
| R15 重发 | 发到产品线(上次看门狗发错任务作废) | 等 R18/R16b 批次一起排卡 |
| R12 部署轴考 | 减速入位的"刹得住"专项考(远目标/跑动场景)——composite 已证无税,行为收益未证 | CPU + 排臂 |
| R17 位置奖励收紧 | jiayi 的 0.20→0.15 发现走梯子 | 排在 R18 批次后 |
