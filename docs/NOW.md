...
- **当前训练池（2026-07-16 约 03:05 CST）：** 两台 Pod 都已按三卡各四条铺满，共 `24/24`
  条 trainer；最新只读审计中 Pod1 为 `12/12`，Pod2 也为 `12/12`，均无 fatal。Pod2 今夜新增七个
  “明早能打”组合候选：两档拍面引导、不同已学母本、释放非击球臂、保守模仿、全栈组合，以及一个
  16 秒多次挥拍长回合。七条均从只读 `model_3500` 完整恢复 policy/value/optimizer 并真实越过首迭代，
  PID 为 `426506/427190/428347/431061/431910/432838/433601`。其中前两条已到绝对
  `model_3700`（母本后 `+200`），文件名迭代、finite、schema-3 hard contract、claim/binding 与
  `lineage_exact=0` 均通过；这只证明续训与 checkpoint 正常，**还没有行为胜者**。自由非击球臂与
  保守模仿的原始两次启动分别在首迭代前遇到 malloc `rc134` 和 stale `rc125`，旧 PID
  `429116/429974` 仍为 `/proc` 与 NVML 双重 absent；证据保留后，各自唯一一次同配方新 namespace
  重试已成功越过首迭代，不再授权重试。下一步按 `+200/+500/+1000` 查看结构激活、安全/平衡和
