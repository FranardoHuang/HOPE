...
| 2-4 | 产出 | "每厘米位移的最优通道"分界表 → 部署规划器分工规则(**存废待 franco 拍板**:真球时代该表可能被轨迹分布自然取代) | 2-3 | claude |
...
| --- | --- | --- | --- | --- |
| 跑着 | **s1w4 消融第一波·动作轴五臂在训**(franco 07-08 深夜授权直接点火;机制检查全过、错峰 75s、热启 s1w3_main model_13000、physical_ball 仪表全开、4096×4000→17000):M1 v5rg 基准(gpu0)/ M2 v4rg(gpu1)/ M3 swing(gpu2)/ M4 混对 v5rg正手+v4rg反手(gpu0)/ M5 基准换种子(gpu1)。**奖励轴 R1-R8 被发射代理正确拦下**(旗标未上 main=机制检查无生效行;且实测 main 静默吞未知 rewards 键,硬发=八个假臂)——旗标实装收尾+补发进行中;补发后三卡按 3-4 任务/卡拉满。臂表/PID/配置串=pod /workspace/franco/s1_wave4/queue.md;每 3h 自动巡检+掉卡看门狗已挂 | — | 3-4/卡新规 | 队列表=queue.md |
| 台账更正(07-08 点火时核实) | ①**pod 实况=3 张 5090 不是 6 卡**(第二台 pod/D9 从未到位,算力池小节的"6 卡 12 槽"是纸面值,待 franco 定是否补第二台);②**wave3 七臂不是被停,是 07-07 两次全局掉卡事件团灭**(08:48 团灭只换题库/原样,10:26 团灭主攻/蒙眼/face3x/face10x/模仿降权;主攻止步 13089≈85%)——此 pod 掉卡有前科,wave4 巡检带自动续跑看门狗;③**修C 已收卷**(model_13599 在,日志末尾 CUDA 报错=收尾噪声),判卷链(修C vs fixE 对判)代理在跑 | — | — | 实况为准 |
