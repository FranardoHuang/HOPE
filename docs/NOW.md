...
| 0b 难度地图/题库原料 | 10 万题逐题反解 + 难度分层(qbank_v0.npz 已在产) | 分层直方图;题库材料转作评估/curriculum 刻度(训练侧改用 torch 在线解) | 完成中 |
| 0c v5 拍面扫描 | ✅ 已做:v5 触球帧 0.47/0.46 vs hopex 0.41/0.39,v5 反手相位 0.18-0.23 有朝前段 | 按 0d 重新裁决(朝前分量只是粗代理) | 完成 |
| **0d 相位×可回球性扫描** | **✅ 已做(07-05 深夜)**。判据:拍面钉在该帧、挥速限该帧 clip 挥速锥(±25°、0.6-1.4×)、击球点用该帧拍位,真球分布下"存在合法回球方案"的球占比。**结果:六条 clip 有五条全程 0%**(hopex 正/反、v5 正、斜录正/反——按各自挥速特征打不回任何球);**唯一窗口 = v5 反手相位 0.32:96%**(0.28-0.32 段 71-96%),而**标注 0.345 处 0%——差两帧,96%→0%** | ①击球帧敏感度=一帧级,给 yikang 一个具体靶子:v5 反手真触球疑在帧 18(相位 0.32)而非帧 20(0.345);②斜录的失败**不是**换相位能救的(全程无窗)——franco 假设在斜录上不成立,在 v5 反手上以"标注偏出唯一窗口"的形式成立;③**阶段 1 固定击球点锚在 v5 反手 0.32 状态附近**(唯一已知能打的锚);正手无任何模板有窗 → 全靠反解目标把策略从模板拉开。口径注意:0% 指"按该动作自身挥速特征",非绝对物理不可能;分辨率 24 球×150 速度样本 | ✅ 完成 |
