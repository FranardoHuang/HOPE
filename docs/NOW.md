...
| 契约日 | 改策略观测格式的大日子(175→179/181),训练端和 C++ 端要同步改、攒着一起改。**候选消融(franco 07-07):参考观测去下半身**——看全身但只奖励上半身可能没意义甚至有害(等球换代事故的结构根源就是策略看着下半身参考做平衡);腿该由任务驱动(站位指令),不该由"舞谱"驱动。契约日拼桌时做一臂:参考观测只留上半身 |
...
| **反手修复消融(franco 07-08 下令跑起来;归因已完成)** | **归因**:反手挥拍完成率两波都是 0——上一波(剪切关)摔率 52-54%,这波(剪切开)93%:**病根=v5 反手参考本身不可平衡执行**(加速度 15.5 rad/s²=hopex 6 倍),剪切只是加重。**"重启就好吗"——不是**,重启不改参考。修复臂(全部=主攻配方+单一修复,同批对照=在跑的主攻臂):**修B 反手换锚 f17**(相位 0.298,合并卷 v5 正手 818 题+f17 反手 791 题已过守卫门,零代码,已排队等槽)→ **修A 反手慢放 0.8×**(新 per-clip 慢放旗标已实装,参考慢放、题目答案不缩)→ **修C 反手换 v4 动作**(v5 正手+v4 反手混对;等 v4 卷 v2 现场生成)→ 赢家组合(如 f17+慢放)。**制度学习(retargeting 为什么没拦住)**:重定向尊重位置限位(腕链审计零裁剪),但**速度/加速度与"边做边平衡"的动力学可行性无任何管线检查**——q_des 剪切剪的是策略不是参考,"合法但暴烈"的参考就这么漏进来;修法=登记表 gate 加全身限位+速度余量审计 + SMASH 式跟踪可执行性过滤(0i ④)用到原始 clip | 反手起死回生的最短路径 |
| (已停臂台账 07-08) | 只换题库臂/产品线原样臂已停(TERM→KILL,存档留在 ~1.2 万步可补考):它们的答案已定型——只换题库=0 上台(拍面被录像锚死),原样=0(旧世界地板);腾出的槽给了模仿降权臂与修复臂 | 省卡 |
...
**第二波·动作组三选**(前置:等球锚旗标实现=半天;斜录对先做几何转正):
上一波赢家配方**全部固定**,唯一变量 = 用哪套老师动作——v4 对 / v5 对 / 斜录对,三臂同批,
...
| --- | --- | --- | --- | --- |
| 跑着 | **第一波重跑·七臂**(结构 2×2 四臂在训 + 比值三臂追发中):主攻/蒙眼/只换题库/产品线原样 + 话语权×3、话语权×10、模仿降权×0.5(追发三臂同代码同种子同热启,前两臂即上、第三臂等首个空槽);9600→13600;已审 WARN=v5 帧 0 未重落地(沿用既有裁决:标定已吸收,资产不动)(claude) | — | 共跑 | 4 在训 + 3 队列 |
...
| 2 | 【阶段1】**六臂合体波(franco 07-07 定稿:奖励和动作组一波开,硬化植体重发)**:基准臂 / 击球窗臂 / 击球窗加权臂 / 课程臂 / v4 老师臂 / 斜录老师臂(人话见上面对照表)。奖励轴 = 击球窗臂、加权臂、课程臂 各减基准臂;动作组轴 = v4 老师臂、斜录老师臂 各减基准臂;基准臂一臂两用。各臂自家题库 v2 + 自家首帧等球锚 | 硬化植体已默认 + 题库 v2(已有)+ 奖励收入记账 + 击球窗代码 + 课程 loader + 等球锚旗标 + 斜录几何转正(yikang) | 共跑;6 卡下可每臂独占(一轮 ~2.5h) | **最高优先** |
