...
| R11b | `task.motion.clip_switch_prob=0.0005`(剂量/4,**已点火 2026-07-05,o3_R11b_switch5e4**) | franco:继续调参找好参数。先扫剂量;若 0.0005 仍税重,下一轮动奖励侧(打断后 hold 补偿)。**⚠ 判读标准更新(2026-07-05 换招压力测试结果)**:抗摔轴上 P2 本来就满分(0 摔),R11b 在该轴不可能赢——它的存在意义只剩"更低剂量是否免税"(Isaac composite 对齐 P2)。**临时量过一次(@2700/4000,exported_it2700_normbaked)**:同样 0 摔、存活 100%,且两种 PD 口径命中率均 1.00(分布内不伤命中,好于 R11 的 0.98)——跑完 4000 后 Isaac composite 是最终裁决;若税仍在,**clip_switch 方向整体关闭** |
