...
| **0d 相位×可回球性扫描(franco 最优先)** | 对 hopex/v5/斜录 六条 clip 逐帧扫:拍面钉在该帧、挥速限制在该帧 clip 挥速的锥内(方向 ±25°、幅度 0.6-1.4×),对真球分布算"至少存在一个合法回球方案"的比例 = **该帧当击球帧的可回球性**。报告:标注相位的值 vs 全程最优相位的值与位置 | ①给 yikang 的相位核对当物理参照;②若斜录/新 clip 的最优相位远离标注 → franco 的"斜录败于相位没对上"假设成立;③训练相位消融只带 top-2 候选,省臂 | CPU ~10 分钟(16 进程) |
