...
- **考卷节奏(franco 2026-07-06)= checkpoint 抽查制**:平时追踪 Isaac 训练内曲线;每个存档
  落盘后后台自动导出+MuJoCo 考卷(异步 CPU,不阻塞训练),攒出低频考卷曲线。信号档臂到线
  考一次;长跑每 1000-2000 步考一个、峰值区加密。流水线=阶段 1 表的 1j。
...
### 等待与预警设计(franco 2026-07-06 立项;"等待时间随机 task"——数字先估后测)

**真机时间线(现有数据+代码推出的起步数字,待 1k 实测校正)**:对手触球 → 动捕锁球+速度
拟合(31 帧@300Hz ≈ 0.10s)+ 传输(≤0.02s)+ 规划求解(拍位/拍速快;**拍面反解 LM 现状
~0.3s**,torch 版待部署)→ **对手触球后 ~0.15-0.45s 有第一版有效目标**;弹跳后(约飞行 60%
处)预测精度大涨 → "精目标"再晚 ~0.1s。场馆球飞行全程:快球 ~0.7-1.0s、中速 ~1.2s、
挑高 1.5s+(来球中位速 1.9 m/s)。**推论:预警时间(有效目标→触球)≈ 快球 0.3-0.6s /
中速 0.8-1.2s;精目标预警再短 ~0.2-0.4s。** 训练现状的预警=整个起手时长:hopex 对 1.31/0.88s
(正/反),v5 对 0.75/0.42s——**反手 v5 已经天然贴着快球下限,正手离 0.3s 还差一倍**。

**设计(三件套,全部旗标默认关;和别的一样"慢慢扩大采样分布")**:

| 件 | 内容(人话) | 课程(分布扩张表) | 状态 |
| --- | --- | --- | --- |
| ① 等待混合采样(`hold_steps_mixture`) | 等球时长:80% 抽短档(0-2s,保击球密度)+ 20% 抽长档——**不能让等待拖慢学习**的解法,替代均匀拉长(击球密度砍半的教训) | 长档 2-4s → 4-8s → 8-12s(场馆节奏 3-8s/球,ARM A 的 5 秒余量要求盖住) | 设计好,半天代码 |
| ② 目标揭示预警窗(`target_reveal_lead_range`) | 等球段目标不可见(obs 喂 Δ=0 占位,=部署"未接战"语义),距击球 W 秒才揭示真目标;W<起手时长时策略必须"没看到题就起拍、看到后中途修正"——这正是真机规划器的样子。实现可复用中途改目标(midswing_resample)通路 = "保证在 tts=W 处改一次" | W∈[0.8,1.3](≈现状,先证不掉分)→ [0.5,1.3] → **[0.3,1.3](快球下限)** | 设计好,1 天代码 |
| ③ 预警分布实测(1k) | 从 ball_mocap_0703 + 规划器日志量真实"触球→首个有效计划/精计划"分布,替换上面的估算;顺带把拍面反解的 0.3s LM 换 torch 批量版(部署提速,数字直接进 ②的档位) | — | CPU 活,yikang+claude |

**归属与顺序(按排序法则)**:①②改任务分布、与奖励结构交互 → **首臂挂 W3 巩固波搭车**
(起步档,先证明"开了不掉分"),S2 起并入所有臂当默认结构(像分阶段出题一样),加固期扩到
真实下限档。**不进 W2**(W2 只测奖励结构,一次只动一个自变量)。
...
| 队 2 | W3 巩固波(跑到底+种子+动作源复验)+ **等待/预警起步档搭车臂**(混合等待+揭示窗 [0.8,1.3],先证不掉分) | W2 判卷 + ①②代码 | 等 |
...
## GPU 舰队计划(2026-07-06 定版:**双 pod 6 卡**;franco 已拍板再租一台 3×5090)

**分工(两台各司其职,不混)**:

| Pod | 角色 | 卡分配 |
| --- | --- | --- |
| 老 pod(基座,卷上有全部数据/环境/补丁) | 长跑 + 判卷 + 导出 + jiayi | gpu0=jiayi;gpu1/2=我方长跑(12k 独占)+ 导出槽 + F/零头臂 |
| 新 pod(消融波突发池,**数据不驻留**) | 整波齐发齐收 | 3 卡 × 2 槽 = 6 槽:W2 波 4-5 臂一轮全落地;之后 S2/S3 各波 |

- **新 pod 用"随用随毁"模式**:每波去程只 rsync 热启存档+题库+动作 npz+代码(<20GB),
  回程只收 ckpt/ONNX/日志到基座——不在新 pod 攒第二份 500GB 卷(躲开"卷钉死宿主机"的
  第二个依赖;哪台便宜/健康租哪台,还能抽渲染栈健康的宿主机彩票,清录像欠账)。
- **bring-up = D9 脚本的首次实弹**:同路径 rsync venv + 带补丁 extscache + env.sh
  (`HOPE_URDF_IMPORTER_NO_UI=1`)+ 资产软链 + 快速冒烟;**实测"登录→可点火"时长记进
  runbook**。调度器同步去掉写死卡号(读卡列表环境变量)。
- 节奏预估(6 卡):W2 当天落地判卷 → W3 巩固与 jiayi 波并行 → S2a/S2b/S3 每天 1-2 波;
  全程纯算力 80-150 卡·时,**墙钟 ≈ 2.5-3 天**(判卷/拍板是节拍器,不是卡)。
- 老规则不变:≤2 任务/卡、错峰 ≥60s、关键路径独占卡、卡型钉死 5090-32GB;
  S2 起若持续 <4 卡占用,新 pod 整台退(按小时计费,数据本来就不驻留)。
- 钱的量级:6 卡 × ~$1/h × 3 天 ≈ $400±,判卷节奏不等卡。
...
| D9 新 pod bring-up 脚本 + 调度器去硬编码 | ★★★(升级:第二台 3×5090 已拍板) | claude | — | 新 pod 到手即实弹;实测时长记 runbook |
| 等待混合采样 + 目标揭示预警窗(①②,旗标默认关) | ★★ | claude | main | 约 1.5 天代码;首臂搭 W3 |
| 1k 预警分布实测(触球→有效计划)+ 拍面反解 torch 化部署 | ★★ | yikang+claude | — | CPU 活;数字回填 ② 的课程档 |
