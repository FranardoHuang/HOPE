...
**「老师动作适配器」接口(franco 2026-07-06:不仅要有动作,还要能随出球需求改老师)**:
一个 flag 化的参考适配层,按每道题的需求在模仿奖励看到之前改造老师动作——
①**转方向**:整套动作绕锚点旋 φ(rally_yaw 机制复用,方向适配零成本);
②**加减速**:R14 的变速重定时机制复用(题目要求快/慢,老师同步快放/慢放);
③**改拍面**:把题目要求的拍面差在触球帧注入腕关节参考(机制同 --grip-rot 的逐帧腕反解,
但目标是每题的 demanded face 而非常量),前后各 N 帧平滑混合,挥拍路径不动;
④(预留)幅度/位移 morph。
与拍面指令通道的关系:通道告诉策略"要什么",适配器把"老师示范的"也改成要的——两头一起
把"模仿 vs 打球"的冲突消掉;阶段 1 第二波消融"适配器开/关"量它的净贡献。
