...
待办(顺序敏感):①难度地图/相位扫描用标定拍面 + rally_yaw 转正后**全部重算**(正手预期
复活,反手先行的结论待重审);②登记表已按 franco 口径落地(打直线为正,rally_yaw 单存,
分支 469590a);③**franco 拍板**:标定角是"烤进重定向"(机器人腕姿态旋转 40° 使其拍面对齐
人的真实拍面——一次修好所有模板先验)还是"保持原样靠指令通道"?④标定求解器产品化进
gen_stage1_questions(--grip)。
