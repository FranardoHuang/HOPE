...
**✅ 已点火(2026-07-06 深夜,franco 下令"开始跑吧")——S1 第一波 4 臂,全部在跑**。
与原计划 6 臂的差异:相位 A/B 两臂被题库选择消解(采纳 yikang 推荐的 **f21 真值帧版
bank**——难度低 5°、锚语义与考卷/登记表一致,相位假设已由 0d 扫描+双版 bank 回答;
f18-19 训练最优版留作第二波备选);权重臂推迟到第二波。改为增加"只换目标"臂,
四臂构成干净的递增阶梯:基线 → +题库目标 → +拍面奖励改锚 → +拍面观测。

| 臂(run_name) | 卡 | 组合(在基线之上逐级加) | 回答 |
| --- | --- | --- | --- |
| s1_A_main | GPU0 | 题库+拍面奖励锚指令+**拍面观测(175→179)**+腕踢除 | 方向成不成(主攻) |
| s1_B_noobs | GPU1 | 题库+拍面奖励锚指令+腕踢除(无观测=SMASH 式"速度隐含") | 观测通道净贡献(A−B);隐含够不够(B 自身) |
| s1_C_targets | GPU2 | 题库+腕踢除(拍面奖励仍锚 clip) | 反解目标单独的净贡献(C−D) |
| s1_D_base | GPU0 | 产品线原样 | 锚(同批同种子对照) |

共同配置:4096 envs、同种子、v5 _cal 正反手对、P2 配方(自适应σ+A1 噪声+midswing 2%)、
热启 P2 model_9600、4000 个新迭代(计数从 9600 → 13600,存档名按此读)。题库=
`/workspace/yikang/s1_v5cal_train.npz`(训练卷 822 正手+707 反手);loader 守卫生效
(HER 混采自动强制归零,防题目被重放目标覆盖)。日志:`/workspace/franco/s1_wave1/`,
运行目录在 **nohope_s1 worktree** 的 logs 下(分支代码,主检出不受扰)。
...
**判卷(~4 小时后,预计 07-06 凌晨)**:各臂 model_13600 → ONNX 导出(179 维臂注意导出器
输入维自检)→ 阶段 1 考卷:**按侧各跑一趟固定锚点**(正手锚 env(0.379,−0.547,0.856)、
反手锚 env(0.772,0.227,0.971),与题库锚逐位一致)、无旋、场馆速度档。北极星=回球率,
**起飞线 ≥50%**(反事实上限 100%);**分母一起记**(题库可解率:训练难度中位数正手 71°/
反手 37°;锥内 0%——超锥拍面正是拍面指令臂要证明学得会的东西)。

**第二波(第一波出信号后,~6 臂)**:SMASH 窗口奖励/相位噪声/区域采样(实现完即插)、
权重细扫 2 档 × 拍面奖励容差 2 档、curriculum(难度窗口自适应 vs 全开)、必要时 f18-19
训练最优 bank 臂。**阶段 1 预算合计 ≈ 2 波 ≈ 1-2 天墙钟。**
