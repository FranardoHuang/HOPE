...
### 发射工序(2026-07-06 立,第一波带病出发的复盘产物;此后每臂照此走)

**发射核对单(每臂六问,过不了不点火)**:
1. 动作对非 hopex?→ `strike_phase_per_clip` 显式传,值=登记表该对的锚相位(默认值是 hopex 的);
2. 登记表五件套齐?(缺烤入标记出题器 fail-closed 会拦——v4 实锤过);
3. 卷与动作同源同锚?(_cal 卷配 _cal 动作,锚=卷 meta 的 anchor_phase);
4. 观测契约与存档维度匹配?(179 臂=扩列存档+contract 放行);
5. 冒烟摘要必须含 **WARN 行**(只抓预期信号=确认偏误,这次的教训);两个 Isaac 不同秒起;
6. run_name 当场进下面的队列表,发射才算完成。

**点火队列(跨阶段统一;所有想跑的臂都进这张表,不再散落各节靠记忆)**:

| # | 臂 | 依赖(什么好了才能排) | 状态 |
| --- | --- | --- | --- |
| 跑着 | s1w2_A/B/C/D/E/F | — | 6 槽满 |
| 队 1 | s1w2_G_v4mo / H_obmo(动作源对比) | v4/斜录卷 ✅;等空槽 | 自动队列在守 |
