...
| 步骤 | 内容(人话) | 判据 | 规模 |
| --- | --- | --- | --- |
| 0a 诊断「目标不是解」 | ~~预期≈0~~ **已做(07-05 晚,2 万题/边):完美跟踪下入界率 正手 0.00%、反手 0.03%,落点误差中位 1.84/1.92 m——和 P2 训练实测(0.03%、1.9 m)严丝合缝。定罪:训练 0 回球 100% 是目标的锅(策略已把这套目标跟到头了)** | ✅ 一锤定音 | 已完成 |
| 0b 题目难度地图 | 按场馆分布出 10 万题逐题反解,统计"需要的拍面偏离模板几度"的分布+各层可解率 | 分层直方图 | CPU 小时级 |
| 0c v5 拍面扫描 | **已做(07-05 晚)**:触球帧朝前分量 v5 正/反 0.47/0.46 vs hopex 0.41/0.39——同病略好;**但 v5 是唯一存在"拍面朝前帧段"的模板**(反手相位 0.18-0.23 有 0.73,正手仅收尾;hopex 全程无一帧 ≥0.7)。疑点留档:v5 反手朝前段在触球标注 0.345 之前、仍在 franco 说的"前 3/7"内——若真实触球更早,反手拍面比标注帧好,franco 有空瞄一眼视频 | ✅ | 已完成 |
