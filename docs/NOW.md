...
| 3 | 【阶段1】**W3 动作组三选波(franco 07-07 提级)**:v4 对 / v5 对 / 斜录对 三臂同批 × W2 赢家配方,各配自家题库、**各用自家首帧等球锚**(统一新等球语义)+ 对照 | W2 判卷 + **1n 等球锚训练端实现** | 共跑 | **高优先(先 reward 后动作组,franco 序)** |
| 4 | 【阶段1】W4 巩固波:动作组赢家跑到底 + 换种子 + 等待/预警起步档搭车臂(claude) | W3 判卷 + 等待/预警设计代码 | 跑到底=独占,其余共跑 | 等 |
