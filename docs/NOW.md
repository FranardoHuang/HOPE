...
| 2 | 【阶段1】**W3 六臂合体波(franco 07-07 定稿:reward+动作组一波开,硬化植体重发)**:①公共对照 = v5 对 × 基准配方(**一臂两用**:奖励侧对照 + 动作组侧 v5 成员);②+击球窗整包;③+击球窗×拍面权重;④+难度课程(最易 30% 滚动扩窗);⑤v4 对 × 基准配方;⑥斜录对 × 基准配方。奖励变体都在 v5 对上跑,动作组都用基准配方跑,各配自家题库 v2 + **自家首帧等球锚**;判读两条轴独立:奖励轴(②③④−①)、动作组轴(⑤⑥−①) | 硬化植体已默认(摩擦+剪切)+ bank v2(已有)+ 1b 记账 + 1c 击球窗 + 1d 课程 loader + **1n 等球锚旗标** + 斜录 clip 几何转正(yikang 取证的补救项) | 共跑;6 卡下可每臂独占(一轮 ~2.5h) | **最高优先** |
