...
- **报数格式(franco 2026-07-08 定死)**:任何臂的读数一律报 **2×2×2 全表——正手/反手 ×
  击球率/上台率 × Isaac 训练内/MuJoCo 考卷**,八个数一个不能少(反手归零就是靠分侧才抓到的);
  训练内分侧指标已实装(virtual_hit/return_rate_forehand/backhand),考卷端本有分侧列。
